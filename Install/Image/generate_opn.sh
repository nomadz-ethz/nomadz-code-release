...
# Script based on original from NaoDevils
...
cp $DIR/installer.sh installer.sh
sed -i "s#IMAGE_CMP_SIZE=\".*\"#IMAGE_CMP_SIZE=\"$IMAGE_CMP_SIZE\"#" ./installer.sh
sed -i "s#IMAGE_RAW_SIZE=\".*\"#IMAGE_RAW_SIZE=\"$IMAGE_RAW_SIZE\"#" ./installer.sh

# copy data
dd if=./installer.sh of="$2" seek=$(($INSTALLER_OFFSET/$BLOCK_SIZE)) count=$(($INSTALLER_SIZE/$BLOCK_SIZE)) bs=$BLOCK_SIZE conv=notrunc
