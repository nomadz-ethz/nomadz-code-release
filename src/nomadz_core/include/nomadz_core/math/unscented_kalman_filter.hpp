...
namespace nomadz_core {
   ...
   */
  template <unsigned N> struct Manifold {
  ...
  namespace impl {
     ...
     */
    template <typename State, unsigned DOF, typename Array, bool IsManifold> struct MeanOfSigmaPoints {
     ...
     */
    template <typename State, unsigned DOF, bool Manifold> class UnscentedKalmanFilter {
    public:
      using CovarianceType = Eigen::Matrix<float, DOF, DOF>;            // The covariance size to use
      template <unsigned N> using Vectorf = Eigen::Matrix<float, N, 1>; // The vector size to use

    private:
      template <typename T> using SigmaArray = std::array<T, DOF * 2 + 1>; // The array type for the sigma points
    ...
    public:
      ...
      void update(const Vectorf<N>& measurement,
                  std::function<Vectorf<N>(const State&)> measurementModel,
     ...
     */
    template <typename State, unsigned DOF, bool Manifold>
    void UnscentedKalmanFilter<State, DOF, Manifold>::predict(std::function<void(State&)> dynamicModel,
    ...
    template <unsigned N>
    void UnscentedKalmanFilter<State, DOF, IsManifold>::update(const Vectorf<N>& measurement,
                                                               std::function<Vectorf<N>(const State&)> measurementModel,
                                                               const Eigen::Matrix<float, N, N>& measurementNoise) {
     ...
     */
    template <typename State, unsigned DOF, bool IsManifold>
    void UnscentedKalmanFilter<State, DOF, IsManifold>::update(float measurement,
                                                               std::function<float(const State&)> measurementModel,
     ...
     */
    template <typename State, unsigned DOF, typename Array> struct MeanOfSigmaPoints<State, DOF, Array, true> {
      ...
      static State run(const Array& sigmaPoints) {
        ...
        // Move the mean each step into the direction of the sigma points till the difference is minimal or the limit is
        // reached
        do {
          lastMean = mean;
          ++iterations;
     ...
     */
    template <typename State, unsigned DOF, typename Array> struct MeanOfSigmaPoints<State, DOF, Array, false> {
      /**
   ...
   */
  template <unsigned DOF> using UKF = impl::UnscentedKalmanFilter<Eigen::Matrix<float, DOF, 1>, DOF, false>;

  template <typename Manifold_> using UKFM = impl::UnscentedKalmanFilter<Manifold_, Manifold_::DOF, true>;
} // namespace nomadz_core
