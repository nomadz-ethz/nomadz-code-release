...
#include "nomadz_core/math/mean_calculator.hpp"

namespace nomadz_core {

  class LineFitter : public MeanCalculator<Eigen::Matrix<float, 2, 3>, Eigen::Vector2f> {
  public:
    LineFitter()
        : MeanCalculator<Eigen::Matrix<float, 2, 3>, Eigen::Vector2f>([](const Eigen::Vector2f& v) {
     ...
     */
    bool fit(Eigen::Vector2f& n0, float& d) const;
  };
   ...
   */
  class CircleFitter : public MeanCalculator<Eigen::Matrix<float, 2, 4>, Eigen::Vector2f> {
  public:
    CircleFitter()
        : MeanCalculator<Eigen::Matrix<float, 2, 4>, Eigen::Vector2f>([](const Eigen::Vector2f& v) {
            Eigen::Matrix<float, 2, 4> m;
            const Eigen::Vector2f v2(v.cwiseProduct(v));
     ...
     */
    bool fit(Eigen::Vector2f& center, float& radius) const;
  };
  ...
  static inline bool fitLine(const IteratorType& begin, const IteratorType& end, Eigen::Vector2f& n0, float& d) {
    LineFitter fitter;
   ...
   * @param d reference to a variable to be filled with the distance of the line
   */
  template <typename ContainerType> static inline bool fitLine(const ContainerType& points, Eigen::Vector2f& n0, float& d) {
   ...
   */
  template <typename IteratorType>
  static inline bool fitCircle(const IteratorType& begin, const IteratorType& end, Eigen::Vector2f& center, float& radius) {
    CircleFitter fitter;
    fitter.add(begin, end);
  ...
  static inline bool fitCircle(const ContainerType& points, Eigen::Vector2f& center, float& radius) {
