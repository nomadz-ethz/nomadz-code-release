...
import umsgpack
from controller import AnsiCodes, Keyboard, Robot
from imageserver import CamIndex, ImageServer
...
class Nao(Robot):
    ...
    def updateSensors(self):
        ...
        if self.key == ord("C"):
            ...
            if self.sensors["Touch"][1] < 1:
                print(
                    AnsiCodes.CYAN_FOREGROUND
                    + "Front head & chest button pressed."
                    + AnsiCodes.RESET
                )
            self.sensors["Touch"][0] = 1.0
            ...
            if self.sensors["Touch"][0] > 0:
                ...
                print(
                    AnsiCodes.CYAN_FOREGROUND
                    + "Front head & chest button released."
                    + AnsiCodes.RESET
                )
            for i in range(0, 4):
    ...
    def print_status(self):
        ...
        print(
            "Options:  Camera={}  Framerate={}  SendTicks={}".format(
                self.args.camera, self.args.fps, self.args.send_ticks
            )
        )
        print(" ")
        ...
        print(
            "     Press 'A' to simulate calibration gesture (front head sensor touched & chest button pressed)"
        )
    ...
    def __init__(self):
        ...
        if self.args.camera:
            ...
            self.bottomImageServer = ImageServer(
                self.args.bcam_addr, w, h, CamIndex.BOTTOM
            )
            ...
            if conn:
                ...
                try:
                    ...
                    packed = umsgpack.packb(
                        self.sensors, force_float_precision="single"
                    )
                    if len(packed) != self.MSGPACK_READ_LENGTH:
                        print(
                            AnsiCodes.RED_FOREGROUND
                            + "Msgpack packet size doesn't match LoLA specifications."
                            + AnsiCodes.RESET
                        )
                    conn.send(packed)
