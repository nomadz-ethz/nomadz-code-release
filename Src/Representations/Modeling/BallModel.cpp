...
BallModelCompressed::BallModelCompressed(const BallModel& ballModel)
    : lastPerception(ballModel.lastPerception), position(ballModel.estimate.position), velocity(ballModel.estimate.velocity),
      positionCovariance(ballModel.estimate.positionCovariance), timeWhenLastSeen(ballModel.timeWhenLastSeen),
      timeWhenDisappeared(ballModel.timeWhenDisappeared), timeSinceLastSeen(ballModel.timeSinceLastSeen),
      seenFraction(ballModel.seenFraction) {}
...
BallModelCompressed::operator BallModel() const {
  ...
  ballModel.estimate.positionCovariance = Matrix2x2<>(positionCovariance);
  ballModel.timeWhenLastSeen = timeWhenLastSeen;
  ballModel.timeWhenDisappeared = timeWhenDisappeared;
  ballModel.timeSinceLastSeen = timeSinceLastSeen;
