...
void MotionRequest::printOut(char* destination) const {
  ...
  case walk:
    if (walkRequest.mode == WalkRequest::targetMode) {
      ...
      sprintf(destination,
              ": %.0lf%% %.0lf%% %.0lf%%",
              walkRequest.speed.translation.x * 100,
              walkRequest.speed.translation.y * 100,
  ...
  COMPLEX_DRAWING("representation:MotionRequest:Image", {
    if (motion == walk || true) {
      ...
      if (walkRequest.speed != Pose2D(0.f, 0.f, 0.f)) {
        ...
        if (walkRequest.mode == WalkRequest::targetMode || walkRequest.mode == WalkRequest::percentageSpeedMode) {
