...
public:
  ...
  const Image::Pixel& fromYCbCrToHSI(const Image::Pixel& yCbCr) const {
    return table[yCbCr.y >> ColorReference::shiftFactor][yCbCr.cb >> ColorReference::shiftFactor]
...
ColorReference::MultiColor ColorReference::getColorClasses(const Image::Pixel* pixel) const {
  return colorTable[pixel->y >> shiftFactor][pixel->cb >> shiftFactor][pixel->cr >> shiftFactor];
}
...
ColorReference::MultiColor ColorReference::getColorClassesFromHSI(const Image::Pixel& pixel) const {
  ...
  if (thresholdGreen.hue.isInside(h) && thresholdGreen.saturation.isInside(s) && thresholdGreen.value.isInside(v)) {
    multiColor.colors |= green;
  }

  if (thresholdOrange.hue.isInside(h) && thresholdOrange.saturation.isInside(s) && thresholdOrange.value.isInside(v)) {
    multiColor.colors |= orange;
  }

  if (thresholdYellow.hue.isInside(h) && thresholdYellow.saturation.isInside(s) && thresholdYellow.value.isInside(v)) {
    multiColor.colors |= yellow;
  }

  if (thresholdBlue.hue.isInside(h) && thresholdBlue.saturation.isInside(s) && thresholdBlue.value.isInside(v)) {
    multiColor.colors |= blue;
  }

  if (thresholdRed.hue.isInside(h) && thresholdRed.saturation.isInside(s) && thresholdRed.value.isInside(v)) {
  ...
  if (pixel.i <= thresholdBlack.first) {
