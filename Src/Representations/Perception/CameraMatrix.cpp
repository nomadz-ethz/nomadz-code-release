...
CameraMatrix::CameraMatrix(const Pose3D& pose) : CameraMatrix() {
...
void CameraMatrix::draw() const {
  ...
  COMPLEX_DRAWING("representation:CameraMatrix:Field", {
    ...
    // calculate the projection to the ground of the intersection points of the line parallel to the horizon and the image
    // borders
    Vector2<int> beginPoint;
    Vector2<int> endPoint;
    if (Geometry::getIntersectionPointsOfLineAndRectangle(Vector2<int>(0, 0),
                                                          Vector2<int>(cameraInfo.width - 1, cameraInfo.height - 1),
                                                          lineBelowHorizon,
                                                          beginPoint,
                                                          endPoint)) {
      ...
      LINE("representation:CameraMatrix:Field",
           pointOnField[4].x,
           pointOnField[4].y,
           pointOnField[5].x,
           pointOnField[5].y,
    ...
    if (pointOnField[0].x != 0 || pointOnField[0].y != 0) {
      ...
      CIRCLE("representation:CameraMatrix:Field",
             pointOnField[0].x,
             pointOnField[0].y,
             100,
             50,
...
void RobotCameraMatrix::draw() const {
  ...
  COMPLEX_DRAWING("representation:RobotCameraMatrix:Field", {
    ...
    if (pointOnField[0].x != 0 || pointOnField[0].y != 0) {
      boundary.add(pointOnField[0]);
      CIRCLE("representation:RobotCameraMatrix:Field",
             pointOnField[0].x,
             pointOnField[0].y,
             100,
             50,
    ...
    LINE("representation:RobotCameraMatrix:Field",
         boundary.x.min,
         boundary.y.min,
         boundary.x.max,
         boundary.y.min,
...
RobotCameraMatrix::RobotCameraMatrix(const RobotDimensions& robotDimensions,
                                     const float headYaw,
...
void RobotCameraMatrix::computeRobotCameraMatrix(const RobotDimensions& robotDimensions,
                                                 const float headYaw,
                                                 const float headPitch,
                                                 const CameraCalibration& cameraCalibration,
                                                 bool upperCamera) {
  *this = RobotCameraMatrix();
  ...
  rotateZ(headYaw);
  // TODO[flip] Is this flip necessary
  rotateY(headPitch);
  // TODO[flip] Check if translation here should have been inverted
  if (upperCamera) {
