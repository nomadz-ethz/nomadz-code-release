...
JPEGImage& JPEGImage::operator=(const Image& src) {
  ...
  if (!cInfo.dest) {
    cInfo.dest =
      (jpeg_destination_mgr*)(*cInfo.mem->alloc_small)((j_common_ptr)&cInfo, JPOOL_PERMANENT, sizeof(jpeg_destination_mgr));
  }
...
void JPEGImage::toAiboAlignment(const unsigned char* src, unsigned char* dst) const {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (; pSrc < pSrcLineEnd; pSrc += 4, ++pDst) {
      ...
      p3 =
        _mm_loadu_si128(pSrc + 3); // yPadd13 cb13 y13 cr13 yPadd14 cb14 y14 cr14 yPadd15 cb15 y15 cr15 yPadd16 cb16 y16 cr16
