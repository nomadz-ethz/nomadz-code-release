...
#ifdef ENABLE_ROS
#include "nomadz_msgs/msg/image_coordinate_system.hpp"
#endif
STREAMABLE_DECLARE(ImageCoordinateSystem)

STREAMABLE_ROS(ImageCoordinateSystem, {
...
public:
   ...
   */
  Vector2<> toHorizonAligned(const Vector2<int>& imageCoords) const { return invRotation * Vector2<>(imageCoords); }
   ...
   * @return The point in horizon-aligned coordinates.
   */
  Vector2<> toHorizonAligned(const Vector2<>& imageCoords) const { return invRotation * imageCoords; }
   ...
   */
  Vector2<> toHorizonBased(const Vector2<int>& imageCoords) const { return invRotation * (Vector2<>(imageCoords) - origin); }
   ...
   * @return The point in horizon-based coordinates.
   */
  Vector2<> toHorizonBased(const Vector2<>& imageCoords) const { return invRotation * (imageCoords - origin); }
  ...
  Vector2<> toCorrected(const Vector2<>& imageCoords) const {
    ...
    return Vector2<>(
      float(cameraInfo.opticalCenter.x -
            tan(atan((cameraInfo.opticalCenter.x - imageCoords.x) / cameraInfo.focalLength) - factor * offset.x) *
              cameraInfo.focalLength),
      float(cameraInfo.opticalCenter.y +
            tan(atan((imageCoords.y - cameraInfo.opticalCenter.y) / cameraInfo.focalLength) - factor * offset.y) *
              cameraInfo.focalLength));
  }
  ...
  Vector2<> fromCorrectedLinearized(const Vector2<>& coords) const {
    ...
    return Vector2<>(coords.x - cameraInfo.focalLength * offset.x * (a + coords.y * b) * temp2,
                     (coords.y + temp1 * a) * temp2);
  }
   ...
   * @return The incorrected point.
   */
  Vector2<> fromCorrectedLinearized(const Vector2<int>& coords) const { return fromCorrectedLinearized(Vector2<>(coords)); }
   ...
   * @return The incorrected point.
   */
  Vector2<> toCorrectedLinearized(const Vector2<int>& coords) const { return toCorrectedLinearized(Vector2<>(coords)); }
  ...
  void draw() const,
     ...
     */
    FIELD_WRAPPER_DEFAULT(Matrix2x2<>, nomadz_msgs::msg::ImageCoordinateSystem::rotation, rotation),
    FIELD_WRAPPER_DEFAULT(Matrix2x2<>,
                          nomadz_msgs::msg::ImageCoordinateSystem::inv_rotation,
                          invRotation), /**< The rotation from the image coordinates to the horizon-aligned coordinates. */
    FIELD_WRAPPER_DEFAULT(Vector2<>,
                          nomadz_msgs::msg::ImageCoordinateSystem::origin,
                          origin), /**< The origin of the horizon in image coordinates. */
    FIELD_WRAPPER_DEFAULT(Vector2<>,
                          nomadz_msgs::msg::ImageCoordinateSystem::offset,
                          offset), /**< The offset of the previous image to the current one. */
    FIELD_WRAPPER(
      float, 0, nomadz_msgs::msg::ImageCoordinateSystem::a, a), /**< Constant part of equation to motion distortion. */
    FIELD_WRAPPER(
      float, 0, nomadz_msgs::msg::ImageCoordinateSystem::b, b), /**< Linear part of equation to motion distortion. */
