...
STREAMABLE_DECLARE_LEGACY(ColorReference)
STREAMABLE_DECLARE_LEGACY(ColorThreshold)
STREAMABLE_DECLARE_LEGACY(HSVColorDefinition)

class ColorReference : public ColorReferenceBaseWrapper {
...
public:
  ...
  STREAMABLE_LEGACY(ColorThreshold,
                    {
                      ,
                      (T)(0)first,
                      (T)(0)second,
   ...
   */
  STREAMABLE_LEGACY(HSVColorDefinition,
                    {
                      ,
                      (Range<>)(0)hue,
                      (Range<>)(0)saturation,
  ...
  struct MultiColor {
    ...
    bool operator!=(ColorClasses::Color color) const { return !(*this == color); }

    bool isNone() const { return colors == none; }

    bool isOrange() const { return (colors & orange) != 0; }

    bool isYellow() const { return (colors & yellow) != 0; }

    bool isBlue() const { return (colors & blue) != 0; }

    bool isWhite() const { return (colors & white) != 0; }

    bool isGreen() const { return (colors & green) != 0; }

    bool isBlack() const { return (colors & black) != 0; }
  ...
  bool isNone(const Image::Pixel* pixel) const { return getColorClasses(pixel).isNone(); }

  bool isOrange(const Image::Pixel* pixel) const { return getColorClasses(pixel).isOrange(); }

  bool isYellow(const Image::Pixel* pixel) const { return getColorClasses(pixel).isYellow(); }

  bool isBlue(const Image::Pixel* pixel) const { return getColorClasses(pixel).isBlue(); }

  bool isWhite(const Image::Pixel* pixel) const { return getColorClasses(pixel).isWhite(); }

  bool isGreen(const Image::Pixel* pixel) const { return getColorClasses(pixel).isGreen(); }

  bool isBlack(const Image::Pixel* pixel) const { return getColorClasses(pixel).isBlack(); }

  bool isRed(const Image::Pixel* pixel) const { return getColorClasses(pixel).isRed(); }
