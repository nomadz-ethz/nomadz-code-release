...
STREAMABLE_DECLARE_LEGACY(Thumbnail)
class Thumbnail : public ThumbnailBaseWrapper {
public:
  template <class Pixel> class TImage : public Streamable {
  public:
  ...
  public:
    ...
    inline Pixel* operator[](const int y) { return image + y * width; }

    inline const Pixel* operator[](const int y) const { return image + y * width; }

  private:
    virtual void serialize(In* in, Out* out);
  };

  template <class Pixel, class PixelUncompressed> class TImageCompressed : public TImage<Pixel> {
  public:
...
template <class Pixel> Thumbnail::TImage<Pixel>::TImage() : width(maxWidth), height(maxHeight) {
...
template <class Pixel> Thumbnail::TImage<Pixel>::~TImage() {
  SystemCall::alignedFree(imagePadding);
}

template <class Pixel> void Thumbnail::TImage<Pixel>::setResolution(int width, int height) {
...
template <class Pixel> Thumbnail::TImage<Pixel>& Thumbnail::TImage<Pixel>::operator=(const TImage& other) {
  width = other.width;
  height = other.height;
  memcpy(image, other.image, maxWidth * maxHeight * sizeof(Pixel));
  return *this;
...
template <class Pixel> void Thumbnail::TImage<Pixel>::serialize(In* in, Out* out) {
