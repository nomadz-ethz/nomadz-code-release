 ...
 *
 * @note The original authors are Colin Graf and Alexis Tsogias
 */
...
void TeamMateData::draw() const {
  ...
  for (int i = 1; i < numOfPlayers; ++i) {
    if (timeStamps[i]) {
      ...
      CIRCLE("representation:TeamMateData",
             rPos.x,
             rPos.y,
             radius,
             20,
      ...
      LINE("representation:TeamMateData", rPos.x, rPos.y, dirPos.x, dirPos.y, 20, Drawings::ps_solid, posCol);
      // Player number
      DRAWTEXT("representation:TeamMateData", rPos.x, rPos.y, 150, ColorClasses::black, i);
      // Role
      DRAWTEXT("representation:TeamMateData",
               rPos.x + 200,
               rPos.y - 200,
               150,
               ColorClasses::black,
               BehaviorStatus::getName(behaviorStatus[i].role));
      ...
      // Ball position
      CIRCLE(
        "representation:TeamMateData", bPos.x, bPos.y, 50, 20, Drawings::ps_solid, ballCol, Drawings::bs_solid, ballCol);
