...
public:
  ...
  /** converts this image to an opencv usable matrix with all 3 channels in YCrCb
   */
  cv::Mat convertToCVMat() const;

  void copyFromRawImagePtr(const unsigned char* buffer, int width, int height);
  ...
  /** Resizes and fills up the image using the Image directly.
   */
  Image resizeAndFillUpInterLinear(int newWidth, int newHeight) const;
