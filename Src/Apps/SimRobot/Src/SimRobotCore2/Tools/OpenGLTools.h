...
public:
   ...
   */
  static inline void
  convertTransformation(const Matrix3x3<>* rotation, const Vector3<>* translation, float transformation[16]) {
    if (rotation) {
      ...
      transformation[1] = transformation[2] = transformation[4] = transformation[6] = transformation[8] = transformation[9] =
        (float)0;
    }
    if (translation) {
   ...
   */
  static inline void computeCameraTransformation(const Vector3<>& eyePosition3D,
    ...
    transformation[12] = (float)(transformation[0] * -eyePosition3D.x - transformation[4] * eyePosition3D.y -
                                 transformation[8] * eyePosition3D.z);
    transformation[13] = (float)(transformation[1] * -eyePosition3D.x - transformation[5] * eyePosition3D.y -
                                 transformation[9] * eyePosition3D.z);
    transformation[14] = (float)(transformation[2] * -eyePosition3D.x - transformation[6] * eyePosition3D.y -
                                 transformation[10] * eyePosition3D.z);
