...
private:
  ...
  float fovy;
  const float zoomMinDistance = 0.5;
  float projection[16];
  int viewport[4];
  ...
  void moveCameraTarget(float x, float y);
  void updateCameraTransformation();

  bool intersectRayAndPlane(
...
public:
  ...
  virtual void adjustCameraVerticalFocusHeight(float heightChanges);
  virtual void resetCameraVerticalFocusHeight();
