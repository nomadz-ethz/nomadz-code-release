...
SimObjectWidget::SimObjectWidget(SimObject& simObject)
    : QGLWidget(QGLFormat(QGL::SampleBuffers | QGL::AccumBuffer), 0, Simulation::simulation->renderer.getWidget()),
      object(dynamic_cast<SimRobot::Object&>(simObject)), objectRenderer(simObject), wkey(false), akey(false), skey(false),
      dkey(false) {
  ...
  objectRenderer.setSurfaceShadeMode(SimRobotCore2::Renderer::ShadeMode(
    settings->value("SurfaceShadeMode", int(objectRenderer.getSurfaceShadeMode())).toInt()));
  objectRenderer.setPhysicsShadeMode(SimRobotCore2::Renderer::ShadeMode(
    settings->value("PhysicsShadeMode", int(objectRenderer.getPhysicsShadeMode())).toInt()));
  objectRenderer.setDrawingsShadeMode(SimRobotCore2::Renderer::ShadeMode(
    settings->value("DrawingsShadeMode", int(objectRenderer.getDrawingsShadeMode())).toInt()));
  objectRenderer.setCameraMode(
    SimRobotCore2::Renderer::CameraMode(settings->value("CameraMode", int(objectRenderer.getCameraMode())).toInt()));
  fovy = settings->value("FovY", objectRenderer.getFovY()).toInt();
  objectRenderer.setDragPlane(
    SimRobotCore2::Renderer::DragAndDropPlane(settings->value("DragPlane", int(objectRenderer.getDragPlane())).toInt()));
  objectRenderer.setDragMode(
    SimRobotCore2::Renderer::DragAndDropMode(settings->value("DragMode", int(objectRenderer.getDragMode())).toInt()));
...
void SimObjectWidget::keyPressEvent(QKeyEvent* event) {
  ...
  case Qt::Key_E:
    event->accept();
    objectRenderer.adjustCameraVerticalFocusHeight(0.05);
    updateGL();
    break;
  ...
  case Qt::Key_C:
    event->accept();
    objectRenderer.resetCameraVerticalFocusHeight();
    updateGL();
    break;
...
void SimObjectWidget::update() {
  QGLWidget::update();
  objectRenderer.updateCameraTarget();
}
...
QMenu* SimObjectWidget::createUserMenu() const {
...

  {
    ...
    qobject_cast<QAction*>(
      signalMapper->mapping(objectRenderer.getRenderFlags() & (SimRobotCore2::Renderer::enableDrawingsOcclusion |
                                                               SimRobotCore2::Renderer::enableDrawingsTransparentOcclusion)))
      ->setChecked(true);
  }
