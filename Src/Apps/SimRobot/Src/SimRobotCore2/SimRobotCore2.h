...
namespace SimRobotCore2 {
  ...
  public:
    ...
    enum DragAndDropPlane { xyPlane, xzPlane, yzPlane };

    /** The drag and drop modes for physical objects */
    enum DragAndDropMode { keepDynamics = 0, resetDynamics, adoptDynamics, applyDynamics };
     ...
     * @parem hasSharedDisplayLists Whether the OpenGL has shared display lists and textures with another context that is
     * already initialized.
     */
    ...
    /** Sets the size of the currently selected OpenGL renderer device. Call this once at the begining to initialize the
     * size.
  ...
  public:
    std::string configName; /**< configName attribute of Body element */
  ...
  public:
    enum SensorType { boolSensor, floatSensor, cameraSensor, floatArraySensor, noSensor };
    ...
    /**
     * Pre-renders the images of multiple camera sensors of the same type at once which improves the performance of camera
     * image rendering.
