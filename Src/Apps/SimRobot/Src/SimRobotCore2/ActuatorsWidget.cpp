...
ActuatorWidget::ActuatorWidget(SimRobotCore2::ActuatorPort* actuator, QWidget* parent)
    : QWidget(parent), actuatorName(actuator->getFullName()), actuator(actuator), value(0) {
  setMaximumWidth(80);
...
ActuatorsWidget::ActuatorsWidget() {
  ...
  settings.endGroup();
  for (QString actuator : openedActuators)
...
void ActuatorsWidget::openActuator(const QString& actuatorName) {
  ...
  SimRobotCore2::ActuatorPort* actuator =
    (SimRobotCore2::ActuatorPort*)CoreModule::application->resolveObject(actuatorName, SimRobotCore2::actuatorPort);
  if (!actuator)
    return;
