...
SensorWidget::SensorWidget(SimRobotCore2::SensorPort* sensor)
    : pen(QColor::fromRgb(255, 0, 0)), sensor(sensor), mineData(0) {
...
void SensorWidget::paintFloatArrayWithLimitsAndWithoutDescriptions() {
  ...
  for (int i = 0; i < sensorDimensions[0]; i++) {
    ...
    painter.fillRect(QRectF(i * widthPerValue, this->height() * (1 - factor), widthPerValue, this->height() * factor),
                     brush2);
