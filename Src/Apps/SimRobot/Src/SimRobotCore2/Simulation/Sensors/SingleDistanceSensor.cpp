...
void SingleDistanceSensor::DistanceSensor::staticCollisionCallback(SingleDistanceSensor::DistanceSensor* sensor,
                                                                   dGeomID geom1,
                                                                   dGeomID geom2) {
  ...
  for (int i = 0; i < contacts; ++i) {
    ...
    const float sqrDistance =
      (Vector3<>((float)contactGeom.pos[0], (float)contactGeom.pos[1], (float)contactGeom.pos[2]) - sensor->pose.translation)
        .squareAbs();
...
void SingleDistanceSensor::DistanceSensor::staticCollisionWithSpaceCallback(SingleDistanceSensor::DistanceSensor* sensor,
                                                                            dGeomID geom1,
                                                                            dGeomID geom2) {
  ASSERT(geom1 == sensor->geom);
  ASSERT(dGeomIsSpace(geom2));
