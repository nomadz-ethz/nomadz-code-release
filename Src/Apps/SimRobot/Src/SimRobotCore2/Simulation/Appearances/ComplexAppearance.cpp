...
void ComplexAppearance::createGraphics() {
  ...
  if (initializedContexts == 0) {
    ...
    if (verticesSize > 0) {
      ...
      for (std::list<PrimitiveGroup*>::const_iterator iter = primitiveGroups.begin(), end = primitiveGroups.end();
           iter != end;
           ++iter) {
        ...
        for (std::list<unsigned int>::iterator iter = primitiveGroup.vertices.begin(), end = primitiveGroup.vertices.end();
             iter != end;) {
...
void ComplexAppearance::assembleAppearances() const {
  ...
  if (verticesSize > 0) {
    ...
    for (std::list<PrimitiveGroup*>::const_iterator iter = primitiveGroups.begin(), end = primitiveGroups.end(); iter != end;
         ++iter) {
      const PrimitiveGroup& primitiveGroup = *(*iter);
      glBegin(primitiveGroup.mode);
      for (std::list<unsigned int>::const_iterator iter = primitiveGroup.vertices.begin(),
                                                   end = primitiveGroup.vertices.end();
           iter != end;
           ++iter) {
    ...
    ++iter)
    {
      const PrimitiveGroup& primitiveGroup = *(*iter);
      glBegin(primitiveGroup.mode);
      for(std::list<unsigned int>::const_iterator iter = primitiveGroup.vertices.begin(), end =
    primitiveGroup.vertices.end(); iter != end; ++iter)
      {
        const unsigned int i = *iter;
        glVertex3fv(&vertexLibrary[i].x);
