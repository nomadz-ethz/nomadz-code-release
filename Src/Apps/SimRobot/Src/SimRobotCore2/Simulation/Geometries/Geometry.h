...
public:
  ...
  private:
    ...
    mutable std::unordered_map<const Material*, float>
      materialToRollingFriction; /**< A pointer map to speed up rolling friction lookups */
  ...
  bool immaterial;      /**< Whether this geometry will not be used to generate contact points when it collides with another
                           geometry */
  float innerRadius;    /**< The radius of a sphere that is enclosed by the geometry. This radius is used for implementing a
                           fuzzy but fast distance sensor */
  ...
  std::list<SimRobotCore2::CollisionCallback*>*
    collisionCallbacks; /**< Collision callback functions registered by another SimRobot module */
...
private:
  ...
  virtual bool registerDrawing(SimRobotCore2::Controller3DDrawing& drawing) {
    return ::PhysicalObject::registerDrawing(drawing);
  }
  virtual bool unregisterDrawing(SimRobotCore2::Controller3DDrawing& drawing) {
    return ::PhysicalObject::unregisterDrawing(drawing);
  }
