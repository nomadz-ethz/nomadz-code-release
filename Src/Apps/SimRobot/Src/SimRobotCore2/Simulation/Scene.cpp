...
void Scene::createGraphics(bool isShared) {
  ...
  if (!isShared) {
    ...
    if (!textures.empty()) {
      for (std::unordered_map<std::string, Texture>::iterator iter = textures.begin(), end = textures.end(); iter != end;
           ++iter)
...
Scene::Light::Light()
    : constantAttenuation(1.f), linearAttenuation(0.f), quadraticAttenuation(0.f), spotCutoff(float(M_PI)),
      spotDirection(0.f, 0.f, -1.f), spotExponent(0.f) {
