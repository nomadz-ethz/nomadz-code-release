...
void PhysicalObject::createPhysics() {
  ...
  for (std::list<PhysicalObject*>::const_iterator iter = physicalChildren.begin(), end = physicalChildren.end(); iter != end;
       ++iter) {
...
void PhysicalObject::drawPhysics(unsigned int flags) const {
  if (flags & SimRobotCore2::Renderer::showControllerDrawings)
    for (std::list<SimRobotCore2::Controller3DDrawing*>::const_iterator iter = controllerDrawings.begin(),
                                                                        end = controllerDrawings.end();
         iter != end;
         ++iter)
      (*iter)->draw();
  for (std::list<PhysicalObject*>::const_iterator iter = physicalDrawings.begin(), end = physicalDrawings.end(); iter != end;
       ++iter)
...
bool PhysicalObject::unregisterDrawing(SimRobotCore2::Controller3DDrawing& drawing) {
  for (std::list<SimRobotCore2::Controller3DDrawing*>::iterator iter = controllerDrawings.begin(),
                                                                end = controllerDrawings.end();
       iter != end;
       ++iter)
    if (*iter == &drawing) {
