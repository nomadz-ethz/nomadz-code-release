...
void Body::createPhysics() {
  ...
  // add geometries
  for (std::list<::PhysicalObject*>::const_iterator iter = physicalDrawings.begin(), end = physicalDrawings.end();
       iter != end;
       ++iter) {
...
void Body::addGeometry(const Pose3<>& parentOffset, Geometry& geometry) {
  ...
  for (std::list<::PhysicalObject*>::const_iterator iter = geometry.physicalDrawings.begin(),
                                                    end = geometry.physicalDrawings.end();
       iter != end;
       ++iter) {
    Geometry* geometry = dynamic_cast<Geometry*>(*iter);
    ASSERT(geometry);
...
void Body::move(const Vector3<>& offset) {
  ...
  Simulation::simulation->scene->lastTransformationUpdateStep =
    Simulation::simulation->simulationStep - 1; // enforce transformation update
}
...
bool Body::getPose(float* pos, float (*rot)[3]) const {
  ...
  rot[0][0] = pose.rotation.c0.x;
  rot[0][1] = pose.rotation.c0.y;
  rot[0][2] = pose.rotation.c0.z;
  rot[1][0] = pose.rotation.c1.x;
  rot[1][1] = pose.rotation.c1.y;
  rot[1][2] = pose.rotation.c1.z;
  rot[2][0] = pose.rotation.c2.x;
  rot[2][1] = pose.rotation.c2.y;
  rot[2][2] = pose.rotation.c2.z;
  return true;
}
...
void Body::move(const float* pos, const float (*rot)[3]) {
  ...
  Pose3<> newPose(Matrix3x3<>(Vector3<>(rot[0][0], rot[0][1], rot[0][2]),
                              Vector3<>(rot[1][0], rot[1][1], rot[1][2]),
                              Vector3<>(rot[2][0], rot[2][1], rot[2][2])),
