...
Simulation::Simulation()
    : scene(0), physicalWorld(0), rootSpace(0), staticSpace(0), movableSpace(0), currentFrameRate(0), simulationStep(0),
      simulatedTime(0), collisions(0), contactPoints(0), contactGroup(0), lastFrameRateComputationTime(0),
      lastFrameRateComputationStep(0) {
  ASSERT(simulation == 0);
  simulation = this;
}
...
void Simulation::staticCollisionCallback(Simulation* simulation, dGeomID geomId1, dGeomID geomId2) {
  ...
  if (geometry1->collisionCallbacks && !geometry2->immaterial) {
    for (std::list<SimRobotCore2::CollisionCallback*>::iterator i = geometry1->collisionCallbacks->begin(),
                                                                end = geometry1->collisionCallbacks->end();
         i != end;
         ++i)
  ...
  if (geometry2->collisionCallbacks && !geometry1->immaterial) {
    for (std::list<SimRobotCore2::CollisionCallback*>::iterator i = geometry2->collisionCallbacks->begin(),
                                                                end = geometry2->collisionCallbacks->end();
         i != end;
         ++i)
    ...
    if (bodyId1)
      ...
      case dCylinderClass:
        if (geometry1->material->getRollingFriction(*geometry2->material, rollingFriction)) {
          ...
          angularVel -=
            Vector3<>(angularVel)
              .normalize(std::min(angularVel.abs(),
                                  std::abs(simulation->scene->gravity) * rollingFriction * simulation->scene->stepLength));
