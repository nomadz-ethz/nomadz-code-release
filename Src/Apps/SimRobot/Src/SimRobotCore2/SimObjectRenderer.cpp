...
SimObjectRenderer::SimObjectRenderer(SimObject& simObject)
    : simObject(simObject), width(0), height(0), cameraMode(targetCam), defaultCameraPos(-3.f, -6.f, 4.f),
      cameraPos(defaultCameraPos), fovy(40.f), surfaceShadeMode(smoothShading), physicsShadeMode(noShading),
      drawingsShadeMode(noShading), renderFlags(enableLights | enableTextures | showCoordinateSystem), dragging(false),
      dragPlane(xyPlane), dragMode(keepDynamics), moving(false), movingLeftStartTime(0), movingRightStartTime(0),
      movingUpStartTime(0), movingDownStartTime(0) {}
...
void SimObjectRenderer::draw() {
  ...
  if (moving) {
    ...
    moveCamera(float(x) * 0.003f, float(y) * 0.003f);
  }
  ...
  // note: a not-physical-object has a constant offset pose relative its parent. hence there is no transformation update
  // required to draw not-physical-objects
  ...
  // since each object will be drawn relative to its parent we need to shift the coordinate system when we want the object to
  // be in the center
  ...
  if (renderFlags & showCoordinateSystem) {
    ...
    glVertex3f(0.3, 0, 0.005);
    glColor3f(0, 1, 0);
    glVertex3f(0, 0, 0.005);
    glVertex3f(0, 0.3, 0.005);
    glColor3f(0, 0, 1);
    glVertex3f(0, 0, 0.005);
    glVertex3f(0, 0, 0.305);
    glEnd();
...
void SimObjectRenderer::moveCameraTarget(float x, float y) {
  if (cameraMode == SimRobotCore2::Renderer::targetCam) {
    Vector3<> v = cameraTarget - cameraPos;
    float cameraDirection = std::atan2(v.y, v.x);
    float fieldX = y * -sin(cameraDirection) + x * cos(cameraDirection);
    float fieldY = x * sin(cameraDirection) + y * cos(cameraDirection);
    cameraTarget.x += fieldX;
    cameraTarget.y += fieldY;
    cameraPos.x += fieldX;
    cameraPos.y += fieldY;
  }
  updateCameraTransformation();
...
void SimObjectRenderer::toggleCameraMode() {
  setCameraMode(cameraMode == SimRobotCore2::Renderer::targetCam ? SimRobotCore2::Renderer::freeCam
                                                                 : SimRobotCore2::Renderer::targetCam);
}
...
bool SimObjectRenderer::intersectRayAndPlane(
  const Vector3<>& point, const Vector3<>& v, const Vector3<>& plane, const Vector3<>& n, Vector3<>& intersection) const {
...
Body* SimObjectRenderer::selectObject(const Vector3<>& projectedClick) {
  ...
  dSpaceCollide2(ray,
                 (dGeomID)Simulation::simulation->movableSpace,
                 &callback,
                 (dNearCallback*)&Callback::staticCollisionWithSpaceCallback);
  dGeomDestroy(ray);
...
bool SimObjectRenderer::startDrag(int x, int y, DragType type) {
  ...
  if (&simObject == Simulation::simulation->scene) {
    ...
    if (dragSelection) {
      switch (dragPlane) {
      case xyPlane:
        dragPlaneVector = Vector3<>(0.f, 0.f, 1.f);
        break;
      case xzPlane:
        dragPlaneVector = Vector3<>(0.f, 1.f, 0.f);
        break;
      case yzPlane:
        dragPlaneVector = Vector3<>(1.f, 0.f, 0.f);
        break;
      }
      if (type == dragNormalObject) {
        if (focusObject == dragSelection) {
          focusObject = nullptr;
        } else {
          focusObject = dragSelection;
          verticalOffset = 0;
        }
      } else {
        focusObject = nullptr;
      ...
      if (!intersectRayAndPlane(
            cameraPos, projectedClick - cameraPos, dragSelection->pose.translation, dragPlaneVector, dragStartPos))
        dragSelection = 0;
      else {
...
bool SimObjectRenderer::moveDrag(int x, int y) {
  ...
  if (!dragSelection) // camera control
  {
    if (cameraMode == SimRobotCore2::Renderer::targetCam) {
      if (dragType == dragRotate || dragType == dragRotateWorld) {
        focusObject = nullptr;
        moveCameraTarget((y - dragStartPos.y) * 0.005, (x - dragStartPos.x) * 0.005);
      } else {
        rotateCamera((x - dragStartPos.x) * -0.01f, (y - dragStartPos.y) * -0.01f);
      }    // if(dragType == DRAG_NORMAL)
    } else // if(cameraMode == SimRobotCore2::Renderer::FREECAM)
    {
      if (dragType == dragRotate || dragType == dragRotateWorld) {
      } else // if(dragType == DRAG_NORMAL)
        rotateCamera(-(x - dragStartPos.x) * -0.003f, -(y - dragStartPos.y) * 0.003f);
    }
  ...
  } else // object control
  {
    ...
    Vector3<> currentPos;
    if (intersectRayAndPlane(
          cameraPos, projectedClick - cameraPos, dragSelection->pose.translation, dragPlaneVector, currentPos)) {
      if (dragType == dragRotate || dragType == dragRotateWorld) {
...
void SimObjectRenderer::updateCameraTarget() {
  if (focusObject) {
    Vector3<> v = cameraPos - cameraTarget;
    cameraTarget = focusObject->pose.translation + Vector3<>(0, 0, verticalOffset);
    cameraPos = v + cameraTarget;
    updateCameraTransformation();
  }
}
...
void SimObjectRenderer::adjustCameraVerticalFocusHeight(float heightChanges) {
  verticalOffset += heightChanges;
}

void SimObjectRenderer::resetCameraVerticalFocusHeight() {
  verticalOffset = 0;
}
