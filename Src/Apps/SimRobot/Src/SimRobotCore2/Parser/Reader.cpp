...
bool Reader::readFile(const std::string& fileName) {
  ...
  public:
    static void
    xmlErrorHandler(Reader* reader, const char* msg, xmlParserSeverities severity, xmlTextReaderLocatorPtr locator) {
  ...
  xmlTextReaderPtr reader = xmlReaderForFile(
    fileName.c_str(), "UTF-8" /* "ISO-8859-1" */, 0); // using UTF-8 since it is the native format used by libxml2
  if (!reader)
    return false;
