...
Parser::Parser()
    : errors(0), sceneMacro(0), recordingMacroElement(0), replayingMacroElement(0), element(0), elementData(0),
  ...
  static const ElementInfo elements[] = {
    ...
    {"Scene", sceneClass, &Parser::sceneElement, 0, 0, 0, solverClass, setClass | bodyClass | compoundClass | lightClass},
    ...
    {"Compound",
     ...
     setClass | jointClass | compoundClass | bodyClass | appearanceClass | geometryClass | extSensorClass},
    {"Body",
     bodyClass,
     &Parser::bodyElement,
     0,
     ...
     massClass,
     translationClass | rotationClass,
     setClass | jointClass | appearanceClass | geometryClass | massClass | intSensorClass | extSensorClass},
    ...
    {"InertiaMatrixMass",
     massClass,
     &Parser::inertiaMatrixMassElement,
     0,
     constantFlag,
     0,
     translationClass | rotationClass,
    ...
    {"CylinderGeometry",
     geometryClass,
     &Parser::cylinderGeometryElement,
     0,
     0,
     0,
     translationClass | rotationClass | materialClass,
    ...
    {"SphereAppearance",
     appearanceClass,
     &Parser::sphereAppearanceElement,
     0,
     constantFlag,
     surfaceClass,
     translationClass | rotationClass,
    ...
    {"CylinderAppearance",
     appearanceClass,
     &Parser::cylinderAppearanceElement,
     0,
     constantFlag,
     surfaceClass,
     translationClass | rotationClass,
    ...
    {"CapsuleAppearance",
     appearanceClass,
     &Parser::capsuleAppearanceElement,
     0,
     constantFlag,
     surfaceClass,
     translationClass | rotationClass,
    ...
    {"ComplexAppearance",
     appearanceClass,
     &Parser::complexAppearanceElement,
     0,
     constantFlag,
     surfaceClass | verticesClass | primitiveGroupClass,
     translationClass | rotationClass | texCoordsClass,
     setClass | primitiveGroupClass | appearanceClass},
    ...
    {"Triangles",
     primitiveGroupClass,
     &Parser::trianglesElement,
     &Parser::trianglesAndQuadsText,
     textFlag | constantFlag,
     0,
     0,
     0},
    ...
    {"Gyroscope", intSensorClass, &Parser::gyroscopeElement, 0, 0, 0, 0, 0},
    {"Accelerometer", intSensorClass, &Parser::accelerometerElement, 0, 0, 0, 0, 0},
    {"Camera", extSensorClass, &Parser::cameraElement, 0, 0, 0, translationClass | rotationClass, 0},
    {"CollisionSensor",
     intSensorClass,
     &Parser::collisionSensor2Element,
     0,
     0,
     0,
     translationClass | rotationClass,
    ...
    {"SingleDistanceSensor",
     extSensorClass,
     &Parser::singleDistanceSensorElement,
     0,
     0,
     0,
     translationClass | rotationClass,
    ...
    {"ApproxDistanceSensor",
     extSensorClass,
     &Parser::approxDistanceSensorElement,
     0,
     0,
     0,
     translationClass | rotationClass,
...
Element* Parser::bodyElement() {
  ...
  body->configName = getString("configName", false);
  return body;
}
...
Parser::~Parser() {
  for (std::unordered_map<std::string, Macro*>::const_iterator iter = macros.begin(), end = macros.end(); iter != end;
       ++iter)
    delete iter->second;
}
...
void Parser::parseMacroElements() {
  ...
  for (std::list<MacroElement*>::const_iterator iter = replayingMacroElement->children.begin(),
                                                end = replayingMacroElement->children.end();
       iter != end;
       ++iter) {
...
void Parser::parseMacroElement() {
  elementStack.push_back(ElementData(this->elementData, replayingMacroElement->elementInfo));
  auto& elementData = elementStack.back();
  this->elementData = &elementData;
  ...
  // detect unexpected elements
  {
    ...
    if (!((parentInfo->requiredChildren | parentInfo->optionalChildren | parentInfo->repeatableChildren) &
          info->elementClass) ||
  ...
  if (isReferenceOnlyElement && macro->element) { // use an already created "reference-only" instance
  ...
  // handle normal macro references
  {
    ...
    for (;;) {
      ...
      for (std::unordered_map<std::string, std::pair<std::string, int>>::const_iterator iter = macro->attributes.begin(),
                                                                                        end = macro->attributes.end();
           iter != end;
           ++iter)
      ...
      if (!macro || macro->replaying) {
        if (macro)
          ...
          handleError("Unresolvable reference \"" + *ref + "\"");
        for (std::list<Macro*>::const_iterator iter = referencedMacros.begin(), end = referencedMacros.end(); iter != end;
             ++iter)
...
void Parser::handleNode(const std::string& nodeName, Attributes& attributes) // from an input file
{
  ...
  if (elementInfo->elementClass == surfaceClass) {
    ...
    if (iter != attributes.end() && iter->second.first != "" && iter->second.first[0] != '/' &&
        iter->second.first[0] != '\\' &&                                 // not absolute path on Unix
  ...
  if (recordingMacroElement) { // since we are currently parsing a macro, add the element to the macro for later
                               // instantiation
...
void Parser::handleError(const std::string& message) {
  std::string fileName =
    this->fileName.find(parseRootDir) == 0 ? this->fileName.substr(parseRootDir.length()) : this->fileName;
  if (line) {
    char lineColumn[128];
...
error:
  ...
  if (Simulation::simulation->scene) {
    for (std::list<Element*>::const_iterator iter = Simulation::simulation->elements.begin(),
                                             end = Simulation::simulation->elements.end();
         iter != end;
         ++iter)
      delete *iter;
...
void Parser::checkAttributes() {
  ...
  if (unexpectedAttributes) { // apparently there are unused attributs, find them and produce error messages
    for (std::unordered_map<std::string, std::pair<std::string, int>>::const_iterator iter = attributes->begin(),
                                                                                      end = attributes->end();
         iter != end;
         ++iter)
  ...
  if (missingChildren) {
    for (int i = 0; i < 32; ++i)
      if (missingChildren & (1 << i)) {
        ...
        std::string elements;
        int count = 0;
        for (std::unordered_map<std::string, const ElementInfo*>::const_iterator iter = elementInfos.begin(),
                                                                                 end = elementInfos.end();
             iter != end;
             ++iter)
...
bool Parser::getColor(const char* key, bool required, float* color) {
  ...
  if (*strclr == '#') { // html style color (#rrggbb and #rgb)
  ...
  } else if (strncmp(strclr, "rgb(", 4) == 0) { // css style rgb color (rgb(r,g,b) with r,g,b\in[0..255]\cup[0%,..,100%])
    strclr += 4;
    for (int i = 0;; ++i) {
    ...
    color[3] = 1.f;
    return true;
  } else if (strncmp(strclr, "rgba(", 5) ==
             0) { // css3 style rgba color (rgba(r,g,b,a) with r,g,b\in[0..255]\cup[0%,..,100%] and a\in[0..1])
