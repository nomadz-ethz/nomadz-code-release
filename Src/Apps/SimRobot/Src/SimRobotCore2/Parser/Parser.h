...
private:
  ...
  enum ElementFlags {
    ...
    constantFlag = (1 << 1), /**< The element is constant in a way that it can be used multiple times in scene graph to avoid
                                multiple element instantiations */
  };
  ...
  public:
    ...
    ElementData(ElementData* parent, const ElementInfo* info = 0)
        : parent(parent), info(info), parsedChildren(0), parsedAttributes(0), usedPlaceholdersInAttributes(false) {}
  };
  ...
  public:
    ...
    MacroElement(MacroElement* parent, const ElementInfo* elementInfo, Attributes& attributes, int line, int column)
  ...
  std::unordered_map<std::string, const ElementInfo*>
    elementInfos; /** Mapping element name strings to handler member function pointers */
  ...
  std::list<ElementData> elementStack;

  /**
   * A handler called for each node from the input file. Call readSubNodes() within in the handler to parse subordinate
   * nodes.
