...
MainWindow::MainWindow(int argc, char* argv[])
    : timerId(0), viewUpdateRateMenu(0), appPath(getAppPath(argv[0])),
      appString(QString("SimRobot" PATH_SEPARATOR "%1").arg(getAppLocationSum(appPath))), settings("B-Human", appString),
      layoutSettings("B-Human", appString + PATH_SEPARATOR "Layouts"),
      recentFiles(settings.value("RecentFiles").toStringList()), opened(false), compiled(false), running(false),
      performStep(false), layoutRestored(true), guiUpdateRate(100), lastGuiUpdate(0), activeDockWidget(0),
      dockWidgetFileMenu(0), dockWidgetEditMenu(0), dockWidgetUserMenu(0), moduleUserMenu(0), sceneGraphDockWidget(0) {
  application = this;
  ...
  saveLayoutAct = new QAction(tr("&Save Layout"), this);
  saveLayoutAct->setShortcut(QKeySequence(QKeySequence::Save));
  saveLayoutAct->setStatusTip(tr("Save Current Window Layout"));
  connect(saveLayoutAct, SIGNAL(triggered()), this, SLOT(saveLayout()));

  fileCloseAct = new QAction(tr("&Close"), this);
  fileCloseAct->setShortcut(QKeySequence(QKeySequence::Close));
...
bool MainWindow::registerObject(const SimRobot::Module& module,
                                SimRobot::Object& object,
                                const SimRobot::Object* parent,
                                int flags) {
...
void MainWindow::updateMenuAndToolBar() {
  ...
  RegisteredDockWidget* registeredDockWidget =
    opened && activeDockWidget ? qobject_cast<RegisteredDockWidget*>(activeDockWidget) : 0;
...
void MainWindow::updateFileMenu() {
  ...
  fileMenu->addAction(saveLayoutAct);
  if (dockWidgetFileMenu) {
...
void MainWindow::open() {
  QString fileName = QFileDialog::getOpenFileName(
    this, tr("Open File"), settings.value("OpenDirectory", "").toString(), tr("Robot Simulation Files (*.ros *.ros2)"));
...
void MainWindow::openFile(const QString& fileName) {
  ...
  if (openedObjectsVar.isValid()) {
    ...
    for (QString object : openedObjects)
  ...
  for (QString docketWidgetName : dockedWidgets) {
  ...
  for (QMap<QString, RegisteredDockWidget*>::iterator it = openedObjectsByName.begin(), end = openedObjectsByName.end();
       it != end;
       ++it) {
  ...
  for (QString module : manuallyLoadedModules)
    loadModule(module);
  compileModules();
...
void MainWindow::simReset() {
  ...
  filePath.clear(); // modules may read the filePath to distinguish between user request window closing and document closing
                    // / resetting
  layoutRestored = false;
...
void MainWindow::about() {
  QMessageBox::about(this, tr("About SimRobot"), tr("<b>SimRobot</b> 2011 (B-Human Edition)<br><br>\
Authors:\
...
void MainWindow::openObject(const QString& fullName, const SimRobot::Module* module, SimRobot::Object* object, int flags) {
  ...
  QString compactTitle = fullName;
  compactTitle.replace(QString("robot"), QString("R"))
    .replace(QString("parameters"), QString("params"))
    .replace(QString("representation"), QString("rep"));
  dockWidget->setWindowTitle(compactTitle);
