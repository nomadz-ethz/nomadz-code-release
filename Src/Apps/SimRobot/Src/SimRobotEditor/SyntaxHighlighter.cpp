...
SyntaxHighlighter::SyntaxHighlighter(QTextDocument* parent) : QSyntaxHighlighter(parent) {
  ...
  const QString nameStartCharList =
    ":A-Z_a-z\\x00C0-\\x00D6\\x00D8-\\x00F6\\x00F8-\\x02FF\\x0370-\\x037D\\x037F-\\x1FFF\\x200C-\\x200D\\x2070-"
    "\\x218F\\x2C00-\\x2FEF\\x3001-\\xD7FF\\xF900-\\xFDCF\\xFDF0-\\xFFFD";
...
void SyntaxHighlighter::highlightBlock(const QString& text) {
  ...
  for (const HighlightingRule& rule : hlRules) {
...
void SyntaxHighlighter::highlightSubBlock(const QString& text, const int startIndex, const int currState) {
  ...
  case (inNothing): {
    // If we're not in anything, then what could be coming is either a comment, processing instruction, a doctype decl, or a
    // tag (open or close).
