...
SimRobotEditor::Editor*
EditorObject::addEditor(const QString& filePath, const QString& subFileRegExpPattern, bool persistent) {
...
EditorObject::~EditorObject() {
  ...
  int i = 0;
  for (EditorObject* editor : editors) {
    settings.setArrayIndex(i++);
...
FileEditorObject::FileEditorObject(const QString& filePath,
                                   const QString& subFileRegExpPattern,
                                   bool persistent,
                                   EditorObject* parent)
    : EditorObject(QFileInfo(filePath).fileName(), parent), filePath(filePath), subFileRegExpPattern(subFileRegExpPattern),
...
SimRobot::Widget* FileEditorObject::createWidget() {
  ...
  if (!file.open(QFile::ReadOnly | QFile::Text)) {
    EditorModule::application->showWarning(QObject::tr("SimRobotEditor"),
...
EditorWidget::EditorWidget(FileEditorObject* editorObject, const QString& fileContent)
    : editorObject(editorObject), canCopy(false), canUndo(false), canRedo(false), highlighter(0) {
...
bool EditorWidget::canClose() {
  ...
  switch (QMessageBox::warning(this,
                               tr("SimRobotEditor"),
                               tr("Do you want to save changes to %1?").arg(editorObject->name),
...
void EditorWidget::contextMenuEvent(QContextMenuEvent* event) { // disable QTextEdit context menu to use our own
...
void EditorWidget::save() {
  ...
  if (!file.open(QFile::WriteOnly | QFile::Text)) {
    EditorModule::application->showWarning(
      QObject::tr("SimRobotEditor"),
