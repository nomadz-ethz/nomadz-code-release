...
Team::Team()
    : players(), selectedPlayers(), name(""), number(0), port(0), teamColor(""), keeperColor(""), location(""),
      wlanConfig(""), buildConfig(""), formation("") {
  init();
}

Team::Team(const std::string& name, unsigned short number)
    : players(), selectedPlayers(), name(name), number(number), port(0), teamColor(""), keeperColor(""), location(""),
      wlanConfig(""), buildConfig(""), formation("") {
  init();
...
std::vector<Team> Team::getTeams(std::string& filename) {
  if (filename == "")
    filename = "teams.cfg";

  std::vector<Team> teams;
  ConfigMap cm;
  filename = File::getFullName(linuxToPlatformPath(filename));
  int status = cm.read(filename);
  if (status < 1) {
    Session::getInstance().log(CRITICAL, "Team: Cannot read teams from " + filename);
    return teams;
  }
  readTeams(cm, teams);
  return teams;
...
ConfigMap& operator<<(ConfigMap& cv, const Team& team) {
  ...
  cv["port"] << team.port;
  cv["teamColor"] << team.teamColor;
  cv["keeperColor"] << team.keeperColor;
  cv["location"] << team.location;
  cv["buildConfig"] << team.buildConfig;
  cv["wlanConfig"] << team.wlanConfig;
  cv["formation"] << team.formation;
...
std::vector<Robot*> Team::getSelectedPlayers() const {
  ...
  for (std::map<Robot*, bool>::const_iterator i = selectedPlayers.begin(); i != selectedPlayers.end(); ++i)
    if (i->second)
...
const ConfigMap& operator>>(const ConfigMap& cv, Team& team) {
  ...
  if (cv.hasKey("port", false))
    ...
    team.port = team.number + 10000;
  if (cv.hasKey("teamColor", false))
    cv["teamColor"] >> team.teamColor;
  else
    team.teamColor = "red";
  if (cv.hasKey("keeperColor", false))
    cv["keeperColor"] >> team.keeperColor;
  else
    team.keeperColor = "black";
