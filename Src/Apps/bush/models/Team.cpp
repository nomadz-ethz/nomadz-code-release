...
Team::Team()
    : players(), selectedPlayers(), name(""), number(0), port(0), color(""), location(""), wlanConfig(""), buildConfig("") {
  init();
}

Team::Team(const std::string& name, unsigned short number)
    : players(), selectedPlayers(), name(name), number(number), port(0), color(""), location(""), wlanConfig(""),
      buildConfig("") {
  init();
