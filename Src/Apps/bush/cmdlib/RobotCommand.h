...
class RobotCommand : public CommandAdapter {
  bool status;
  const bool checkReachability;

public:
  RobotCommand(bool checkReachability = true) : status(true), checkReachability(checkReachability) {}
...
public:
   ...
   */
  RobotTask(Context& context, Robot* robot) : Task(context, true), robot(robot) {}
