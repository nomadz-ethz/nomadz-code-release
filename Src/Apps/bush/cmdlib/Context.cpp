...
CommandRunnable::CommandRunnable(Context* context, const std::string& cmdLine)
    : ContextRunnable(context), context(context), cmdLine(cmdLine) {}
...
Context::Context(Context* parent, const std::string& cmdLine)
    : AbstractConsole(parent), parent(parent), cmdLine(new std::string(cmdLine)), task(0), thread(0),
      selectedRobots(parent->getSelectedRobots()), selectedTeam(parent->getSelectedTeam()), cmds(), detached(false),
      canceled(false), status(true), requestApplicationShutdown(false) {}

Context::Context(Context* parent, Task* task)
    : AbstractConsole(parent), parent(parent), cmdLine(0), task(task), thread(0),
      selectedRobots(parent->getSelectedRobots()), selectedTeam(parent->getSelectedTeam()), cmds(), detached(true),
      canceled(false), status(true), requestApplicationShutdown(false) {}

Context::Context(Context* parent, const std::string& cmdLine, bool detach)
    : AbstractConsole(parent), parent(parent), cmdLine(new std::string(cmdLine)), task(0), thread(0),
      selectedRobots(parent->getSelectedRobots()), selectedTeam(parent->getSelectedTeam()), cmds(), detached(detach),
      canceled(false), status(true), requestApplicationShutdown(false) {}

Context::Context(const std::vector<Robot*>& selectedRobots, Team* selectedTeam)
    : AbstractConsole(0), parent(0), cmdLine(0), task(0), thread(0), selectedRobots(selectedRobots),
      selectedTeam(selectedTeam), cmds(), detached(false), canceled(false), status(true), requestApplicationShutdown(false) {
}
