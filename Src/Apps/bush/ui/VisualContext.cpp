...
VisualContext::VisualContext(QWidget* parent)
    : QFrame(parent), entries(), widgets(), formLayout(new QFormLayout()), nl(true) {
...
static inline VisualContext::Entry::Type targetToType(ConsolePrintTarget target) {
  return target == CPT_PRINT || target == CPT_PRINT_LINE ? VisualContext::Entry::TEXT_OUTPUT
                                                         : VisualContext::Entry::TEXT_ERROR;
}
...
void VisualContext::commandExecuted(Context* context, const QString& cmdLine) {
  ...
  connect(context,
          SIGNAL(sExecute(Context*, const QString&)),
          sub,
          SLOT(commandExecuted(Context*, const QString&)),
          Qt::BlockingQueuedConnection);
  connect(
    context, SIGNAL(sPrint(ConsolePrintTarget, const QString&)), sub, SLOT(doPrint(ConsolePrintTarget, const QString&)));
