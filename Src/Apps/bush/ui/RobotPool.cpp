...
RobotPoolDelegate::RobotPoolDelegate(RobotPool* pool) : robotPool(pool) {}
...
RobotPool::RobotPool(TeamSelector* teamSelector)
    : QListWidget(), teamSelector(teamSelector), robotViews(), toBeDeletedLater(0) {
...
void RobotPool::update() {
  ...
  for (std::map<std::string, Robot*>::const_iterator i = robots.begin(); i != robots.end(); ++i) {
