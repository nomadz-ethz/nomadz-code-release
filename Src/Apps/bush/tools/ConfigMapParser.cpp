...
enum State_T { NONE = 0, EOL_C, BLOCK_C, STRING, ESCAPE };

#define MK_TOKEN(type, value) ConfigMapLexer::Token(type, tknLine, tknColumn, value)
...
ConfigMapLexer::ConfigMapLexer(const std::string& filename)
    : input(new std::ifstream(filename.c_str(), std::ifstream::in)), filename(new std::string(filename)), line(1), column(0),
      tkn(0) {}
...
void ConfigMapParser::expect(T::Type type) {
  if (token.type == type)
    nextToken();
  else
...
bool prefixValue(const T& token) {
  return token.type == T::CMT_VALUE || token.type == T::CMT_ABL || token.type == T::CMT_CBL;
}

int ConfigMapParser::file(ConfigMap* configMap) {
...
ConfigValue* ConfigMapParser::value() {
...
PlainConfigValue* ConfigMapParser::plain() {
...
ListConfigValue* ConfigMapParser::list() {
  expect(T::CMT_ABL);
...
std::string* ConfigMapParser::comments() {
