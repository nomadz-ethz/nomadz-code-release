...
public:
  struct Token {
    ...
    Token(Type t, int l, int c, std::string v) : type(t), line(l), column(c), value(v) {}
  };
...
public:
  ConfigMapLexer(std::istream* in) : input(in), filename(0), line(1), column(0), tkn(0) {}
...
public:
  ...
  public:
    ParseException(size_t line, size_t column, const std::string& msg) throw() : line(line), column(column), msg(msg) {}
...
private:
  ...
  void nextToken();
  void expect(ConfigMapLexer::Token::Type type);
  int file(ConfigMap* configMap);
  ListConfigValue* list();
  ConfigMap* map();
  PlainConfigValue* plain();
  ConfigValue* value();
  std::string* comments();

public:
  ConfigMapParser() : lexer(0), configMap(0), token(TKN_EOF), verbose(false), error(0) {}
  ConfigMapParser(ConfigMap* map) : lexer(0), configMap(map), token(TKN_EOF), verbose(false), error(0) {}
  ConfigMapParser(ConfigMap* map, std::istream* in)
      : lexer(new ConfigMapLexer(in)), configMap(map), token(TKN_EOF), verbose(false), error(0) {}
  ConfigMapParser(ConfigMap* map, const std::string& file)
      : lexer(new ConfigMapLexer(file)), configMap(map), token(TKN_EOF), verbose(false), error(0) {}
  ~ConfigMapParser();
   ...
   */
  inline void setVerbose(bool b = true) { this->verbose = b; }
