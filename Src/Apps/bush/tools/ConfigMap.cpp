...
PlainConfigValue::PlainConfigValue() : ConfigValueBase(), strn("") {}

PlainConfigValue::PlainConfigValue(const std::string& v) : strn(v) {}

PlainConfigValue::PlainConfigValue(const PlainConfigValue& other) : ConfigValueBase(other), strn(other.strn) {}
...
ListConfigValue::ListConfigValue() : list() {}

ListConfigValue::ListConfigValue(const ListConfigValue& other) : ConfigValueBase(other), list(other.list) {
...
ListConfigValue::ListConfigValue(std::vector<ConfigValue*> values) : list(values) {}
...
void ListConfigValue::clear() {
  for (std::vector<ConfigValue*>::iterator i = list.begin(); i != list.end(); ++i)
    delete *i;
  list.clear();
}
...
bool hasComplexValue(const std::vector<ConfigValue*>& list) {
  for (size_t i = 0; i < list.size(); ++i) {
    ...
    if (cv->getType() == ConfigValue::MAP || (cv->getType() == ConfigValue::LIST && cv->length() > 0))
      return true;
  }
  return false;
}
...
std::ostream& ListConfigValue::write(std::ostream& os, bool comment, std::string indentation) const {
  ...
  bool sep = false;
  for (std::vector<ConfigValue*>::const_iterator i = list.begin(); i != list.end(); ++i) {
    if (sep) {
      os << ",";
      if (wrap)
        os << std::endl << currIndent;
...
ConfigValueProxy ListConfigValue::operator[](size_t index) {
  if (index >= list.size()) {
    ...
    if (isReadOnly())
      throw invalid_key("Index " + std::string(buf) +
                        " is not set and "
...
ConfigValue* ListConfigValue::append(size_t index, const ConfigValue& cv) {
  if (list.size() != index)
    throw invalid_key("Cannot append!");
 ...
 * ConfigMap
 */
ConfigMap::ConfigMap() : dict(), flags(0), error("") {}

ConfigMap::ConfigMap(const ConfigMap& other)
    : ConfigValueBase(other), dict(), keyOrder(other.keyOrder), flags(other.flags), error(other.error) {
  for (std::map<std::string, ConfigValue*>::const_iterator i = other.dict.begin(); i != other.dict.end(); ++i) {
...
ConfigMap::ConfigMap(unsigned int flags) : dict(), flags(flags), error("") {}

ConfigMap::~ConfigMap() {
  for (std::map<std::string, ConfigValue*>::const_iterator i = dict.begin(); i != dict.end(); ++i)
    delete i->second;
  dict.clear();
}
...
ConfigValue* ConfigMap::set(const std::string& key, const ConfigValue& cv) {
  if (isReadOnly())
    throw invalid_key("Cannot set `" + key + "` since the map is read-only.");
  if (!hasKey(key))
...
bool hasComplexValue(const std::map<std::string, ConfigValue*>& map) {
  for (std::map<std::string, ConfigValue*>::const_iterator i = map.begin(); i != map.end(); ++i)
...
std::ostream& ConfigMap::write(std::ostream& os, bool withComment, std::string indentation) const {
  ...
  if (p) {
    ...
    currIndent = "";
  bool sep = false;
  for (std::vector<std::string>::const_iterator key = keyOrder.begin(); key != keyOrder.end(); ++key) {
...
std::vector<std::string> ConfigMap::getKeys(bool deep) const {
  ...
  for (std::map<std::string, ConfigValue*>::const_iterator i = dict.begin(); i != dict.end(); ++i) {
    ...
    if (deep && i->second->getType() == ConfigValue::MAP) {
      ...
      for (std::vector<std::string>::const_iterator j = cKeys.begin(); j != cKeys.end(); ++j) {
        keys.push_back(*j);
      }
    }
  }
...
ConfigValueProxy::ConfigValueProxy(ListConfigValue& lcv, int index) : list(&lcv), index(index), key(0), configValue(0) {}

ConfigValueProxy::ConfigValueProxy(ListConfigValue& lcv, int index, ConfigValue* configValue)
    : list(&lcv), index(index), key(0), configValue(configValue) {}

ConfigValueProxy::ConfigValueProxy(ConfigMap& cm, const std::string& key)
    : map(&cm), index(-1), key(new std::string(key)), configValue(0) {}

ConfigValueProxy::ConfigValueProxy(ConfigMap& cm, const std::string& key, ConfigValue* configValue)
    : map(&cm), index(-1), key(new std::string(key)), configValue(configValue) {}
...
void ConfigValueProxy::setParent(ConfigValue* cv) {
  if (!configValue)
    throw invalid_key("Cannot access or create " + std::string(1, index));
...
ConfigValue& ConfigMap::operator+=(const ConfigValue& rhs) {
  ...
  if (rhsMap.length() == 0) {
    for (std::map<std::string, ConfigValue*>::iterator i = dict.begin(); i != dict.end(); ++i) {
      delete i->second;
    }
    dict.clear();
    keyOrder.clear();
  } else {
    for (std::map<std::string, ConfigValue*>::const_iterator i = rhsMap.dict.begin(); i != rhsMap.dict.end(); ++i) {
      ConfigValue* left = 0;
      if (hasKey(i->first)) {
