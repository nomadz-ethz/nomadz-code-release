...
std::vector<std::string> Filesystem::getWlanConfigs(const std::string prefix) {
  return getEntries(std::string(File::getBHDir()) + "/Install/Network/Profiles/" + prefix, true, false);
}

std::vector<std::string> Filesystem::getLocations(const std::string prefix) {
  return getEntries(std::string(File::getBHDir()) + "/Config/Locations/" + prefix, false, true);
}

std::vector<std::string> Filesystem::getProjects(const std::string prefix) {
#ifdef WIN32
  return getEntries(
    std::string(File::getBHDir()) + "/Make/" + makeDirectory() + "/" + prefix, true, false, ".vcxproj", false);
#else
  return getEntries(std::string(File::getBHDir()) + "/Make/Linux/" + prefix, true, false, ".make", false);
#endif
}

std::vector<std::string> Filesystem::getEntries(
  const std::string& directory, bool files, bool directories, const std::string& suffix, bool keepSuffixes) {
...
std::string Filesystem::getNaoKey() {
  ...
  QFile::setPermissions(QString::fromUtf8(keyFile.c_str()),
                        QFile::ReadOwner); // set correct permissions, otherwise ssh will complain
  return keyFile;
}
