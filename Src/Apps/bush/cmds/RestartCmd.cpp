...
#else
    std::string command = remoteCommand("systemctl --user stop nomadz && sleep 5 && systemctl --user start nomadz", ip);
#endif
...
#else
    std::string command = remoteCommand("systemctl --user stop naobridge && systemctl --user start nomadz", ip);
#endif
    ProcessRunner r(context(), command);
    r.run();
    reportStatus(r);
...
#else
    std::string command =
      remoteCommand("systemctl stop --user nomadz && systemctl stop --user naobridge && sleep 10 && reboot", ip);
#endif
    ProcessRunner r(context(), command);
    r.run();
    reportStatus(r);
