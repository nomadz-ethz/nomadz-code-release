...
QStringList CompileCmd::getParams(const QString& config, const QString& project, Context& context) {
  ...
  if (v6) {
    QString v6_dir;
    v6_dir = fromString(std::string(File::getBHDir())) + "/Build/V6";

    QString mke = "mkdir -p " + v6_dir + " && cd " + v6_dir + " && cmake ";
    mke += base_mk;

    mke += " -DNOMADZ_TARGET=v6";
...
QStringList CompileCmd::getParams(const QString& config, const QString& project, Context& context) {
  ...
  total_mk += " && PATH=\"" + yocto_sdk + "/sysroots/x86_64-naosdk-linux/usr/bin:$PATH\"" + " NOMADZ_YOCTO_SDK=" + yocto_sdk;
  total_mk += " colcon build --merge-install --event-handlers console_direct+";
  total_mk += " --cmake-args -DCMAKE_TOOLCHAIN_FILE=" + nomadz_ng_package_dir + "/cmake/v6-64.toolchain.cmake " + cmake_args;
