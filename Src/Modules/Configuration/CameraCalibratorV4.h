...
DEFINES_PARAMETER(bool,
                  calibrateBothCameras,
                  true) /**< Whether both cameras or only the lower camera should be calibrated */
DEFINES_PARAMETER(bool,
                  errorInImage,
                  true) /**< Whether the error is computed from the distance in the image or on the ground */
DEFINES_PARAMETER(float,
                  terminationCriterion,
                  0.001f) /**< The difference of two succesive parameter sets that are taken as a convergation */
DEFINES_PARAMETER(float,
                  aboveHorizonError,
                  1000000.f) /**< The error for a sample the error of which cannot be computed regularly */
DEFINES_PARAMETER(
  int,
  numOfFramesToWait,
  15) /**< The number of frames to wait between two iterations (necessary to keep the debug connection alive) */
DEFINES_PARAMETER(int,
                  minSuccessfulConvergences,
                  5) /**< The number of consecutive iterations that fulfil the termination criterion to converge */
DEFINES_PARAMETER(int, minSegmentLengthUpper, 30) /**< In line mode, the minimum distance (px) between consecutive points */
DEFINES_PARAMETER(int, minSegmentLengthLower, 15) /**< Same for the lower camera (resolution is half of upper) */
END_MODULE
...
private:
   ...
   */
  ENUM(CalibrationState, Idle, Accumulate, Optimize);
  ...
  ENUM(DrawMode,
       point, /** clicking draws points */
       line   /** clicking starts a line, clicking again completes the line */
  );
  DrawMode drawMode;
  bool startedDrawingLine; /** If DrawMode == line, true if last sample is starting point of line */
   ...
   * @return The distance.
   */
  float computeError(const Sample& sample,
                     const CameraCalibration& cameraCalibration,
                     const RobotPose& robotPose,
                     bool inImage = true) const;
   ...
   * @param cameraCalibration The camera calibration the values of which are set to the corresponding values in the parameter
   * vector.
   * @param robotPose The robot pose the values of which are set to the corresponding values in the parameter vector.
   */
  void translateParameters(const std::vector<float>& parameters,
   ...
   */
  void translateParameters(const CameraCalibration& cameraCalibration,
                           const RobotPose& robotPose,
  ...
  /**
   * If valid, add the given point to list of samples; return whether it was valid.
   * @param point The point to add, in image coordinates.
   * @return Whether the point was valid & added; invalid when point is above the horizon.
   */
  bool addPoint(const Vector2<int>& point);
   ...
   * @return Whether a valid result was computed, which is not the case if the field line lies completely behind the camera
   * plane.
   */
  bool projectLineOnFieldIntoImage(const Geometry::Line& lineOnField,
  ...
  private:
    ...
    float (C::*pFunction)(const M& measurement,
                          const std::vector<float>& parameters) const; /**< A pointer to the error function. */
    const float delta; /**< The delta used to approximate the partial derivatives of the Jacobian. */

  public:
    GaussNewtonOptimizer(const std::vector<float>& parameters,
                         const std::vector<M>& measurements,
                         const C& object,
                         float (C::*pFunction)(const M& measurement, const std::vector<float>& parameters) const)
        : numOfMeasurements(measurements.size()), numOfParameters(parameters.size()), currentParameters(numOfParameters, 1),
    ...
    float iterate() {
      ...
      try {
        YaMatrix<float> result =
          (jacobiMatrix.transpose() * jacobiMatrix).inverse() * jacobiMatrix.transpose() * currentValues;
     ...
     */
    std::vector<float> getParameters() const { return currentParameters.transpose()[0]; }
  };

  GaussNewtonOptimizer<Sample, CameraCalibratorV4>*
    optimizer; /**< A pointer to the currently used optimizer, or NULL if there is none. */

  int successfulConvergences; /**< The number of consecutive iterations that fulfil the termination criterion. */
  int framesToWait;           /**< The remaining number of frames to wait for the next iteration. */

  const CameraCalibration*
    cameraCalibration; /**< A pointer to the current camera calibration, refreshed in every call of the update method. */
