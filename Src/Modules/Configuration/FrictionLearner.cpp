 ...
 *
 * @note The original author is Martin Böschen
 */
...
void FrictionLearner::updateAndOutput(float vec, float acc) {
  if (std::abs(vec) >= 15) {
    ...
    OUTPUT(idText,
           text,
           numSamples << ", Velocity: " << vec
                      << ", Estimate: " << estimate_nom / estimate_denom * (1 / theCognitionFrameInfo.cycleTime));
  }
}
...
void FrictionLearner::update(FieldDimensions& fieldDimensions) {
  ...
  DEBUG_RESPONSE_ONCE("module:FrictionLearner:writeDataToFile", writeToFile(););

  DEBUG_RESPONSE_ONCE("module:FrictionLearner:startCollectData", collectData = true;);

  DEBUG_RESPONSE_ONCE("module:FrictionLearner:stopCollectData", collectData = false;);

  DEBUG_RESPONSE_ONCE("module:FrictionLearner:resetEstimator", upperCameraPercepts.clear(); lowerCameraPercepts.clear();
                      collectData = false;
                      estimate_nom = 0.0f;
                      estimate_denom = 0.0f;
                      numSamples = 0;);
