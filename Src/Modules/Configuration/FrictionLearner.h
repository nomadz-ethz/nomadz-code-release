...
private:
  ...
  std::vector<ballPercept>
    allBallPercepts; /* A list of ball percepts which can be saved into an csv file for offline analysis. */
  ...
  void handleBallPercept(CameraInfo::Camera c,
                         std::vector<Vector2<>>& ballPecepts); /* Checks for ball percepts for the corresponding camera und
                                                                  updates the ball percepts list and the estimator. */
  void updateAndOutput(
    float vec, float acc); /* Updates the estimator with a new tuple of acceleartion and velocity and outputs the result. */
};
