...
option(PenaltyKeeper) {
  ...
  state(detectShot) {
    transition {
      ...
      float distanceKB = dist(theBallModel.estimate.position.x, 0.f, theBallModel.estimate.position.y, 0.f);

      // ball has been moved from the penalty mark and moves toward us
      if (distanceKB <= distanceToPenaltyMark * 0.9f && theBallModel.estimate.velocity.x < 0.f &&
          libCodeRelease.timeSinceBallWasSeen() < 1000.f && state_time > 2000.f) {
        goto saveShot;
      }
    }
    action {
