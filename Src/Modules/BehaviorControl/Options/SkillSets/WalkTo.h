...
option(WalkTo, Pose2D target, bool stopAtTarget = true) {
  ...
  const float alignThreshold = 350.f;
  ...
  initial_state(rotate) {
    transition {
      if (relTarget.translation.abs() < alignThreshold) {
        goto align;
      } else if (std::abs(angleToGlobalTarget(target.translation)) < fromDegrees(25.f)) {
        goto walk;
      }
    }
    action {
  ...
  state(walk) {
    transition {
      if (relTarget.translation.abs() < alignThreshold) {
        goto align;
      } else if (std::abs(angleToGlobalTarget(target.translation)) > fromDegrees(35.f)) {
        goto rotate;
      }
    }
    action {
