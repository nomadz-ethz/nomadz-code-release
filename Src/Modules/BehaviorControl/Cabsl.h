...
public:
  ...
  ENUM(StateType, normalState, targetState, abortedState);
  ...
  StateType
    subOptionStateType;    /**< The type of the state of the last suboption executed (for action_done and action_aborted). */
...
public:
  ...
  void sendDebugMessage() const {
    if (!context.messageSent) {
      instance->theActivationGraph.graph.push_back(
        ActivationGraph::ActivationGraphNode(optionName,
                                             instance->_depth,
                                             context.stateName,
...
public:
   ...
   */
  OptionDescriptor(const char* name,
                   const char* parameters,
                   void (Behavior::*option)(const OptionExecution&),
                   size_t offsetOfContext)
...
private:
  ...
  static std::unordered_map<std::string, OptionDescriptor*>
    optionsByName; /**< All parameterless options, index by their names. */
...
public:
  ...
  OptionInfos() {
    ...
    Behavior b{internal::ref_init_tag()};
...
#define option(name, ...)                                                                                                   \
  static OptionDescriptor _get##name##Descriptor() {                                                                        \
    return OptionDescriptor(#name,                                                                                          \
                            "" #__VA_ARGS__,                                                                                \
                            (void (Behavior::*)(const OptionExecution&)) & Behavior::name,                                  \
                            ((size_t) & ((Behavior*)16)->_##name##Context) - 16);                                           \
  }                                                                                                                         \
  OptionInfo<&Behavior::_get##name##Descriptor> _##name##Context;                                                           \
  void name(__VA_ARGS__ _CABSL_GETCOMMA(__VA_ARGS__) const OptionExecution& _o =                                            \
              OptionExecution(#name, _theInstance->_##name##Context, _theInstance))
 ...
 */
#define transition if (!_o.context.transitionExecuted && (_o.context.transitionExecuted = true))
