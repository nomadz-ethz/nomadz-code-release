...
void InertiaSensorFilter::update(OrientationData& orientationData) {
  ...
  bool useLeft = true;
  if (theMotionInfo.motionRequest.motion == MotionRequest::walk && theWalkingEngineOutput.speed.translation.x != 0) {
    useLeft = theWalkingEngineOutput.speed.translation.x > 0 ? (leftOffset.translation.x > rightOffset.translation.x)
  ...
  if ((theMotionInfo.motionRequest.motion == MotionRequest::walk || theMotionInfo.isStanding()) &&
    ...
    // insert acceleration sensor values
    if (theInertiaSensorData.acc.x != InertiaSensorData::off && theInertiaSensorData.acc.y != InertiaSensorData::off &&
        theInertiaSensorData.acc.z != InertiaSensorData::off) {
...
void InertiaSensorFilter::meanOfSigmaPoints() {
  ...
  for (int i = 0; i < 1; ++i) {
    Vector2f chunk((sigmaPoints[0] - x) + ((sigmaPoints[1] - x) + (sigmaPoints[2] - x)) +
...
void InertiaSensorFilter::covOfSigmaPoints() {
  cov = tensor(sigmaPoints[0] - x) + (tensor(sigmaPoints[1] - x) + tensor(sigmaPoints[2] - x)) +
...
void InertiaSensorFilter::meanOfSigmaReadings() {
  ...
  // for(int i = 0; i < 5; ++i) // ~= 0 .. inf
  for (int i = 0; i < 1; ++i) {
    Vector3f chunk((sigmaReadings[0] - readingMean) + ((sigmaReadings[1] - readingMean) + (sigmaReadings[2] - readingMean)) +
