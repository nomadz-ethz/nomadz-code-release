...
FsrZmpProvider::FsrZmpProvider() {
  ...
  fsrBasePositions[SensorData::fsrLBR - SensorData::fsrLFL] =
    Vector3<>(-29.65f, -19.1f, -theRobotDimensions.heightLeg5Joint);
  ...
  fsrBasePositions[SensorData::fsrRBR - SensorData::fsrLFL] =
    Vector3<>(-29.65f, -29.9f, -theRobotDimensions.heightLeg5Joint);
}
...
void FsrZmpProvider::update(FsrZmp& fsrZmp) {
  ...
  COMPLEX_DRAWING(
    "module:FsrZmpProvider:copLeft",
    for (int i = 0; i < 4; ++i) {
      CROSS("module:FsrZmpProvider:copLeft",
            fsrBasePositions[i].x * 8,
            fsrBasePositions[i].y * 8,
            20,
            4,
            Drawings::ps_solid,
            ColorClasses::blue);
  ...
  COMPLEX_DRAWING(
    "module:FsrZmpProvider:copRight",
    for (int i = 4; i < 8; ++i) {
      CROSS("module:FsrZmpProvider:copRight",
            fsrBasePositions[i].x * 8,
            fsrBasePositions[i].y * 8,
            20,
            4,
  ...
  COMPLEX_DRAWING(
    "module:FsrZmpProvider:cop", for (int i = 0; i < 8; ++i) {
      CROSS("module:FsrZmpProvider:cop",
            fsrPositions[i].x * 4,
            fsrPositions[i].y * 4,
            20,
            4,
