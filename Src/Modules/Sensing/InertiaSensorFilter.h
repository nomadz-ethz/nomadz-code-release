...
DEFINES_PARAMETER(Vector2f,
                  calculatedAccLimit,
                  Vector2f(fromDegrees(20.f), fromDegrees(30.f))) /**< Use a calculated angle up to this angle (in rad). (We
                                                                     use the acceleration sensors otherwise.) */
END_MODULE
...
private:
  ...
  inline Matrix2x2f tensor(const Vector2f& a) const { return Matrix2x2f(a * a.x, a * a.y); }

  inline Matrix3x2f tensor(const Vector3f& a, const Vector2f& b) { return Matrix3x2f(a * b.x, a * b.y); }
