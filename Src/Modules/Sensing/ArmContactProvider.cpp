...
void ArmContactProvider::update(ArmContact& armContact) {
  ...
  if (enableAvoidArmContact && (theGameInfo.state == STATE_READY || theGameInfo.state == STATE_PLAYING) &&
    ...
    if (!theArmMovement.armsInContactAvoidance) {
      ...
      Angle angleDiff =
        lastRequest.jointAngles.angles[JointData::LShoulderPitch] - theJointData.angles[JointData::LShoulderPitch];
      ...
      angleDiff = lastRequest.jointAngles.angles[JointData::RShoulderPitch] - theJointData.angles[JointData::RShoulderPitch];
      if (std::abs(angleDiff) > minAngleDiff) {
