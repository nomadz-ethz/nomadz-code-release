...
void FallDownStateDetector::update(FallDownState& fallDownState) {
  ...
  if (isCalibrated() && !isPlayDead()) {
    ...
    } else if (fallDownState.state == FallDownState::staggering && isFalling() && theGroundContactState.contact) {
...
bool FallDownStateDetector::isPlayDead() {
