...
ArmContactModelProvider::ArmContactModelProvider() : soundDelay(1000), lastSoundTime(0), lastGameState(STATE_INITIAL) {}

void ArmContactModelProvider::checkArm(bool left, float factor) {
  Vector2f retVal;
  /* Calculate arm diffs */
  struct ArmAngles angles = angleBuffer[frameDelay];
  retVal.x = left ? theFilteredJointData.angles[JointData::LShoulderPitch] - angles.leftX
                  : theFilteredJointData.angles[JointData::RShoulderPitch] - angles.rightX;
  retVal.y = left ? theFilteredJointData.angles[JointData::LShoulderRoll] - angles.leftY
                  : theFilteredJointData.angles[JointData::RShoulderRoll] - angles.rightY;
  retVal *= factor;

  if (left) {
    ARROW("module:ArmContactModelProvider:armContact",
          0,
          0,
          -(toDegrees(retVal.y) * SCALE),
          toDegrees(retVal.x) * SCALE,
          20,
          Drawings::ps_solid,
          ColorClasses::blue);
...
void ArmContactModelProvider::update(ArmContactModel& model) {
  ...
  if (theGameInfo.state == STATE_INITIAL || theGameInfo.state == STATE_FINISHED ||
      ...
      (theMotionInfo.motionRequest.motion == MotionRequest::specialAction &&
       theMotionInfo.motionRequest.specialActionRequest.specialAction != SpecialActionRequest::stand &&
       theMotionInfo.motionRequest.specialActionRequest.specialAction != SpecialActionRequest::standHigh) ||
      theMotionInfo.motionRequest.motion == MotionRequest::kick || !theGroundContactState.contact ||
  ...
  CIRCLE("module:ArmContactModelProvider:armContact",
         0,
         0,
         200,
         30,
  ...
  angles.leftX = theJointRequest.jointAngles.angles[JointData::LShoulderPitch];
  angles.leftY = theJointRequest.jointAngles.angles[JointData::LShoulderRoll];
  angles.rightX = theJointRequest.jointAngles.angles[JointData::RShoulderPitch];
  angles.rightY = theJointRequest.jointAngles.angles[JointData::RShoulderRoll];
  ...
  if ((theMotionInfo.motionRequest.motion == MotionRequest::walk ||
    ...
    const float leftFactor =
      calculateCorrectionFactor(theRobotModel.limbs[MassCalibration::foreArmLeft], odometryOffset, lastLeftHandPos);
    const float rightFactor =
      calculateCorrectionFactor(theRobotModel.limbs[MassCalibration::foreArmRight], odometryOffset, lastRightHandPos);
    ...
    // update the model
    model.contactLeft = (leftX || leftY) && leftErrorBuffer.isFilled();

    model.contactRight = (rightX || rightY) && rightErrorBuffer.isFilled();
    ...
    model.lastPushDirectionLeft =
      model.pushDirectionLeft != ArmContactModel::NONE ? model.pushDirectionLeft : model.lastPushDirectionLeft;
    model.lastPushDirectionRight =
      model.pushDirectionRight != ArmContactModel::NONE ? model.pushDirectionRight : model.lastPushDirectionRight;
    ...
    ARROW("module:ArmContactModelProvider:armContact",
          0,
          0,
          -(toDegrees(left.y) * SCALE),
          toDegrees(left.x) * SCALE,
          20,
          Drawings::ps_solid,
          ColorClasses::green);
    ARROW("module:ArmContactModelProvider:armContact",
          0,
          0,
          toDegrees(right.y) * SCALE,
          toDegrees(right.x) * SCALE,
          20,
          Drawings::ps_solid,
          ColorClasses::red);
    ...
    COMPLEX_DRAWING("module:ArmContactModelProvider:armContact", {
      ...
      DRAWTEXT(
        "module:ArmContactModelProvider:armContact", -2300, 1100, 200, ColorClasses::black, "ErrorX: " << toDegrees(left.x));
      DRAWTEXT(
        "module:ArmContactModelProvider:armContact", -2300, 900, 200, ColorClasses::black, "ErrorY: " << toDegrees(left.y));
      DRAWTEXT("module:ArmContactModelProvider:armContact",
               -2300,
               500,
               200,
               ColorClasses::black,
               ArmContactModel::getName(model.pushDirectionLeft));
      DRAWTEXT("module:ArmContactModelProvider:armContact",
               -2300,
               300,
               200,
               ColorClasses::black,
               "Time: " << model.timeOfLastContactLeft);

      DRAWTEXT("module:ArmContactModelProvider:armContact", 1300, 1300, 200, ColorClasses::black, "RIGHT");
      DRAWTEXT(
        "module:ArmContactModelProvider:armContact", 1300, 1100, 200, ColorClasses::black, "ErrorX: " << toDegrees(right.x));
      DRAWTEXT(
        "module:ArmContactModelProvider:armContact", 1300, 900, 200, ColorClasses::black, "ErrorY: " << toDegrees(right.y));
      DRAWTEXT("module:ArmContactModelProvider:armContact",
               1300,
               500,
               200,
               ColorClasses::black,
               ArmContactModel::getName(model.pushDirectionRight));
      DRAWTEXT("module:ArmContactModelProvider:armContact",
               1300,
               300,
               200,
               ColorClasses::black,
               "Time: " << model.timeOfLastContactRight);
    ...
    if (debugMode && theFrameInfo.getTimeSince(lastSoundTime) > soundDelay && (model.contactLeft || model.contactRight)) {
...
ArmContactModelProvider::getDirection(bool left, bool contactX, bool contactY, Vector2f error) {
...
float ArmContactModelProvider::calculateCorrectionFactor(const Pose3D foreArm,
                                                         const Pose2D odometryOffset,
                                                         Vector2<>& lastArmPos) {
