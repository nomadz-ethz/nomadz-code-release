...
FootSupportProvider::FootSupportProvider() {
  weights[Leg::left][FsrSensorPosition::fl] = weights[Leg::left][FsrSensorPosition::bl] = outerWeight;
  weights[Leg::right][FsrSensorPosition::fr] = weights[Leg::right][FsrSensorPosition::br] = -outerWeight;
  weights[Leg::left][FsrSensorPosition::fr] = weights[Leg::left][FsrSensorPosition::br] = innerWeight;
  weights[Leg::right][FsrSensorPosition::fl] = weights[Leg::right][FsrSensorPosition::bl] = -innerWeight;

  for (int leg = 0; leg < Leg::numOfSides; leg++) {
...
void FootSupportProvider::update(FootSupport& theFootSupport) {
  ...
  PLOT("module:FootSupportProvider:switched", theFootSupport.switched);
  PLOT("module:FootSupportProvider:predictedSwitched", theFootSupport.predictedSwitched);
}
