/**
 * @file FootContactModelProvider.cpp
 *
...
FootContactModelProvider::FootContactModelProvider()
    : contactDurationLeft(0), contactDurationRight(0), leftFootLeftDuration(0), leftFootRightDuration(0),
      rightFootLeftDuration(0), rightFootRightDuration(0) {}

void FootContactModelProvider::update(FootContactModel& model) {
  // Check, if any bumper is pressed
  bool leftFootLeft =
    !theDamageConfiguration.leftFootBumperDefect && checkContact(KeyStates::leftFootLeft, leftFootLeftDuration);
  bool leftFootRight =
    !theDamageConfiguration.leftFootBumperDefect && checkContact(KeyStates::leftFootRight, leftFootRightDuration);
  bool rightFootLeft =
    !theDamageConfiguration.rightFootBumperDefect && checkContact(KeyStates::rightFootLeft, rightFootLeftDuration);
  bool rightFootRight =
    !theDamageConfiguration.rightFootBumperDefect && checkContact(KeyStates::rightFootRight, rightFootRightDuration);
  ...
  if ((theMotionInfo.motionRequest.motion == MotionRequest::walk) &&
      (theGameInfo.state == STATE_READY || theGameInfo.state == STATE_SET ||
       theGameInfo.state == STATE_PLAYING) && // The bumper is used for configuration in initial
