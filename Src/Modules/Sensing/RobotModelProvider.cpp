...
void RobotModelProvider::update(RobotModel& robotModel) {
  ...
  PLOT("module:RobotModel:soleLeftX", robotModel.soleLeft.translation.x);
  PLOT("module:RobotModel:soleLeftY", robotModel.soleLeft.translation.y);

  PLOT("module:RobotModel:soleRightX", robotModel.soleRight.translation.x);
  PLOT("module:RobotModel:soleRightY", robotModel.soleRight.translation.y);

  PLOT("module:RobotModel:centerOfMassX", robotModel.centerOfMass.x);
  PLOT("module:RobotModel:centerOfMassY", robotModel.centerOfMass.y);
  PLOT("module:RobotModel:centerOfMassZ", robotModel.centerOfMass.z);
  ...
  DECLARE_DEBUG_DRAWING3D("module:RobotModelProvider:joints", "origin", {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int i = 0; i < (MassCalibration::footLeft + 1) - MassCalibration::pelvisLeft; ++i) {
        ...
        SPHERE3D("module:RobotModelProvider:joints",
                 next.translation.x,
                 next.translation.y,
                 next.translation.z,
                 3,
        ...
        Vector3<> axis = i == 0
                           ? Vector3<>(0.f, firstJoint == MassCalibration::pelvisLeft ? -50.f : 50.f, 50.f).normalize(50.f)
                           : i == 1 || i == 5 ? Vector3<>(50.f, 0.f, 0.f) : Vector3<>(0.f, 50.f, 0.f);
        ColorRGBA color = i == 0 ? ColorRGBA(0, 0, 255) : i == 1 || i == 5 ? ColorRGBA(255, 0, 0) : ColorRGBA(0, 255, 0);
        Vector3<> p = next * axis;
        LINE3D("module:RobotModelProvider:joints",
               next.translation.x,
               next.translation.y,
               next.translation.z,
               p.x,
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int i = 0; i < (MassCalibration::foreArmLeft + 1) - MassCalibration::shoulderLeft; ++i) {
        ...
        Vector3<> axis =
          i == 1 || i == 3 ? Vector3<>(0.f, 0.f, 50.f) : i == 2 ? Vector3<>(50.f, 0.f, 0.f) : Vector3<>(0.f, 50.f, 0.f);
