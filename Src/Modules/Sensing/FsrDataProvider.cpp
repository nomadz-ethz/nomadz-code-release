...
void FsrDataProvider::update(FsrData& fsrData) {
  ...
  if (fsrL.isFilled() && fsrL.getAverageFloat() + fsrWeightOffset >= theRobotModel.totalMass) {
    fsrData.leftFootContact = true;
  } else {
    fsrData.leftFootContact = false;
  }
  ...
  if (fsrR.isFilled() && fsrR.getAverageFloat() + fsrWeightOffset >= theRobotModel.totalMass) {
    fsrData.rightFootContact = true;
  } else {
    fsrData.rightFootContact = false;
  }

  if (fsrData.leftFootContact == true && fsrData.rightFootContact == false) {
    fsrData.centerOfPressure =
      LFsrFL * theSensorData.data[SensorData::fsrLFL] + LFsrFR * theSensorData.data[SensorData::fsrLFR] +
      LFsrRL * theSensorData.data[SensorData::fsrLBL] + LFsrRR * theSensorData.data[SensorData::fsrLBR];

    fsrData.centerOfPressure /= leftSum;
  } else if (fsrData.leftFootContact == false && fsrData.rightFootContact == true) {
    fsrData.centerOfPressure =
      RFsrFL * theSensorData.data[SensorData::fsrRFL] + RFsrFR * theSensorData.data[SensorData::fsrRFR] +
      RFsrRL * theSensorData.data[SensorData::fsrRBL] + RFsrRR * theSensorData.data[SensorData::fsrRBR];
