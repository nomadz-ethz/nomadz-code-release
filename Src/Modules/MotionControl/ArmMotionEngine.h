...
private:
  ...
  public:
    ...
    bool autoReverse;    /**< If set to true, the arm will be moved to its default position once it stayed long enough in its
                            target state */
    int autoReverseTime; /**< Time in motion frames after which the arm should be automatically moved back to default
                            position */
    ...
    ArmMotion currentMotion; /**< The motion which is currently performed by this arm. Contains the actual target states in
                                order they should be reached */
     ...
     */
    void
    startMotion(ArmMotion motion, bool fast, bool autoReverse, int autoReverseTime, const FilteredJointData& currentJoints) {
      stateIndex = 0;
      interpolationTime = 1;
      targetTime = 0;
      isMotionActive = true;
      for (int i = 0; i < motion.states.size(); ++i) {
        if (this->id == ArmMotionRequest::right) {
          motion.states[i].angles[1] *= -1;
          motion.states[i].angles[2] *= -1;
          motion.states[i].angles[3] *= -1;
          motion.states[i].angles[4] *= -1;
        }
      }
