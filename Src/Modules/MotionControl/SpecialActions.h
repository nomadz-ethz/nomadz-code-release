...
private:
  ...
  public:
    ...
    float d[JointData::numOfJoints + 4]; /**< Represent a set of values from a data line. */
    /*
    //possible content:
    ...
    */
    ...
    void toJointRequest(JointRequest& jointRequest,
                        int& dataRepetitionCounter,
                        bool& interpolationMode,
                        bool& deShakeMode) const {
      for (int i = 0; i < JointData::numOfJoints; ++i) {
        jointRequest.jointAngles.angles[i] = (float)d[i + 1];
      }
      interpolationMode = (int(d[JointData::numOfJoints + 1]) & 1) != 0;
      deShakeMode = (int(d[JointData::numOfJoints + 1]) & 2) != 0;
      dataRepetitionCounter = int(d[JointData::numOfJoints + 2]);
    }
    ...
    void toHardnessRequest(HardnessData& hardnessRequest, int& hardnessInterpolationTime) {
      ...
      hardnessInterpolationTime = int(d[JointData::numOfJoints + 1]);
    }

    SpecialActionRequest::SpecialActionID getSpecialActionID() const {
      return SpecialActionRequest::SpecialActionID(short(d[JointData::numOfJoints + 3]));
    }
  };
  ...
  static PROCESS_WIDE_STORAGE(SpecialActions)
  ...
  unsigned lastSpecialActionTimeStamp;                     /**< time stamp of last executed special action. */
  ...
  bool isFallProtectionType(const SpecialActionRequest::SpecialActionID& specialActionType);
