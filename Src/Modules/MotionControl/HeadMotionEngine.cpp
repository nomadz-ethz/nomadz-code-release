...
HeadMotionEngine::HeadMotionEngine() {
  ...
  deathPoints[0] = Geometry::getCircle(Vector2<int>(45, -25), Vector2<int>(120, -20), Vector2<int>(90, -17));
  deathPoints[1] = Geometry::getCircle(Vector2<int>(-45, -25), Vector2<int>(-120, -20), Vector2<int>(-90, -17));
  deathPoints[2] = Geometry::getCircle(Vector2<int>(17, 46), Vector2<int>(85, 23), Vector2<int>(120, 27));
  deathPoints[3] = Geometry::getCircle(Vector2<int>(-17, 46), Vector2<int>(-85, 23), Vector2<int>(-120, 27));
...
void HeadMotionEngine::update(HeadJointRequest& headJointRequest) {
  ...
  float pan = requestedPan == JointData::off ? JointData::off
                                             : Range<>(theJointCalibration.joints[JointData::HeadYaw].minAngle,
                                                       theJointCalibration.joints[JointData::HeadYaw].maxAngle)
                                                 .limit(requestedPan);
  float tilt = requestedTilt == JointData::off ? JointData::off
                                               : Range<>(theJointCalibration.joints[JointData::HeadPitch].minAngle,
                                                         theJointCalibration.joints[JointData::HeadPitch].maxAngle)
                                                   .limit(requestedTilt);
  if (std::abs(pan - lastReqPan) > 0.05) {
    lastReqPan = pan;
  } else {
    pan = lastReqPan;
  }
  if (std::abs(tilt - lastReqTilt) > 0.05) {
    lastReqTilt = tilt;
  } else {
    tilt = lastReqTilt;
  }
  const float deltaTime = theFrameInfo.cycleTime;
  const Vector2<> position(headJointRequest.pan == JointData::off ? theFilteredJointData.angles[JointData::HeadYaw]
                                                                  : static_cast<float>(headJointRequest.pan),
                           headJointRequest.tilt == JointData::off ? theFilteredJointData.angles[JointData::HeadPitch]
                                                                   : static_cast<float>(headJointRequest.tilt));
  ...
  */
  ...
  headJointRequest.moving = pan != JointData::off && tilt != JointData::off &&
                            ((newPosition - position) / deltaTime).squareAbs() > sqr(maxAcc * deltaTime * 0.5f);
