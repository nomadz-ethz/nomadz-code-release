 ...
 *
 * @note The original authors are <A href="mailto:dueffert@informatik.hu-berlin.de">Uwe Düffert</A>, <A
 * href="mailto:Thomas.Roefer@dfki.de">Thomas Röfer</A>, Max Risler and Martin Lötzsch
 */
  ...
  for (int i = 0; i < numberOfNodes; ++i) {
    ...
    case 3:
      ...
      for (int j = 1; j < JointData::numOfJoints + 1; ++j) {
        stream >> nodeArray[i].d[j];
        if (nodeArray[i].d[j] != JointData::off && nodeArray[i].d[j] != JointData::ignore) {
...
SpecialActions::SpecialActions()
    : wasEndOfSpecialAction(false),
      ...
      hardnessInterpolationCounter(0), hardnessInterpolationLength(0), wasActive(false), dataRepetitionCounter(0),
      lastSpecialAction(SpecialActionRequest::numOfSpecialActionIDs), lastSpecialActionTimeStamp(0), mirror(false) {
  theInstance = this;
  ...
  if (!cm.exists()) {
    ...
    cm >> infos;
    for (std::vector<SpecialActionInfo>::const_iterator it = infos.specialActionInfos.begin();
         it != infos.specialActionInfos.end();
         it++) {
...
bool SpecialActions::getNextData(const SpecialActionRequest& specialActionRequest,
  ...
  while ((MotionNetNode::NodeType) short(motionNetData.nodeArray[currentNode].d[0]) != MotionNetNode::typeData) {
    // std::cout << "-> " << currentNode << " " << short(motionNetData.nodeArray[currentNode].d[0]) << std::endl;
    switch ((MotionNetNode::NodeType) short(motionNetData.nodeArray[currentNode].d[0])) {
    case MotionNetNode::typeHardness:
      lastHardnessRequest = specialActionsOutput.jointRequest.jointHardness; // currentHardnessRequest;
  ...
  if (specialActionsOutput.executedSpecialAction.specialAction != lastSpecialAction) {
    ...
    lastSpecialActionTimeStamp = theFrameInfo.time;
  }
...
void SpecialActions::calculateJointRequest(JointRequest& jointRequest) {
  ...
  if (interpolationMode) {
    ...
    for (int i = 0; i < JointData::numOfJoints; ++i) {
      f = lastRequest.jointAngles.angles[i];
      if (!mirror) {
        t = currentRequest.jointAngles.angles[i];
      } else {
      ...
      if (t == JointData::off || t == JointData::ignore) {
        jointRequest.jointAngles.angles[i] = t;
        // interpolate
      } else {
        jointRequest.jointAngles.angles[i] = (float)(t + (f - t) * ratio);
      }
    }
  } else {
    if (!mirror) {
...
bool SpecialActions::isFallProtectionType(const SpecialActionRequest::SpecialActionID& specialActionType) {
  return specialActionType == SpecialActionRequest::fallprotectionback ||
         specialActionType == SpecialActionRequest::fallprotectionfront ||
         specialActionType == SpecialActionRequest::fallprotectionside;
}
...
void SpecialActions::update(SpecialActionsOutput& specialActionsOutput) {
  ...
  if (theMotionSelection.specialActionMode != MotionSelection::deactive) {
    if (isFallProtectionType(theMotionSelection.specialActionRequest.specialAction) &&
        specialActionsOutput.executedSpecialAction.specialAction == SpecialActionRequest::stand) {
    ...
    if (dataRepetitionCounter <= 0) {
      if (!wasActive) {
        ...
        currentNode = 0;
        for (int i = 0; i < JointData::numOfJoints; ++i) {
          lastRequest.jointAngles.angles[i] = theFilteredJointData.angles[i];
        }
      ...
      if (wasEndOfSpecialAction) {
        specialActionsOutput.jointRequest.jointHardness.resetToDefault();
        if (!mirror) {
      ...
      if (!getNextData(theMotionSelection.specialActionRequest, specialActionsOutput)) {
        ...
        specialActionsOutput.isMotionDone = true;
        specialActionsOutput.odometryOffset = Pose2D();
        return;
      } else {
        specialActionsOutput.isMotionDone = false;
      }
    } else {
    ...
    calculateJointRequest(specialActionsOutput.jointRequest);

    // odometry update
    if (currentInfo.type == SpecialActionInfo::homogeneous || currentInfo.type == SpecialActionInfo::once) {
      if (mirror) {
        specialActionsOutput.odometryOffset = Pose2D(-currentInfo.odometryOffset.rotation,
                                                     currentInfo.odometryOffset.translation.x,
                                                     -currentInfo.odometryOffset.translation.y);
      } else {
    ...
    // (specialActionsOutput.executedSpecialAction.specialAction == SpecialActionRequest::stand ||
    //  specialActionsOutput.executedSpecialAction.specialAction == SpecialActionRequest::standHigh) &&
    //     (theFrameInfo.getTimeSince(lastSpecialActionTimeStamp) < 2000)
    //   ? true
    //   : false;
    if (deShakeMode) {
      for (int i = JointData::LShoulderPitch; i <= JointData::RElbowRoll; ++i) {
        if (randomFloat() < 0.25) {
          specialActionsOutput.jointRequest.jointAngles.angles[i] = JointData::off;
        }
      }
    }
  }
