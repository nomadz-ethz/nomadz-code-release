...
bool ArmMotionEngine::newMotionPossible(Arm& arm) {
  return !arm.isMotionActive && theMotionInfo.motionRequest.motion == MotionRequest::walk && theGroundContactState.contact;
...
void ArmMotionEngine::updateArm(Arm& arm, ArmMotionEngineOutput& armMotionEngineOutput) {
  ...
  if (newMotionPossible(arm)) {
    ...
    if (theArmMotionRequest.motion[arm.id] != ArmMotionRequest::useDefault) {
    ...
    } else if (!theArmMotionRequest.forceReturnDefault && theFrameInfo.getTimeSince(arm.lastContactAction) > actionDelay &&
               (arm.id == ArmMotionRequest::left ? theArmContactModel.contactLeft : theArmContactModel.contactRight)) {
      // #ifdef TARGET_ROBOT
      // check for armcontact
      ArmContactModel::PushDirection dir =
        (arm.id == ArmMotionRequest::left) ? theArmContactModel.pushDirectionLeft : theArmContactModel.pushDirectionRight;
      ...
      //   //                 theArmMotionRequest.autoReverseTime[arm.id],
      //   //                 theFilteredJointData);
      //   nextMotion.states[0].angles[1] = 0.25 + std::abs(theWalkingEngineOutput.speed.translation.y) * 0.0005;
      //   if (arm.id == ArmMotionRequest::left) {
      //     nextMotion.states[0].angles[0] =
      //       (90_deg - (theRobotModel.limbs[MassCalibration::footRight].translation.x + 12) * 0.006);
      //   } else {
      //     nextMotion.states[0].angles[0] =
      //       (90_deg - (theRobotModel.limbs[MassCalibration::footLeft].translation.x + 12) * 0.006);
      //   }
      //   arm.startMotion(nextMotion,
      //                   theArmMotionRequest.fast[arm.id],
      //                   theArmMotionRequest.autoReverse[arm.id],
  ...
  if (arm.isMotionActive) {
    ...
    if (arm.stateIndex == arm.currentMotion.states.size()) {
      ...
      if (arm.currentMotion.id == ArmMotionRequest::useDefault) {
        ...
        armMotionEngineOutput.arms[arm.id].move = false;
                 ...
                 (theArmMotionRequest.motion[arm.id] != arm.currentMotion.id && !arm.contactTriggered) ||
                 theArmMotionRequest.forceReturnDefault) // different motion requested
      {
        // start a reversed motion to reach the default position

        for (int i = 0; i < arm.currentMotion.states.size(); ++i) {
          if (arm.id == ArmMotionRequest::right) {
            arm.currentMotion.states[i].angles[1] *= -1;
            arm.currentMotion.states[i].angles[2] *= -1;
            arm.currentMotion.states[i].angles[3] *= -1;
            arm.currentMotion.states[i].angles[4] *= -1;
          }
        }

        arm.startMotion(
          arm.currentMotion.reverse(defaultPos), theArmMotionRequest.fast[arm.id], false, 0, theFilteredJointData);
      } else {
    ...
    armMotionEngineOutput.arms[arm.id].angles[2] = 0.f;
    armMotionEngineOutput.arms[arm.id].angles[3] = 0.f;
    armMotionEngineOutput.arms[arm.id].angles[5] = 0.f;
    if (arm.id == ArmMotionRequest::left) {
      armMotionEngineOutput.arms[arm.id].angles[0] =
        (90_deg - (theWalkingEngineOutput.requestedRightFoot.translation.x + 12) * 0.006);
      armMotionEngineOutput.arms[arm.id].angles[1] = 0.25 + std::abs(theWalkingEngineOutput.speed.translation.y) * 0.0005;
      armMotionEngineOutput.arms[arm.id].angles[4] = -90_deg;
    } else {
      armMotionEngineOutput.arms[arm.id].angles[0] =
        (90_deg - (theWalkingEngineOutput.requestedLeftFoot.translation.x + 12) * 0.006);
      armMotionEngineOutput.arms[arm.id].angles[1] = -0.25 - std::abs(theWalkingEngineOutput.speed.translation.y) * 0.0005;
      armMotionEngineOutput.arms[arm.id].angles[4] = 90_deg;
    }
  }
...
void ArmMotionEngine::createOutput(Arm& arm, ArmMotion::ArmAngles target, int& time, ArmMotion::ArmAngles& result) {
  ...
  for (unsigned i = 0; i < from.angles.size(); ++i) {
    ...
    result.hardness[i] =
      target.hardness[i] == HardnessData::useDefault ? theHardnessSettings.hardness[arm.firstJoint + i] : target.hardness[i];
  }
  ++time;
}
