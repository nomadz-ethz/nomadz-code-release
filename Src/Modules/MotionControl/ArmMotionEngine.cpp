...
bool ArmMotionEngine::newMotionPossible(Arm& arm) {
  return !arm.isMotionActive && (theGameInfo.state == STATE_PLAYING || theGameInfo.state == STATE_READY) &&
         (theMotionInfo.motionRequest.motion == MotionRequest::walk) && theGroundContactState.contact;
}
...
void ArmMotionEngine::updateArm(Arm& arm, ArmMotionEngineOutput& armMotionEngineOutput) {
  ...
  if (theFallDownState.state == FallDownState::falling && arm.isMotionActive &&
      arm.currentMotion.id != ArmMotionRequest::falling) {
    arm.startMotion(allMotions[ArmMotionRequest::falling], true, false, 0, theFilteredJointData);
  } else if ((theFallDownState.state == FallDownState::falling || theFallDownState.state == FallDownState::onGround ||
              !theGroundContactState.contact) &&
             !arm.isMotionActive) {
    // fallen
    return;
  } else if (theMotionInfo.motionRequest.motion == MotionRequest::kick && arm.isMotionActive &&
             arm.currentMotion.id != ArmMotionRequest::useDefault) {
    ...
    if (arm.stateIndex == arm.currentMotion.states.size()) {
      ...
      if (arm.currentMotion.id == ArmMotionRequest::useDefault) {
        ...
        arm.isMotionActive = false;
                 ...
                 (theArmMotionRequest.motion[arm.id] != arm.currentMotion.id &&
                  !arm.contactTriggered)) // different motion requested
      {
