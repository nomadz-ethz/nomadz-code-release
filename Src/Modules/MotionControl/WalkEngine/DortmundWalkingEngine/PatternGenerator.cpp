...
PatternGenerator::PatternGenerator(const WalkingEngineParams& theWalkingEngineParams,
    ...
    : theWalkingEngineParams(theWalkingEngineParams), thePatternGenRequest(thePatternGenRequest),
      theRobotModel(theRobotModel), theRobotDimensions(theRobotDimensions), theFallDownState(theFallDownState),
      theControllerParams(theControllerParams), theMotionSelection(theMotionSelection), theWalkingInfo(theWalkingInfo),
      theReferenceModificator(theReferenceModificator), theFrameInfo(theFrameInfo), theBallModel(theBallModel),
      theBallModelAfterPreview(theBallModelAfterPreview), speedBuffer(Pose2D()) {
  wasReady = false;
  reset();
  baseRot = 0;
...
void PatternGenerator::calcWalkParams() {
  if (dynamicDuration) {
    if (theMotionSelection.walkRequest.kickStrength == 0) {
      float distToMax = std::abs(theWalkingEngineParams.speedLimits.xForward / 1000 - currentMovement.speed.translation.x) /
