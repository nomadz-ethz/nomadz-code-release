...
CSConverter::CSConverter(const Footpositions& theFootpositions,
    ...
    : theFootpositions(theFootpositions), theTargetCoM(theTargetCoM), theWalkingEngineParams(theWalkingEngineParams),
      theControllerParams(theControllerParams), theActualCoMRCS(theActualCoMRCS), theFallDownState(theFallDownState),
      theInertiaSensorData(theInertiaSensorData),
      // theFsrSensorData(theFsrSensorData),
      theBodyTilt(theBodyTilt), theTorsoMatrix(theTorsoMatrix), theRobotModel(theRobotModel), theRobotInfo(theRobotInfo),
      theFootSteps(theFootSteps), theArmContact(theArmContact), theRobotDimensions(theRobotDimensions),
      theWalkCalibration(theWalkCalibration) {
  reset();

  InMapFile file("csConverter.cfg");
  file >> params;
}
...
void CSConverter::toRobotCoords(StepData* requiredOffset, Point newCoMTarget, Footposition curPos, Point CoM) {
  ...
  if (!curPos.instantKickRunning) {
    ...
    if (theWalkingEngineParams.sensorControlRatio[0] > 0)
    {
      ...
      angleY = params.filter_alpha * -theInertiaSensorData.angle.y() + (1 - params.filter_alpha) * angleY;
      ...
      if (!curPos.onFloor[LEFT_FOOT] || !curPos.onFloor[RIGHT_FOOT])
      {
        ...
        curPos.footPos[footInAir].rx -= intfac * theInertiaSensorData.angle.x();
...
#endif
    {
      ...
      if (footInAir != -1) {
        ...
        float stepHeight =
          (1 - yFac) * xStepHeight +
          yFac * theWalkingEngineParams.footMovement
                   .stepHeight[2]; // For sidesteps, raise feet higher if faster, but here calculated only for the next value

        float heightOverGroundFac =
          curPos.footPos[footInAir].z / stepHeight; // fraction of current height over ground vs. maximum height over ground
        // PD controller for footpitch using body angle
        float cX = 0;
        float cY = theWalkingEngineParams.footMovement.footPitchPD[0] * theInertiaSensorData.angle.y +
                   theWalkingEngineParams.footMovement.footPitchPD[1] * theInertiaSensorData.gyro.y * 0.0075f;
        ...
        // ------------ DONT USE BOTH! ---------------
        Point stanceToAirFootVec =
          curPos.footPos[footInAir] - curPos.footPos[!footInAir]; // Vector from stance to air foot origin
        ...
        PLOT("module:CSConverter:angleY", theInertiaSensorData.angle.y);
        PLOT("module:CSConverter:gyroY", theInertiaSensorData.gyro.y);
  ...
  *requiredOffset = tempStep;
  requiredOffset->footPos[RIGHT_FOOT].x = tempStep.footPos[RIGHT_FOOT].x * std::cos(curPos.direction) +
                                          tempStep.footPos[RIGHT_FOOT].y * std::sin(curPos.direction);
  requiredOffset->footPos[LEFT_FOOT].x =
    tempStep.footPos[LEFT_FOOT].x * std::cos(curPos.direction) + tempStep.footPos[LEFT_FOOT].y * std::sin(curPos.direction);

  requiredOffset->footPos[RIGHT_FOOT].y = tempStep.footPos[RIGHT_FOOT].y * std::cos(curPos.direction) -
                                          tempStep.footPos[RIGHT_FOOT].x * std::sin(curPos.direction);
  requiredOffset->footPos[LEFT_FOOT].y =
    tempStep.footPos[LEFT_FOOT].y * std::cos(curPos.direction) - tempStep.footPos[LEFT_FOOT].x * std::sin(curPos.direction);
  ...
  for (int foot = LEFT_FOOT; foot <= RIGHT_FOOT; foot++) {
    ...
    if (theWalkingEngineParams.sensorControl.steppingRotSpeedCapture > 0) {
      ...
      requiredOffset->footPos[foot] =
        requiredOffset->footPos[foot].norm() * (requiredOffset->footPos[foot].getPositionVecLen() - std::abs(dz));
    }
  }

  // Rotate the foot positions relative to CoM around the x axis (roll)
  tempStep = *requiredOffset;
  tempStep.footPos[RIGHT_FOOT].y = requiredOffset->footPos[RIGHT_FOOT].y * std::cos(-curPos.roll) -
                                   requiredOffset->footPos[RIGHT_FOOT].z * std::sin(-curPos.roll);
  tempStep.footPos[RIGHT_FOOT].z = requiredOffset->footPos[RIGHT_FOOT].y * std::sin(-curPos.roll) +
                                   requiredOffset->footPos[RIGHT_FOOT].z * std::cos(-curPos.roll);

  tempStep.footPos[LEFT_FOOT].y = requiredOffset->footPos[LEFT_FOOT].y * std::cos(-curPos.roll) -
                                  requiredOffset->footPos[LEFT_FOOT].z * std::sin(-curPos.roll);
  tempStep.footPos[LEFT_FOOT].z = requiredOffset->footPos[LEFT_FOOT].y * std::sin(-curPos.roll) +
                                  requiredOffset->footPos[LEFT_FOOT].z * std::cos(-curPos.roll);
  *requiredOffset = tempStep;
...
#define ODOMETRY_FROM_INERTIA_MATRIX
  ...
  if (params.odometryVariant == 1) {
    ...
    if (theRobotInfo.hasFeature(RobotInfo::zGyro)) {
      odometry.r = theInertiaSensorData.gyro.z * 0.01f; // was -gyro, but BHuman changed sign at NaoProvider
    }
    lastPos = newPos;
    originWCS = lastPos;
  }
...
void CSConverter::updateKinematicRequest(KinematicRequest& kinematicRequest) {
  ...
  if (isRunning) {
    ...
    if (std::abs(currentStep.footPos[LEFT_FOOT].x) < theWalkingEngineParams.walkTransition.stopPosThresholdX &&
        ...
        std::abs(currentStep.footPos[LEFT_FOOT].y + currentStep.footPos[RIGHT_FOOT].y) <
          theWalkingEngineParams.walkTransition.stopPosThresholdY &&
...
#if 0
  RotationMatrix rotM(theInertiaSensorData.angle.x(),
    theInertiaSensorData.angle.y(), 0);

  Vector3f rotAcc = rotM * theInertiaSensorData.acc;
#else
  const Vector3<>& rotAcc = theInertiaSensorData.acc;
#endif
  ...
  double yOffset = curparams.comOffsets.yFixed;
  if (zerocount < 100) {
  ...
  double dynYOffset = 0.0f;
  int yDir = theFootpositions.speed.y < 0;
  dynYOffset = theWalkingEngineParams.comOffsets.ySpeedDependent[yDir] * std::abs(theFootpositions.speed.y * 1000) /
               theWalkingEngineParams.speedLimits.y;
  yOffset += dynYOffset;

  double dynXOffset = 0;
  if (theFootpositions.speed.x > 0) {
    dynXOffset = theWalkingEngineParams.comOffsets.xSpeedDependent * theFootpositions.speed.x * 1000 /
                 theWalkingEngineParams.speedLimits.xForward;
  }
  xOffset += dynXOffset;

  // position left foot
  kinematicRequest.leftFoot[0] =
    (float)(currentStep.footPos[LEFT_FOOT].x * 1000 - (INTERPOLATE(xOffset, (float)xOffset) * 1000) -
            (INTERPOLATE(comXShift, (float)comXShift) * 1000));
  kinematicRequest.leftFoot[1] =
    (float)(currentStep.footPos[LEFT_FOOT].y * 1000 - (INTERPOLATE(yOffset, (float)yOffset) * 1000) -
            (INTERPOLATE(comYShift, (float)comYShift) * 1000));
  kinematicRequest.leftFoot[2] = (float)(currentStep.footPos[LEFT_FOOT].z * 1000);

  // position right foot
  kinematicRequest.rightFoot[0] =
    (float)(currentStep.footPos[RIGHT_FOOT].x * 1000 - (INTERPOLATE(xOffset, (float)xOffset) * 1000) -
            (INTERPOLATE(comXShift, (float)comXShift) * 1000));
  kinematicRequest.rightFoot[1] =
    (float)(currentStep.footPos[RIGHT_FOOT].y * 1000 - (INTERPOLATE(yOffset, (float)yOffset) * 1000) -
            (INTERPOLATE(comYShift, (float)comYShift) * 1000));
  ...
  if (currentStep.onFloor[LEFT_FOOT] && !currentStep.onFloor[RIGHT_FOOT]) {
    for (int i = 0; i < 6; i++) {
      {
        kinematicRequest.offsets.angles[i + (int)JointData::LHipYawPitch] =
          theWalkingEngineParams.jointCalibration.offsetLeft[i];
      }
      INTERPOLATE_ARRAY_ELEMENT_AND_STORE(offsetLeft, kinematicRequest.offsets.angles[i + (int)JointData::LHipYawPitch], i);
    }
  }
  if (!currentStep.onFloor[LEFT_FOOT] && currentStep.onFloor[RIGHT_FOOT]) {
    for (int i = 0; i < 6; i++) {
      {
        kinematicRequest.offsets.angles[i + (int)JointData::RHipYawPitch] =
          theWalkingEngineParams.jointCalibration.offsetRight[i];
      }

      INTERPOLATE_ARRAY_ELEMENT_AND_STORE(offsetRight, kinematicRequest.offsets.angles[i + (int)JointData::RHipYawPitch], i);
    }
  }
  for (int i = 0; i < 6; i++) {
...
void CSConverter::updateWalkingInfo(WalkingInfo& walkingInfo) {
  ...
  JointData jA;
  for (int i = 0; i < 12; i++) {
    jA.angles[i + JointData::LHipYawPitch] = theWalkingEngineParams.jointCalibration.jointCalibration[i];
  }
  for (int i = 0; i < 6; i++) {
    jA.angles[i + JointData::LHipYawPitch] += theWalkingEngineParams.jointCalibration.offsetLeft[i];
  }
  for (int i = 0; i < 6; i++) {
    jA.angles[i + JointData::RHipYawPitch] += theWalkingEngineParams.jointCalibration.offsetRight[i];
  }
  ...
  // for (int i = 2; i < 5; i++)
  //  walkingInfo.desiredBodyRot.y() += theWalkingEngineParams.jointCalibration.jointCalibration[currentStep.onFloor[1] * 6 +
  //  i];
  if (fallingDown) {
