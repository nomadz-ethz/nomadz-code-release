...
PatternGenerator2016::PatternGenerator2016() : speedBuffer(Pose2D()) {
  wasReady = false;
  reset();
  baseRot = 0;
...
void PatternGenerator2016::initWalkingPhase() {
  if (currentWalkingPhase != unlimitedDoubleSupport && currentWalkingPhase != unlimitedSingleSupportLeft &&
...
void PatternGenerator2016::transitionToWalk(MovementInformation& moveInf) {
  ...
  currentMovement.speed = Pose2D();
  newMovement = moveInf;
  applySpeed(true,
             (currentWalkingPhase == firstSingleSupport && newMovement.speed.translation.y >= 0) ||
               (currentWalkingPhase == secondSingleSupport && (newMovement.speed.translation.y < 0)) ||
               currentWalkingPhase == unlimitedDoubleSupport,
             (currentWalkingPhase == firstSingleSupport && newMovement.speed.rotation < 0) ||
               (currentWalkingPhase == secondSingleSupport && newMovement.speed.rotation > 0) ||
               currentWalkingPhase == unlimitedDoubleSupport,
             true);
...
bool PatternGenerator2016::transitionToCustomStepsNoCheck() {
  if (theMotionSelection.walkRequest.stepRequest >= WalkRequest::StepRequest::beginScript //&&
                                                                                          //      deltaDirection[0] == 0 &&
                                                                                          //      deltaDirection[1] == 0 &&
                                                                                          //      deltaDirection[2] == 0 &&
                                                                                          //      deltaDirection[3] == 0
    ...
    if ((currentWalkingPhase == secondSingleSupport && stepFiles[idx].steps.begin()->onFloor[1]) ||
        (currentWalkingPhase == firstSingleSupport && stepFiles[idx].steps.begin()->onFloor[0])) {
...
bool PatternGenerator2016::transitionToCustomSteps() {
  ...
  // Nur aktivieren, wenn der n�chste Schritt kleiner werden muss, um
  // Zielpunkt zu erreichen. Muss er gr��er werden, liegt es au�erhalb
  // der Reichweite. Um diese F�lle zu vermeiden muss die aktuelle
  // Geschwindigkeit hier automatisch gew�hlt werden, so dass der Fall
  // fr�her oder sp�ter eintritt.

  // Vector2<> ballPos = theBallModelAfterPreview.estimate.position;
  Vector2<> ballPos = theBallModel.estimate.position;
  coordinateSystem = Point(theWalkingInfo.ballCStoWalkingCS(ballPos).x, theWalkingInfo.ballCStoWalkingCS(ballPos).y);
  ...
  if (theMotionSelection.walkRequest.stepRequest >= WalkRequest::StepRequest::beginScript) {
    ...
    if ((currentWalkingPhase == firstDoubleSupport && stepFiles[idx].steps.begin()->onFloor[1]) ||
        (currentWalkingPhase == secondDoubleSupport && stepFiles[idx].steps.begin()->onFloor[0])) {
      executedStep = stepFiles[idx].steps.begin();
      int footIndex = executedStep->onFloor[LEFT_FOOT] ? LEFT_FOOT : RIGHT_FOOT;
      ...
      float maxRadius = static_cast<float>(sqrt(
                          theWalkingEngineParams.speedLimits.xForwardOmni * theWalkingEngineParams.speedLimits.xForwardOmni +
                          theWalkingEngineParams.speedLimits.y * theWalkingEngineParams.speedLimits.y)) *
                        theControllerParams.dt * singleSupportDuration() / 1000;
...
void PatternGenerator2016::calcWalkParams() {
  if (dynamicDuration) {
    if (!steps->empty()) {
      float factorX =
        fabs(theWalkingEngineParams.speedLimits.xForward / 1000.f - (float)currentMovement.speed.translation.x) /
        (theWalkingEngineParams.speedLimits.xForward / 1000.f);
      float factorY =
        fabs(theWalkingEngineParams.speedLimits.y / 1000.f - fabs((float)currentMovement.speed.translation.y)) /
        (theWalkingEngineParams.speedLimits.y / 1000.f);
      float distToMax = std::min(factorX, factorY);
      curStepDuration = distToMax * theWalkingEngineParams.footMovement.maxStepDuration +
                        (1.f - distToMax) * theWalkingEngineParams.footMovement.minStepDuration;
    } else {
...
void PatternGenerator2016::setCustomStepDeltaDirection() {
  ...
  _nextStep++;
  if (_nextStep != stepFiles[currentStepFileIndex].steps.end() && _nextStep->onFloor[LEFT_FOOT] &&
      _nextStep->onFloor[RIGHT_FOOT]) {
    targetRot = (_nextStep->footPos[LEFT_FOOT].rotation - _nextStep->footPos[RIGHT_FOOT].rotation) * 0.5f +
                _nextStep->footPos[RIGHT_FOOT].rotation;
...
void PatternGenerator2016::updateCounter() {
  ...
  // int previewLength;
  if (currentMovement.speed.translation.x == 0 && currentMovement.speed.translation.y == 0 &&
      currentMovement.speed.rotation == 0 && currentStepFileIndex == -1) {
    stopPossible = true;
  }
  ...
  if (stateCounter == 0) {
    ...
    case walking:
      ...
      case firstDoubleSupport:
        ...
        robotPoseAfterStep.x = robotPose.x + footModifier[secondDoubleSupport][ROBOT_POSE].x +
        ...
        robotPoseAfterStep.y = robotPose.y + footModifier[secondDoubleSupport][ROBOT_POSE].y +
        ...
        if (currentMovement.speed.rotation < 0) {
          robotPoseAfterStep.r = robotPose.r + footModifier[secondDoubleSupport][ROBOT_POSE].r +
                                 footModifier[firstSingleSupport][ROBOT_POSE].r +
                                 footModifier[firstDoubleSupport][ROBOT_POSE].r;
        }

        if (currentMovement.speed.rotation > 0) {
          robotPoseAfterStep.r =
            robotPose.r + footModifier[secondDoubleSupport][ROBOT_POSE].r + footModifier[firstSingleSupport][ROBOT_POSE].r +
            footModifier[firstDoubleSupport][ROBOT_POSE].r + footModifier[secondSingleSupport][ROBOT_POSE].r;
        }
      ...
      case secondDoubleSupport:
        ...
        robotPoseAfterStep.x = robotPose.x + footModifier[firstDoubleSupport][ROBOT_POSE].x +
                               footModifier[secondSingleSupport][ROBOT_POSE].x +
                               footModifier[secondDoubleSupport][ROBOT_POSE].x;
        robotPoseAfterStep.y = robotPose.y + footModifier[firstDoubleSupport][ROBOT_POSE].y +
                               footModifier[secondSingleSupport][ROBOT_POSE].y +
                               footModifier[secondDoubleSupport][ROBOT_POSE].y;

        if (currentMovement.speed.rotation < 0) {
          robotPoseAfterStep.r =
            robotPose.r + footModifier[firstDoubleSupport][ROBOT_POSE].r + footModifier[secondSingleSupport][ROBOT_POSE].r +
            footModifier[secondDoubleSupport][ROBOT_POSE].r + footModifier[firstSingleSupport][ROBOT_POSE].r;
        }

        if (currentMovement.speed.rotation > 0) {
          robotPoseAfterStep.r = robotPose.r + footModifier[firstDoubleSupport][ROBOT_POSE].r +
...
StepData PatternGenerator2016::getNextStep() {
  ...
  // #endif
  int crouchingLength =
    wasReady ? theWalkingEngineParams.walkTransition.crouchingDownPhaseLength : 30; // Was ist das f�r ein Hack?!
  ...
  case standby:
    // startZ=sqrt(pow(theWalkingEngineParams.maxLegLength, 2)-pow(theWalkingEngineParams.xOffset,
    // 2)-pow(theWalkingEngineParams.footYDistance, 2));
    step.direction = 0;
...
unsigned int PatternGenerator2016::singleSupportDuration() {
  if (currentStepFileIndex != -1) {
    ...
    } else if (std::next(executedStep) != stepFiles[currentStepFileIndex].steps.end()) {
...
void PatternGenerator2016::addStep() {
  ...
  if (currentStepFileIndex != -1) {
    for (int i = 0; i < 2; i++) {
      ...
      if (!newStep.onFloor[i]) {
        newStep.footPos[i].z = newStep.speed.x > 0 ? theWalkingEngineParams.footMovement.stepHeight[0]
  ...
  for (int i = 0; i < 2; i++) {
    ...
    {
      // TODO: fix this crash
      // ASSERT(!(lastStep.onFloor[i] && newStep.onFloor[i]) ||
      // (lastStep.footPos[i].euklidDistance3D(newStep.footPos[i]-appliedReactiveStep[i]) < 0.001 &&
      // fabs(lastStep.footPos[i].r - newStep.footPos[i].r) < 0.01));
    }
  }
...
void PatternGenerator2016::update(FootSteps& steps) {
  DEBUG_RESPONSE_ONCE("module:PatternGenerator2016:saveCustomSteps", saveStepFiles(););
  DEBUG_RESPONSE_ONCE("module:PatternGenerator2016:loadCustomSteps", loadStepFiles(););
...
void PatternGenerator2016::loadStepFile(std::string file, int idx) {
  ...
  ASSERT(!(stepFiles[idx].steps.front().onFloor[0] && stepFiles[idx].steps.front().onFloor[1]) &&
         !(stepFiles[idx].steps.back().onFloor[0] && stepFiles[idx].steps.back().onFloor[1]));

  // First steps have zero rotation by definition
  ASSERT(stepFiles[idx].steps.front().footPos[0].rotation == 0 && stepFiles[idx].steps.front().footPos[1].rotation == 0);
}
