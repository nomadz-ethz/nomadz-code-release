...
void FLIPMObserver::update(ObservedFLIPMError& observedFLIPMError) {
  ...
  // Vector2f Offset(-theWalkingEngineParams.comOffsets.xFixed, 0.0f - theWalkingEngineParams.comOffsets.yFixed - yOffset /
  // 1000.0f);
  Vector2<> Offset(0.f, 0.0f);
  ...
  Vector2<> targetAccWCS(theTargetCoM.state_x[2], theTargetCoM.state_y[2]);
  Vector2<> targetCoM1WCS(theTargetCoM.state_x[0] + Offset.x, theTargetCoM.state_y[0] + Offset.y);
  Vector2<> targetCoM2WCS(theTargetCoM.state_x[3] + Offset.x, theTargetCoM.state_y[3] + Offset.y);
  ...
  Vector2<> realAcc, realCoM1, realCoM2;
  ...
  if (flipmObserverParams.ACC1Provider == FLIPMObserverParams::ZMP_CoM1_Acc) {
    ...
    filteredAccX =
      filteredAccX * (1 - filter) + (-1 * theZMPModel.ZMP_WCS.x + realCoM1DelayBuffer[flipmObserverParams.CoM1Delay].x) *
                                      (paramsFLIPMObsvX.g / paramsFLIPMObsvX.z_h) * filter;
    filteredAccY =
      filteredAccY * (1 - filter) + (-1 * theZMPModel.ZMP_WCS.y + realCoM1DelayBuffer[flipmObserverParams.CoM1Delay].y) *
                                      (paramsFLIPMObsvY.g / paramsFLIPMObsvY.z_h) * filter;
    ...
    float filter = flipmObserverParams.accFilter;
    filteredAccX = filteredAccX * (1 - filter) + (theInertiaSensorData.acc.x) * filter;
    filteredAccY = filteredAccY * (1 - filter) + (theInertiaSensorData.acc.y) * filter;
    realAcc.x = filteredAccX;
    realAcc.y = filteredAccY;
  }
  ...
  Vector2<> ACC_1_diff, CoM_1_diff, CoM_2_diff;
  ...
  if (theTargetCoM.isRunning &&
    ...
    ZMPDelay.x() = coM1DelayBuffer[theWalkingEngineParams.halSensorDelay].x() -
    (accDelayBuffer[theWalkingEngineParams.sensorDelay].x()*(paramsFLIPMObsv.z_h / paramsFLIPMObsv.g));
    ZMPDelay.y() = coM1DelayBuffer[theWalkingEngineParams.halSensorDelay].y() -
    (accDelayBuffer[theWalkingEngineParams.sensorDelay].y()*(paramsFLIPMObsv.z_h / paramsFLIPMObsv.g));
    ...
    CoM_1_diff = realCoM1DelayBuffer[flipmObserverParams.CoM1Delay] -
                 coM1DelayBuffer[theWalkingEngineParams.sensorControl.halSensorDelay];
    CoM_2_diff = realCoM1DelayBuffer[flipmObserverParams.CoM1Delay] - realCoM2DelayBuffer[flipmObserverParams.CoM2Delay] +
                 Vector2<>(flipmObserverParams.CoM2DiffXOffset, flipmObserverParams.CoM2DiffYOffset);

    ACC_1_diff.x = ACC_1_diff.x > 0 ? std::min(ACC_1_diff.x, flipmObserverParams.maxDiffACCClip)
                                    : std::max(ACC_1_diff.x, -flipmObserverParams.maxDiffACCClip);
    ACC_1_diff.y = ACC_1_diff.y > 0 ? std::min(ACC_1_diff.y, flipmObserverParams.maxDiffACCClip)
                                    : std::max(ACC_1_diff.y, -flipmObserverParams.maxDiffACCClip);

    CoM_1_diff.x = CoM_1_diff.x > 0 ? std::min(CoM_1_diff.x, flipmObserverParams.maxDiffCoMClip)
                                    : std::max(CoM_1_diff.x, -flipmObserverParams.maxDiffCoMClip);
    CoM_1_diff.y = CoM_1_diff.y > 0 ? std::min(CoM_1_diff.y, flipmObserverParams.maxDiffCoMClip)
                                    : std::max(CoM_1_diff.y, -flipmObserverParams.maxDiffCoMClip);

    CoM_2_diff.x = CoM_2_diff.x > 0 ? std::min(CoM_2_diff.x, flipmObserverParams.maxDiffCoMClip)
                                    : std::max(CoM_2_diff.x, -flipmObserverParams.maxDiffCoMClip);
    CoM_2_diff.y = CoM_2_diff.y > 0 ? std::min(CoM_2_diff.y, flipmObserverParams.maxDiffCoMClip)
                                    : std::max(CoM_2_diff.y, -flipmObserverParams.maxDiffCoMClip);
  } else {
    ACC_1_diff = Vector2<>(0, 0);
    CoM_1_diff = Vector2<>(0, 0);
    CoM_2_diff = Vector2<>(0, 0);
    filteredAccX = 0.0;
    filteredAccY = 0.0;
  }
...
/* calculate if walking or not!*/
  ...
  if (flipmObserverParams.useRCS) {
    ...
    CoM_1_diff.x *=
      (flipmObserverParams.sensorControlRatioObserverX[0] * localSensorScale) * flipmObserverParams.sensorXFactor;
    CoM_1_diff.y *=
      (flipmObserverParams.sensorControlRatioObserverY[0] * localSensorScale) * flipmObserverParams.sensorYFactor;
    CoM_1_diff.rotate(robotPosition.rotation);

    ACC_1_diff.rotate(-robotPosition.rotation);
    ACC_1_diff.x *=
      (flipmObserverParams.sensorControlRatioObserverX[1] * localSensorScale) * flipmObserverParams.sensorXFactor;
    ACC_1_diff.y *=
      (flipmObserverParams.sensorControlRatioObserverY[1] * localSensorScale) * flipmObserverParams.sensorYFactor;
    ACC_1_diff.rotate(robotPosition.rotation);

    CoM_2_diff.rotate(-robotPosition.rotation);
    CoM_2_diff.x *=
      (flipmObserverParams.sensorControlRatioObserverX[2] * localSensorScale) * flipmObserverParams.sensorXFactor;
    CoM_2_diff.y *=
      (flipmObserverParams.sensorControlRatioObserverY[2] * localSensorScale) * flipmObserverParams.sensorYFactor;
    CoM_2_diff.rotate(robotPosition.rotation);
  }
  ...
  if (flipmObserverParams.activateSensorX == true) {
    observedFLIPMError.ObservedError[0][0] = paramsFLIPMObsvX.L * Vector3f(CoM_1_diff.x, ACC_1_diff.x, CoM_2_diff.x);
  } else {
    observedFLIPMError.ObservedError[0][0] = paramsFLIPMObsvX.L * Vector3f(0, 0, 0);
  }

  if (flipmObserverParams.activateSensorY == true) {
    observedFLIPMError.ObservedError[0][1] = paramsFLIPMObsvY.L * Vector3f(CoM_1_diff.y, ACC_1_diff.y, CoM_2_diff.y);
  } else {
    observedFLIPMError.ObservedError[0][1] = paramsFLIPMObsvY.L * Vector3f(0, 0, 0);
  }
  ...
  LOG("FLIPMObserver", "angleX", theInertiaSensorData.angle.x);
  LOG("FLIPMObserver", "angleY", theInertiaSensorData.angle.y);
  if (theTargetCoM.isRunning) {
