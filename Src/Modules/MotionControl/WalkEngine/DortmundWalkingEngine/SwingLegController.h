...
public:
   ...
   */
  SwingLegController(const WalkingEngineParams& theWalkingEngineParams,
      ...
      :
        ...
        theWalkingEngineParams(theWalkingEngineParams), theFootSteps(theFootSteps), theBallModel(theBallModel),
        theMotionRequest(theMotionRequest), theWalkingInfo(theWalkingInfo), theFallDownState(theFallDownState),
        thePatternGenRequest(thePatternGenRequest), theObservedError(theObservedError),
        theControllerParams(theControllerParams), infos(phases), debug(false), modification(localModification) {
    reset();
  };
...
private:
  ...
  public:
    ...
    void replace(const Point& p, int footNum) { footPos[footNum] = p; }

    void modify(const Point& p, int footNum) { footPos[footNum] += p; }
  ...
  public:
    Phase(const Footposition& fp) { members.push_back(fp); }
    ~Phase() { members.clear(); }
    ...
    void replace(Point p[], int footNum) {
      int i = 0;
      for (FootList::iterator fp = members.begin(); fp != members.end(); fp++, i++) {
    ...
    const Point& getSidestep() const { return sidestep; }
    void modifySidestepSum(const Point& p) { sidestep += p; }
  ...
  void modifyFp(int startFoot,
                ...
                int dim);
  bool modPossible(PhaseList::iterator ph, float modificatorRCS, int dim);
};
