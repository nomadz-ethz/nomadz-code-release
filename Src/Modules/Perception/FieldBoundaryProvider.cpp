...
void FieldBoundaryProvider::update(FieldBoundary& fieldBoundary) {
  ...
  if (theCameraMatrix.isValid) {
    ...
    InImage boundarySpots = fieldBoundary.boundarySpots;
    bool valid = cleanupBoundarySpots(boundarySpots);
    fieldBoundary.boundarySpots.clear();
    for (const auto& el : boundarySpots) {
      fieldBoundary.boundarySpots.push_back(el);
    }

    fieldBoundary.convexBoundaryCandidates = calcBoundaryCandidates(fieldBoundary.boundarySpots);

    InImage convexBoundary = fieldBoundary.convexBoundary;
    findBestBoundary(fieldBoundary.convexBoundaryCandidates, fieldBoundary.boundarySpots, convexBoundary);
    fieldBoundary.convexBoundary.clear();
    for (const auto& el : convexBoundary) {
      fieldBoundary.convexBoundary.push_back(el);
    }
  ...
  if (theCameraInfo.camera == CameraInfo::Camera::upper) {
    ...
    std::sort(fieldBoundary.boundaryInImage.begin(),
              fieldBoundary.boundaryInImage.end(),
              [](Vector2<int> a, Vector2<int> b) { return a.x < b.x || (a.x == b.x && a.y < b.y); });
    InImage boundaryInImage = getUpperConvexHull(fieldBoundary.boundaryInImage);
    fieldBoundary.boundaryInImage.clear();
    for (const auto& point : boundaryInImage) {
      fieldBoundary.boundaryInImage.push_back(point);
    }
  }
...
void FieldBoundaryProvider::findBestBoundary(const vector<InImage>& boundaryCandidates,
                                             const InImage& boundarySpots,
                                             InImage& boundary) const {
  int spotsOnLine;
  int spotsNearLine;
  int score;
  int y;
...
inline bool FieldBoundaryProvider::isLeftOf(const Vector2<int>& a, const Vector2<int>& b, const Vector2<int>& c) const {
