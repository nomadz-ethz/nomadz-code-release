...
void GoalPerceptor::update(GoalPercept& percept) {
  ...
  // check if the horizon is visible in the image, Alex Locher Iran Open 2015
  if ((int)theImageCoordinateSystem.origin.y < 1) {
    return;
  }
  ...
  LINE(
    "module:GoalPerceptor:Spots", 1, scanHeight, theImage.width - 1, scanHeight, 1, Drawings::ps_dash, ColorClasses::orange);
...
void GoalPerceptor::update(GoalSymbols& symbols) {}
...
void GoalPerceptor::verticalColorScanUp() {
  for (std::list<Spot>::iterator i = spots.begin(), end = spots.end(); i != end; ++i) {
    ...
    while (mid.y != lastMid.y) {
      ...
      if (crossbarChecking) {
        if (lastLeft - left > 2 * initialWidth && abs(right - lastRight) < initialWidth) {
          ...
          goto end_vcs_up;
        } else if (right - lastRight > 2 * initialWidth && abs(left - lastLeft) < initialWidth) {
          i->top = Vector2<int>(mid.x, topY);
  ...
  for (std::list<Spot>::iterator i = spots.begin(), end = spots.end(); i != end; ++i) {
    if (i->base.y > theImage.height - 5) {
      ...
      if (theCameraInfo.camera == CameraInfo::upper) {
        ...
        for (unsigned e = 0; e < lastPosts.size(); e++) {
          ...
          if (projection.x < i->end && projection.x > i->start) {
            ...
            Geometry::Line l1 =
              Geometry::Line(Vector2<>(i->start, height), (Vector2<>(i->end, height) - Vector2<>(i->start, height)));
            Geometry::Line l2 = Geometry::Line(Vector2<>(projection.x, projection.y),
                                               Vector2<>(i->base.x - projection.y, i->base.y - projection.y));
      ...
      if (matching) {
        ...
        float distance =
          Geometry::getDistanceBySize(theCameraInfo, theFieldDimensions.goalPostRadius * 2.f, (float)i->width);
        Vector2<> angle;
...
void GoalPerceptor::validate() {
  ...
  float value;
  float expectedValue;
  float maxDistance = (Vector2<>(theFieldDimensions.xPosOpponentFieldBorder, theFieldDimensions.yPosLeftFieldBorder) -
                       Vector2<>(theFieldDimensions.xPosOwnFieldBorder, theFieldDimensions.yPosRightFieldBorder))
                        .abs() *
                      1.3f;
  ...
  for (std::list<Spot>::iterator i = spots.begin(); i != spots.end(); i++) {
    ...
    i->position.abs() > maxDistance || i->position.abs() < theFieldDimensions.goalPostRadius ? distanceEvaluation = 0
                                                                                             : distanceEvaluation = 1;

    // min height
    height < Geometry::getSizeByDistance(theCameraInfo, theFieldDimensions.goalHeight, maxDistance) ? minimalHeight = 0
                                                                                                    : minimalHeight = 1;

    // if goalpost base is above the fieldborde
    value = Geometry::findBoundaryY(i->base.x, theFieldBoundary, theCameraInfo);
    i->base.y > value - (value / 20) ? belowFieldBorder = 1 : belowFieldBorder = 0;

    // if the LEFT <-> RIGHT is not set and the top is below the image border, this is
    // probably not a post
    cornerCheck = (i->leftRight == GoalPost::IS_UNKNOWN && i->top.y > 2) ? 0 : 1;
    ...
    if (spots.size() > 1) {
      for (std::list<Spot>::iterator j = spots.begin(); j != spots.end(); j++) {
        if (i != j) {
          ...
          if ((int)(100 - (std::abs(expectedValue - value) / expectedValue) * 50) > 75) {
            distanceToEachOther = 75;
          }

          // matching crossbars
          if ((i->leftRight == GoalPost::IS_LEFT && j->leftRight == GoalPost::IS_RIGHT) ||
              (i->leftRight == GoalPost::IS_RIGHT && j->leftRight == GoalPost::IS_LEFT)) {
            matchingCrossbars = 75;
          }
        }
      }
    }
    ...
    i->validity =
      ((relationWidthToHeight + expectedWidth + expectedHeight + distanceToEachOther + matchingCrossbars) / 5.0f) *
      distanceEvaluation * minimalHeight * belowFieldBorder * constantWidth * cornerCheck;
    ...
    if (i->validity < high && i->validity > low) {
      DRAWTEXT("module:GoalPerceptor:Validation",
               i->mid.x,
               i->mid.y - 55,
               10,
               ColorClasses::black,
      ...
      DRAWTEXT("module:GoalPerceptor:Validation",
               ...
               10,
               ColorClasses::black,
               "belowFieldBorder: " << belowFieldBorder);
      DRAWTEXT("module:GoalPerceptor:Validation",
               i->mid.x,
               i->mid.y - 22,
               10,
      ...
      DRAWTEXT("module:GoalPerceptor:Validation",
               ...
               10,
               ColorClasses::black,
               "relationWidthToHeight: " << relationWidthToHeight);
      DRAWTEXT(
        "module:GoalPerceptor:Validation", i->mid.x, i->mid.y, 10, ColorClasses::black, "expectedWidth: " << expectedWidth);
      DRAWTEXT("module:GoalPerceptor:Validation",
               i->mid.x,
               i->mid.y + 11,
               10,
      ...
      DRAWTEXT("module:GoalPerceptor:Validation",
               ...
               10,
               ColorClasses::black,
               "distanceToEachOther: " << distanceToEachOther);
      DRAWTEXT("module:GoalPerceptor:Validation",
               i->mid.x,
               i->mid.y + 33,
               10,
               ColorClasses::black,
               "matchingCrossbars: " << matchingCrossbars);
      DRAWTEXT(
        "module:GoalPerceptor:Validation", i->mid.x, i->mid.y + 44, 10, ColorClasses::black, "validity: " << i->validity);
    }
  }
  spots.sort();
}
...
inline bool GoalPerceptor::isWhite(const int& x, const int& y) {
  return (theColorReference.isWhite(&theImage[y][x]) && !theColorReference.isRed(&theImage[y][x]) &&
          !theColorReference.isBlue(&theImage[y][x]));
}
