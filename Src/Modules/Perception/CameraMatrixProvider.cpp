...
void CameraMatrixProvider::update(CameraMatrix& cameraMatrix) {
  ...
  cameraMatrix.isValid =
    theTorsoMatrix.isValid && theMotionInfo.isMotionStable && theFallDownState.state == FallDownState::upright &&
    theFrameInfo.getTimeSince(theFilteredJointData.timeStamp) < 500 && theRobotInfo.penalty == PENALTY_NONE;
  ...
  DECLARE_DEBUG_DRAWING3D("module:CameraMatrixProvider:cameraMatrix", "field", {
    ...
    Vector3<> tl = robotPose + robotPoseCorrection * cameraMatrix * Vector3<>(len, ml.y, mt.z);
    Vector3<> tr = robotPose + robotPoseCorrection * cameraMatrix * Vector3<>(len, -ml.y, mt.z);
    Vector3<> bl = robotPose + robotPoseCorrection * cameraMatrix * Vector3<>(len, ml.y, -mt.z);
    Vector3<> br = robotPose + robotPoseCorrection * cameraMatrix * Vector3<>(len, -ml.y, -mt.z);
    const Vector3<>& ce = robotPose + robotPoseCorrection * cameraMatrix.translation;
...
bool CameraMatrixProvider::intersectLineWithCullPlane(const Vector3<>& lineBase,
...
void CameraMatrixProvider::drawFieldLines(const CameraMatrix& cameraMatrix) const {
  ...
  for (const auto& line : theFieldDimensions.getAllFieldLines()) {
    ...
    relativeLine.conc(line.corner());
    const Vector2<> start0(Pose2D(relativeLine).translate(0, -halfFieldLinesWidth).translation);
    const Vector2<> end1(Pose2D(relativeLine).translate(line.length(), halfFieldLinesWidth).translation);
    ...
    Vector2<> start1(Pose2D(relativeLine).translate(0, halfFieldLinesWidth).translation);
    Vector2<> end0(Pose2D(relativeLine).translate(line.length(), -halfFieldLinesWidth).translation);
    ...
    LINE("module:CameraMatrixProvider:calibrationHelper",
         start0I.x,
         start0I.y,
         end0I.x,
         end0I.y,
...
void CameraMatrixProvider::drawRobotParts() {
  for (unsigned int i = 0; i < p.thighIndex.size(); i += 3) {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      f[k].translate(
        p.thighPoints[tIndex[k]] * 1000, p.thighPoints[tIndex[k] + 1] * 1000, p.thighPoints[tIndex[k] + 2] * 1000); // right
      f[k + 3].translate(
        p.thighPoints[tIndex[k]] * 1000, -p.thighPoints[tIndex[k] + 1] * 1000, p.thighPoints[tIndex[k] + 2] * 1000); // left
    ...
    for (int k = 0; k < 3; ++k) {
      if (k < 2) {
        if (projected2Image[k].x > 0 && projected2Image[k + 1].x > 0) { // right
          LINE("module:CameraMatrixProvider:robotParts",
               projected2Image[k].x,
               projected2Image[k].y,
               projected2Image[k + 1].x,
               projected2Image[k + 1].y,
               0,
               Drawings::ps_solid,
               ColorClasses::red);
        }
        if (projected2Image[k + 3].x > 0 && projected2Image[k + 4].x > 0) { // left
          LINE("module:CameraMatrixProvider:robotParts",
               projected2Image[k + 3].x,
               projected2Image[k + 3].y,
               projected2Image[k + 4].x,
               projected2Image[k + 4].y,
               0,
               Drawings::ps_solid,
               ColorClasses::red);
        }
        ...
        if (projected2Image[k].x > 0 && projected2Image[k - 2].x > 0) { // right
          LINE("module:CameraMatrixProvider:robotParts",
               projected2Image[k].x,
               projected2Image[k].y,
               projected2Image[k - 2].x,
               projected2Image[k - 2].y,
               0,
               Drawings::ps_solid,
               ColorClasses::red);
        }
  ...
  for (unsigned int i = 0; i < p.shineIndex.size(); i += 3) {
    ...
    for (int k = 0; k < 3; ++k) {
      // add triangle point to ankle pose and convert from meter to mm
      f[k].translate(
        p.shinePoints[tIndex[k]] * 1000, p.shinePoints[tIndex[k] + 1] * 1000, p.shinePoints[tIndex[k] + 2] * 1000); // right
      f[k + 3].translate(
        p.shinePoints[tIndex[k]] * 1000, -p.shinePoints[tIndex[k] + 1] * 1000, p.shinePoints[tIndex[k] + 2] * 1000); // left
  ...
  for (unsigned int i = 0; i < p.footIndex.size(); i += 3) {
    ...
    Pose3D f[9] = {theRobotModel.limbs[MassCalibration::footRight],
                   ...
                   theRobotModel.limbs[MassCalibration::footRight],
                   theRobotModel.limbs[MassCalibration::footRight]}; // draw mirrored right foot in order to compare where
                                                                     // the left foot should be given a stand position and a
                                                                     // right foot position
    // triangle point indices
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      // add triangle point to ankle pose and convert from meter to mm
      f[k].translate(
        p.footPoints[tIndex[k]] * 1000, p.footPoints[tIndex[k] + 1] * 1000, p.footPoints[tIndex[k] + 2] * 1000); // right
      f[k + 3].translate(
        p.footPoints[tIndex[k]] * 1000, -p.footPoints[tIndex[k] + 1] * 1000, p.footPoints[tIndex[k] + 2] * 1000); // left
      f[k + 6].translate(p.footPoints[tIndex[k]] * 1000,
                         -p.footPoints[tIndex[k] + 1] * 1000,
                         p.footPoints[tIndex[k] + 2] * 1000); // mirrored right
    ...
    for (int k = 0; k < 3; ++k) {
      if (k < 2) {
        ...
        if (projected2Image[k + 6].x > 0 && projected2Image[k + 7].x > 0) { // mirrored right
          LINE("module:CameraMatrixProvider:robotParts",
               projected2Image[k + 6].x,
               projected2Image[k + 6].y,
               projected2Image[k + 7].x,
               projected2Image[k + 7].y,
               0,
               Drawings::ps_solid,
               ColorClasses::yellow);
        }
