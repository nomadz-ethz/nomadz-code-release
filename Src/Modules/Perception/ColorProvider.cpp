...
bool ColorProvider::compareAndSet(Range<float>& toSet, const float min, const float max) {
  return compareAndSet(toSet.min, min) || compareAndSet(toSet.max, max);
}
...
bool ColorProvider::compareAndSet(ColorReference::HSVColorDefinition& toSet,
                                  ...
                                  const float minV,
                                  const float maxV) {
  return compareAndSet(toSet.hue, minH, maxH) || compareAndSet(toSet.saturation, minS, maxS) ||
...
void ColorProvider::update(ColorReference& colorReference) {
  ...
  if (setupColorTable || calculateGreen(colorReference) || calculateWhite(colorReference) ||
      calculateYellow(colorReference) || calculateOrange(colorReference) || calculateRed(colorReference) ||
      calculateBlue(colorReference) || calculateBlack(colorReference)) {
  ...
  DEBUG_RESPONSE_ONCE("representation:ColorReference", { OUTPUT(idColorReference, bin, colorReference); });
}

bool ColorProvider::calculateGreen(ColorReference& colorReference) {
  return compareAndSet(colorReference.thresholdGreen, minHGreen, maxHGreen, minSGreen, maxSGreen, minVGreen, maxVGreen);
}
...
bool ColorProvider::calculateYellow(ColorReference& colorReference) {
  return compareAndSet(
    colorReference.thresholdYellow, minHYellow, maxHYellow, minSYellow, maxSYellow, minVYellow, maxVYellow);
}

bool ColorProvider::calculateOrange(ColorReference& colorReference) {
  return compareAndSet(
    colorReference.thresholdOrange, minHOrange, maxHOrange, minSOrange, maxSOrange, minVOrange, maxVOrange);
}

bool ColorProvider::calculateRed(ColorReference& colorReference) {
  return compareAndSet(colorReference.thresholdRed, minHRed, maxHRed, minSRed, maxSRed, minVRed, maxVRed);
}

bool ColorProvider::calculateBlue(ColorReference& colorReference) {
  return compareAndSet(colorReference.thresholdBlue, minHBlue, maxHBlue, minSBlue, maxSBlue, minVBlue, maxVBlue);
}
