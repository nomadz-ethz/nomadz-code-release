...
std::vector<BallSpot> TfLiteBallPerceptor::filterBallSpots(const std::vector<BallSpot>& ballSpots) {
  ...
  for (int i = 0; i < ballSpots.size(); ++i) {
    ...
    if (useBallOutsideFieldCheck && (theRobotPose.validity > 0.3) && ballOutsideOfField(relFieldPos)) {
      continue;
    }
...
void TfLiteBallPerceptor::update(BallPercept& ballPercept) {
  ...
  std::random_shuffle(ballSpots.begin(), ballSpots.end());
  std::sort(
    ballSpots.begin(), ballSpots.end(), [](BallSpot& spot1, BallSpot& spot2) { return spot1.radius > spot2.radius; });
