...
void ShapePerceptor::findBalls(const std::vector<int>& edgeList,
  ...
  int nIter = std::min<int>(edgeList.size(), circMaxIter);
  int scaledMinVotes = circMinVotes * (int)((float)nIter / edgeList.size());

  const unsigned int yTop = yStart + 1;
  const unsigned int yBottom = h - frame - 1;
  for (unsigned int y = yBottom - 1; y >= yTop; --y) {
    for (unsigned int x = frame + 1; x < w - frame; ++x) {
      if (circleHough[y * w + x] >= scaledMinVotes && circleHough[y * w + x] > circleHough[(y - 2) * w + (x - 2)] &&
    ...
    COMPLEX_DRAWING("module:ShapePerceptor:balls", {
      for (unsigned char i = 0; i < ballList.size(); ++i) {
        ...
        CIRCLE("module:ShapePerceptor:balls",
               ...
               ColorRGBA());
        DRAWTEXT(
          "module:ShapePerceptor:balls", x * drawW / w, y * drawH / h, 10, ColorRGBA(0, 255, 0), circleHough[ballList[i]]);
      }
    });
  }
}
