...
public:
  ...
  public:
     ...
     */
    inline unsigned char coverage(unsigned time) const { return coverage(time, this->lastseen); }
  ...
  public:
     ...
     */
    void drawOnImage(const CameraMatrix& cameraMatrix,
  ...
  private:
     ...
     */
    Vector2<> projectOnImage(const Vector2<>& absPosOnField,
...
private:
  ...
  int lastWorstNoTurnHalfRangeIdx;   /**< Index of the worst covered visible cell closer than half the range from the last
                                        frame. */
  ...
  const float
    throwInLineDistanceGL;        /**< Distance between the throw-in line and the closest field border groundline in mm. */
  ...
  BallTime
    lastValidInsideLyingBall;    /**< Position of the last valid global ball which is inside the field and does not move. */
  ...
  /**
   * Computes the shadows of all robots in the playerModel.
   */
  void computeShadows(std::vector<RobotShadow>& shadows, const RobotPose& robotPose, const PlayerModel& playerModel);
