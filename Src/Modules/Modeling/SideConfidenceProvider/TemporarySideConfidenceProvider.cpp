/**
 * @file TemporarySideConfidenceProvider.cpp
 *
...
TemporarySideConfidenceProvider::TemporarySideConfidenceProvider()
    : lost(false), lastTimeOutsideOwnPenaltyArea(0), timeOfLastFall(0), lastTimeWithoutArmContact(0),
...
void TemporarySideConfidenceProvider::update(SideConfidence& sideConfidence) {
  ...
  if (theFallDownState.state != theFallDownState.upright && theFallDownState.state != theFallDownState.undefined &&
...
void TemporarySideConfidenceProvider::updateSideConfidenceFromOwn(SideConfidence& sideConfidence) {
  ...
  if (theFrameInfo.getTimeSince(timeOfLastFall) == 0) // fallen now
  {
    ...
    if (distToFieldCenter < maxDistanceToFieldCenterForFallDownConsideration) {
      ...
      if (distToFieldCenter > rCircle) {
        currentFallDownModificator *=
          1 - ((distToFieldCenter - rCircle) / (maxDistanceToFieldCenterForFallDownConsideration - rCircle));
      }
  ...
  if (theFrameInfo.getTimeSince(lastTimeWithoutArmContact) > minContinuousArmContact) {
    ...
    if (distToFieldCenter < maxDistanceToFieldCenterForArmConsideration) {
      ...
      const float rCircle = theFieldDimensions.centerCircleRadius;
      if (distToFieldCenter > rCircle) {
        currentArmContactModificator *=
          1 - ((distToFieldCenter - rCircle) / (maxDistanceToFieldCenterForArmConsideration - rCircle));
      }
...
TemporarySideConfidenceProvider::BallModelSideConfidence TemporarySideConfidenceProvider::computeCurrentBallConfidence() {
  ...
  float originalWeighting = computeAngleWeighting(
    angleObserved, theCombinedWorldModel.ballStateOthers.position, theRobotPose, standardDeviationBallAngle);
  originalWeighting *= computeDistanceWeighting(distanceAsAngleObserved,
                                                theCombinedWorldModel.ballStateOthers.position,
                                                theRobotPose,
                                                camZ,
                                                standardDeviationBallDistance);

  // Weighting for mirrored pose
  const Pose2D mirroredPose = Pose2D(pi) + (Pose2D)(theRobotPose);
  float mirroredWeighting = computeAngleWeighting(
    angleObserved, theCombinedWorldModel.ballStateOthers.position, mirroredPose, standardDeviationBallAngle);
  mirroredWeighting *= computeDistanceWeighting(distanceAsAngleObserved,
                                                theCombinedWorldModel.ballStateOthers.position,
                                                mirroredPose,
                                                camZ,
                                                standardDeviationBallDistance);
  ...
  if ((mirroredWeighting > weightingFactor * originalWeighting) ||
      (originalWeighting > weightingFactor * mirroredWeighting)) {
    return mirroredWeighting > originalWeighting ? MIRROR : OK;
  }
  return UNKNOWN;
}

float TemporarySideConfidenceProvider::computeAngleWeighting(float measuredAngle,
...
float TemporarySideConfidenceProvider::computeDistanceWeighting(float measuredDistanceAsAngle,
                                                                const Vector2<>& modelPosition,
                                                                const Pose2D& robotPose,
                                                                float cameraZ,
                                                                float standardDeviation) const {
