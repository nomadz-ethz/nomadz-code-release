...
LOADS_PARAMETER(float,
                weightingFactor) /**< Multiplier for defining the minimum difference between normal and mirrored pose */
...
LOADS_PARAMETER(float,
                relativeBallDropin) /**< The value how far the ball is dorpped in behind the robot after kicking out */
...
LOADS_PARAMETER(float,
                minWeighting); /**< The minimum weighting a an original or mirrored ball needs to have to be considered */
END_MODULE
...
private:
  ...
  ENUM(BallModelSideConfidence,
       OK,
       MIRROR,
       UNKNOWN); /**< Discrete states of confidence resulting from comparison of ball models (own vs. others) */
  ...
  unsigned lastTimeOutsideOwnPenaltyArea; /**< The last time the robot's position was reported outside the own penalty area
                                             (in ms) */
  unsigned timeOfLastFall;                /**< Timestamp to see if the robot has fallen down */
  unsigned lastTimeWithoutArmContact;
  Vector2<>
    lastBallObservation; /**< Position (relative to the robot) of the last estimated ball that has actually been observed */
