...
DEFINES_PARAMETER(Vector2<>,
                  robotRotationDeviation,
                  Vector2<>(0.02f, 0.08f))                             /**< Deviation of the rotation of the robot's torso */
DEFINES_PARAMETER(Pose2D, odometryDeviation, Pose2D(0.5f, 0.5f, 0.5f)) /**< The percentage inaccuracy of the odometry */
DEFINES_PARAMETER(bool, handleCollisions, true)    /**< Whether to model ball bouncing off our feet and update samples
                                                      accordingly (useful if estimates/detection very precise) */
DEFINES_PARAMETER(float, initialStateWeight, 0.1f) /**< The weight of newly created states (between >0 and <1) */
DEFINES_PARAMETER(Vector2<>, footOffset, Vector2<>(65.f, 0.f))   /**< Offset from foot ankle point to a "center" of the foot
                                                                    used for approximating the foot shape with a circle */
DEFINES_PARAMETER(float, footRadius, 40.f)                       /**< The radius of the approximated foot shape */
...
DEFINES_PARAMETER(unsigned, ballNotSeenTimeout, 300)
DEFINES_PARAMETER(unsigned, ballNotUpdatedTimeout, 2000)
DEFINES_PARAMETER(float, minTimeUpdateSeenFraction, 0.1)
END_MODULE
...
private:
  ...
  void createNewStates(const Vector2f& ballPercept,
  ...
  bool getSmallestLineWithCircleIntersectionFactor(const Vector2<>& lineBase,
                                                   const Vector2<>& lineDir,
                                                   const Vector2<>& circleBase,
                                                   float circleRadius,
                                                   float& factor) const;
  bool getLineWithLineIntersectionFactors(const Vector2<>& lineBase1,
