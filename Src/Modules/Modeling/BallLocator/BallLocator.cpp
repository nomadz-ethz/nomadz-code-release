...
void BallLocator::update(BallModel& ballModel) {
  ...
  leftFoot.conc(theRobotModel.limbs[MassCalibration::footLeft])
    .translate(footOffset.x, footOffset.y, -theRobotDimensions.heightLeg5Joint);
  rightFoot.conc(theRobotModel.limbs[MassCalibration::footRight])
    .translate(footOffset.x, -footOffset.y, -theRobotDimensions.heightLeg5Joint);
  ...
  COMPLEX_DRAWING("module:BallLocator:Image", {
    ...
    if (Geometry::calculatePointInImage(
          Vector3<>(leftFootCenter.x, leftFootCenter.y, 0), theCameraMatrix, theCameraInfo, leftCenterImage) &&
        Geometry::calculatePointInImage(
          Vector3<>(leftFootCenter.x + footRadius, leftFootCenter.y, 0), theCameraMatrix, theCameraInfo, leftTopImage)) {
      CIRCLE("module:BallLocator:Image",
             leftCenterImage.x,
             leftCenterImage.y,
             (leftCenterImage - leftTopImage).abs(),
             0,
             Drawings::ps_solid,
             ColorRGBA(0, 0, 0),
             Drawings::bs_null,
    ...
    if (Geometry::calculatePointInImage(
          Vector3<>(rightFootCenter.x, rightFootCenter.y, 0), theCameraMatrix, theCameraInfo, rightCenterImage) &&
        Geometry::calculatePointInImage(
          Vector3<>(rightFootCenter.x + footRadius, rightFootCenter.y, 0), theCameraMatrix, theCameraInfo, rightTopImage)) {
      CIRCLE("module:BallLocator:Image",
             rightCenterImage.x,
             rightCenterImage.y,
             (rightCenterImage - rightTopImage).abs(),
             0,
             Drawings::ps_solid,
             ColorRGBA(0, 0, 0),
             Drawings::bs_null,
  ...
  if (ballWasSeenInThisFrame) {
    // reset filters..
    if (theFrameInfo.getTimeSince(theBallModel.timeWhenDisappeared) >= 1000) {
      // FIXME: might not be needed
      reset();
    }

    // calculate variance of the percept
    ballPerceptCov = getCovOfPixelInWorld(theImageCoordinateSystem.toCorrected(theBallPercept.positionInImage),
  ...
  COMPLEX_DRAWING("module:BallLocator:Field", {
    for (State *state = states, *end = states + stateCount; state < end; ++state) {
      if (state->type == State::moving) {
        DOT("module:BallLocator:Field",
            state->movingX[0],
            state->movingX[1],
            ColorRGBA(0xff, 0xaa, 0xaa),
            ColorRGBA(0xff, 0xaa, 0xaa));
...
void BallLocator::motionUpdate(BallModel& ballModel) {
  ...
  Vector2f odometryTranslationDeviation(odometryOffset.translation.x * odometryDeviation.translation.x,
                                        odometryOffset.translation.y * odometryDeviation.translation.y);
  ...
  Matrix2x2f fixedOdometryRotationDeviationRotation(Vector2f(odometryDeviationCos, -odometryDeviationSin),
                                                    Vector2f(odometryDeviationSin, odometryDeviationCos));
  ...
  Matrix4x4f movingOdometryRotation(Vector4f(odometryCos, -odometrySin, 0, 0),
                                    Vector4f(odometrySin, odometryCos, 0, 0),
                                    Vector4f(0, 0, odometryCos, -odometrySin),
                                    Vector4f(0, 0, odometrySin, odometryCos)); // second part of "a"
  Matrix4x4f movingOdometryRotationDeviationRotation(Vector4f(odometryDeviationCos, -odometryDeviationSin, 0, 0),
                                                     Vector4f(odometryDeviationSin, odometryDeviationCos, 0, 0),
                                                     Vector4f(0, 0, odometryDeviationCos, -odometryDeviationSin),
                                                     Vector4f(0, 0, odometryDeviationSin, odometryDeviationCos));
  Vector4f movingOdometryTranslation(-odometryOffset.translation.x, -odometryOffset.translation.y, 0, 0); // u
  Matrix4x4f movingMotionMatrix(Vector4f(1, 0, 0, 0),
                                Vector4f(0, 1, 0, 0),
                                Vector4f(deltaTime, 0, 1, 0),
                                Vector4f(0, deltaTime, 0, 1)); // first part of "a"
  Matrix4x4f movingA = movingOdometryRotation * movingMotionMatrix;
  Matrix4x4f movingATransponsed = movingA.transpose();

  // prepare process noise
  Vector4f processCov(
    sqr(processDeviation[0]), sqr(processDeviation[1]), sqr(processDeviation[2]), sqr(processDeviation[3]));
  ...
  if (hasLastBallPercept) {
    ...
    Vector2f odometryRotationDeviationTranslation =
      fixedOdometryRotationDeviationRotation * lastBallPercept - lastBallPercept;
  ...
  if (!ballWasSeenInThisFrame) {
    Vector2f lastPercept =
      fixedOdometryRotation * Vector2f(ballModel.lastPerception.x, ballModel.lastPerception.y) + fixedOdometryTranslation;
...
void BallLocator::handleCollisionWithFeet(const Vector2<>& leftFootCenter, const Vector2<>& rightFootCenter) {
  ...
  leftCollision = getSmallestLineWithCircleIntersectionFactor(
    leftAssumedLastBallPosition, leftAssumedBallOffset, lastLeftFootCenter, assumedRadius, leftCollisionFactor);
  ...
  rightCollision = getSmallestLineWithCircleIntersectionFactor(
    rightAssumedLastBallPosition, rightAssumedBallOffset, lastRightFootCenter, assumedRadius, rightCollisionFactor);
  ...
  if (leftCollision || rightCollision) {
    ...
    Vector2<> collisionPoint = leftCollision
                                 ? (leftAssumedLastBallPosition + leftAssumedBallOffset * leftCollisionFactor)
                                 : (rightAssumedLastBallPosition + rightAssumedBallOffset * rightCollisionFactor);
    ...
    float passedMomentum =
      footMomentum * std::cos(std::abs(normalize(collisionAngle - (footCenter - lastFootCenter).angle())));
    ...
    if (getLineWithLineIntersectionFactors(leftFootCenter,
                                           rightFootCenter - leftFootCenter,
                                           lastBallPosition,
  ...
  if (leftCollision || rightCollision || centerCollision || ballShift != Vector2<>()) {
    if (ballVelocity != Vector2<>()) {
      ...
      if (bestState->type != State::moving) {
        ...
        bestState->movingCov =
          Matrix4x4f(Vector4f(cov[0][1], cov[0][1], 0, 0), Vector4f(cov[1][0], cov[1][1], 0, 0), Vector4f(), Vector4f());
      }
  ...
  for (State *state = states, *end = states + stateCount; state < end; ++state) {
    if (state->type == State::moving) {
      ...
      if (!(state->gain >= 0.f && state->gain <= 10.f)) {
        ...
        OUTPUT_ERROR("measurementCov c0x=" << measurementCov.c[0].x << " c0y=" << measurementCov.c[0].y
                                           << " c1x=" << measurementCov.c[1].x << " c1y=" << measurementCov.c[1].y);
        OUTPUT_ERROR("x x.x=" << x[0] << " x.y=" << x[1]);
      }
      ASSERT(state->gain >= 0.f && state->gain <= 10.f);
      state->weight *= state->gain;
      state->height =
        state->weight * getProbabilityAtMean(Matrix2x2f(Vector2f(cov[0][0], cov[0][1]), Vector2f(cov[1][0], cov[1][1])));
      state->age++;
...
void BallLocator::createNewStates(const Vector2f& ballPercept,
                                  const Matrix2x2f& ballPerceptCov,
                                  State* worstStationaryState,
                                  State* worstMovingState) {
  ...
  if (hasLastBallPercept) {
    ...
    newState->movingX = Vector4f(ballPercept.x,
                                 ballPercept.y,
                                 (ballPercept.x - lastBallPercept.x) * timeScale,
                                 (ballPercept.y - lastBallPercept.y) * timeScale); // z
    newState->movingCov = Matrix4x4f(Vector4f(ballPerceptCov[0][0], ballPerceptCov[0][1], 0, 0),
                                     Vector4f(ballPerceptCov[1][0], ballPerceptCov[1][1], 0, 0),
                                     Vector4f(0,
                                              0,
                                              (ballPerceptCov[0][0] + lastBallPerceptCov[0][0]) * timeScale,
                                              (ballPerceptCov[0][1] + lastBallPerceptCov[0][1]) * timeScale),
                                     Vector4f(0,
                                              0,
                                              (ballPerceptCov[1][0] + lastBallPerceptCov[1][0]) * timeScale,
                                              (ballPerceptCov[1][1] + lastBallPerceptCov[1][1]) * timeScale));
  }
}
...
void BallLocator::generateModel(BallModel& ballModel) {
  ...
  if (ballWasSeenInThisFrame) {
    if (ballModel.seenFraction >= minTimeUpdateSeenFraction) {
      ballModel.timeWhenLastSeen = theFrameInfo.time;
    }
    ballModel.timeWhenLastDetected = theFrameInfo.time;
    ballModel.lastPerception = theBallPercept.relativePositionOnField;
  }
  ballModel.timeSinceLastSeen = theFrameInfo.getTimeSince(ballModel.timeWhenLastSeen);
  ...
  // update timeWhenDisappeared
  {
    ...
    bool ballShouldBeVisible = false;
    {
      const Vector3<> camera =
        theCameraMatrix.invert() *
        Vector3<>(theBallModel.estimate.position.x, theBallModel.estimate.position.y, theFieldDimensions.ballRadius);
      if (camera.x > 1) {
        ...
        if ((image - Vector2<>(theCameraInfo.opticalCenter.x, theCameraInfo.opticalCenter.y)).squareAbs() <
            sqr(theCameraInfo.height * 0.42f)) {
    ...
    if (theFrameInfo.getTimeSince(theBallModel.timeWhenLastSeen) >= (int)ballNotUpdatedTimeout) {
      ...
      ballModel.lost = false;
    }
  }

  // update seen fraction
  if (theCameraInfo.camera == CameraInfo::lower) {
    ballModel.seenBuffer <<= 1;
  }
  if (ballWasSeenInThisFrame) {
    ballModel.seenBuffer.set(0, true);
  }
  ballModel.seenFraction = ballModel.seenBuffer.count() / (float)ballModel.seenBuffer.size();
}

Matrix2x2f BallLocator::getCovOfPixelInWorld(const Vector2<>& correctedPointInImage, float pointZInWorld) const {
  const Vector3<> unscaledVectorToPoint(theCameraInfo.focalLength,
                                        theCameraInfo.opticalCenter.x - correctedPointInImage.x,
                                        theCameraInfo.opticalCenter.y - correctedPointInImage.y);
  ...
  Matrix2x2f cov(
    Vector2f(sqr(h / std::tan((distance == 0.f ? pi_2 : std::atan(h / distance)) - robotRotationDeviation.x) - distance),
             0.f),
...
bool BallLocator::getSmallestLineWithCircleIntersectionFactor(const Vector2<>& lineBase,
                                                              const Vector2<>& lineDir,
                                                              const Vector2<>& circleBase,
                                                              float circleRadius,
                                                              float& factor) const {
...
bool BallLocator::getLineWithLineIntersectionFactors(const Vector2<>& lineBase1,
