...
void SelfLocator::handleGameStateChanges(const Pose2D& propagatedRobotPose) {
  ...
  // If a penalty is over, reset samples to reenter positions
    ...
    if (lastPenalty != PENALTY_SPL_ILLEGAL_MOTION_IN_SET) {
      for (int i = 0; i < samples->size(); ++i) {
        Pose2D pose = sampleGenerator.getTemplateAtReenterPosition(i, samples->size());
        samples->at(i).init(pose, defaultPoseDeviation);
      }
      sampleSetHasBeenResetted = true;
...
void SelfLocator::initWalkInPositions() {
  ...
  if (theGameInfo.gamePhase != GAME_PHASE_PENALTYSHOOT) {
    for (int i = 0; i < numberOfSamples; ++i) {
      samples->at(i).init(initPositions[i], defaultPoseDeviation);
    }
  } else {
    for (int i = 0; i < numberOfSamples; ++i) {
...
void SelfLocator::computeInitPosition() {
  initPositions.clear();
  const int numSampleRightSide = numberOfSamples / 2;
  const int numSamplesLeftSide = numberOfSamples - numSampleRightSide;

  const float right_side_pose_step = std::abs(theFieldDimensions.xPosOwnGroundline / numSampleRightSide);
  for (int i = 0; i < numSampleRightSide; ++i) {
    float initPoseX = theFieldDimensions.xPosOwnGroundline + i * right_side_pose_step;
    float initPoseY = theFieldDimensions.yPosRightSideline;
    float initRot = fromDegrees(90.f);
    initPositions.push_back(Pose2D(initRot, initPoseX, initPoseY));
  }

  const float left_side_pose_step = std::abs(theFieldDimensions.xPosOwnGroundline / numSamplesLeftSide);
  for (int i = 0; i < numSamplesLeftSide; ++i) {
    float initPoseX = theFieldDimensions.xPosOwnGroundline + i * left_side_pose_step;
    float initPoseY = theFieldDimensions.yPosLeftSideline;
    float initRot = fromDegrees(-90.f);
    initPositions.push_back(Pose2D(initRot, initPoseX, initPoseY));
  }
}
...
void SelfLocator::computeModel(RobotPose& robotPose) const {
  ...
  robotPose.valid = robotPose.timeSinceLastValid < poseValidTime;
  robotPose.lost = robotPose.timeSinceLastValid > poseLostTime;
