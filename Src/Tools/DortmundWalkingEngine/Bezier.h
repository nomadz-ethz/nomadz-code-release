...
inline void FourPointBezier(Point* point, const Point* polygon, float t) {
  point->x = (1 - t) * (1 - t) * (1 - t) * polygon[0].x + 3 * t * (1 - t) * (1 - t) * polygon[1].x +
             3 * t * t * (1 - t) * polygon[2].x + t * t * t * polygon[3].x;
  point->y = (1 - t) * (1 - t) * (1 - t) * polygon[0].y + 3 * t * (1 - t) * (1 - t) * polygon[1].y +
             3 * t * t * (1 - t) * polygon[2].y + t * t * t * polygon[3].y;
  point->z = (1 - t) * (1 - t) * (1 - t) * polygon[0].z + 3 * t * (1 - t) * (1 - t) * polygon[1].z +
             3 * t * t * (1 - t) * polygon[2].z + t * t * t * polygon[3].z;
}
...
inline float FourPointBezier1D(const float* polygon, float t) {
  return (1 - t) * (1 - t) * (1 - t) * polygon[0] + 3 * t * (1 - t) * (1 - t) * polygon[1] +
         3 * t * t * (1 - t) * polygon[2] + t * t * t * polygon[3];
}
