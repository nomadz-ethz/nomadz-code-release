...
public:
   ...
   */
  void
  build_half_hull(std::vector<std::pair<float, float>> input, std::vector<std::pair<float, float>>& output, int factor) {
    //
   ...
   */
  static float direction(std::pair<float, float> p0, std::pair<float, float> p1, std::pair<float, float> p2) {
    return ((p0.first - p1.first) * (p2.second - p1.second)) - ((p2.first - p1.first) * (p0.second - p1.second));
  }

  void addPoint(float x, float y) { raw_points.push_back(std::make_pair(x, y)); }
