...
#define RESET_INTERPOLATION_VAR(varname) interpolate##varname(0, 0, true);

#define INTERPOLATE_VAR(name, val, maxDiff) interpolate##name(val, maxDiff)
...
#define INTERPOLATE(name, val) interpolate##name(val)

#define RESET_INTERPOLATION(varname) interpolate##varname(0, true);

#define INTERPOLATE_AND_STORE(name, val) name = interpolate##varname(val)
...
#define DECLARE_INTERPOLATE_ARRAY(varname, type, maxDiff, size)                                                             \
  inline type interpolate##varname(type val, int index) {                                                                   \
    ...
    if (newFrame[index]) {                                                                                                  \
      if (val - old[index] > maxDiff)                                                                                       \
        val = old[index] + maxDiff;                                                                                         \
      if (val - old[index] < -maxDiff)                                                                                      \
        val = old[index] - maxDiff;                                                                                         \
      old[index] = val;                                                                                                     \
...
#define INTERPOLATE_ARRAY_ELEMENT(name, val, index) interpolate##name(val, index)

#define INTERPOLATE_ARRAY_ELEMENT_AND_STORE(name, val, index) val = interpolate##name(val, index)
