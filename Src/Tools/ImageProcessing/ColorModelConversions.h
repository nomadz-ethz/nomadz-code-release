...
public:
   ...
   */
  static void
  fromYCbCrToRGB(unsigned char Y, unsigned char Cb, unsigned char Cr, unsigned char& R, unsigned char& G, unsigned char& B) {
    int r = Y + ((1436 * (Cr - 128)) >> 10), g = Y - ((354 * (Cb - 128) + 732 * (Cr - 128)) >> 10),
        b = Y + ((1814 * (Cb - 128)) >> 10);
    if (r < 0)
      r = 0;
    else if (r > 255)
   ...
   */
  static void
  fromRGBToYCbCr(unsigned char R, unsigned char G, unsigned char B, unsigned char& Y, unsigned char& Cb, unsigned char& Cr) {
    int y = (int)(0.2990 * R + 0.5870 * G + 0.1140 * B), cr = 127 + (int)(-0.1687 * R - 0.3313 * G + 0.5000 * B),
   ...
   */
  static void
  fromYCbCrToHSI(unsigned char Y, unsigned char Cb, unsigned char Cr, unsigned char& H, unsigned char& S, unsigned char& I) {
    ...
    int h = std::abs(k + (std::abs(g - b) << 8) / (chroma ? 6 * chroma : 1));
   ...
   */
  static void
  fromHSIToYCbCr(unsigned char H, unsigned char S, unsigned char I, unsigned char& Y, unsigned char& Cb, unsigned char& Cr) {
    ...
    fromRGBToYCbCr((unsigned char)r, (unsigned char)g, (unsigned char)b, Y, Cb, Cr);
  }

  static void fromYCbCrToHSI2(unsigned char Y, unsigned char Cb, unsigned char Cr, float& H, float& S, float& I) {

    unsigned char R, G, B;
    ColorModelConversions::fromYCbCrToRGB(Y, Cb, Cr, R, G, B);
    // RGB -> HSI
