 ...
 *
 * @note The original author of this file is <a href="mailto:timlaue@informatik.uni-bremen.de">Tim Laue</a>
 */
...
public:
  /** The id of the robot, which sent the request*/
  unsigned char id;
   ...
   */
  NTPRequest(unsigned char sender)
      : sender(sender){},
...
public:
  ...
  SynchronizationMeasurement computeSynchronizationMeasurement() const {
    ...
    // check for violations in input, this should normally never happen
    if (responseReceipt < requestOrigination || responseOrigination < requestReceipt || t.roundTrip < 0) {
      t.roundTrip = INT_MAX;
      t.offset = 0;
      return t;
    }
    t.offset = (int(requestReceipt + responseOrigination) - int(requestOrigination + responseReceipt)) / 2;
    return t;
  }
  ,
...
private:
  enum {
    MAX_NUM_OF_NTP_CLIENTS = 255,
    MAX_NUM_OF_NTP_PACKAGES = 12,
    NTP_REQUEST_INTERVAL = 30000
  };                                /**< Some constants for NTP snychronization*/
  ...
  SynchronizationMeasurementsBuffer
    timeSyncBuffers[MAX_NUM_OF_NTP_CLIENTS]; /**< A buffer which contains synchronization data for all other robots*/
...
public:
  ...
  unsigned int
    sendTimeStamp; /**< The send time stamp from the remote robot of the last handled team communication message. */
  ...
  /**
   * Constructor to pass local index
   */
  NTP(unsigned char id);
  ...
  inline unsigned getRemoteTimeInLocalTime(unsigned remoteTime) const {
    return getRemoteTimeInLocalTime(remoteTime, currentRemoteId);
  }
