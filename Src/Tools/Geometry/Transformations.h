...
namespace Geometry {
  ...
  /**
   * calculates whether a pixel in the image is inside the field or outside.
   * Based on the deepFieldBoundary
   *
   */
  bool
  calculatePixelInsideBoundary(const int x, const int y, const FieldBoundary& fieldBoundary, const CameraInfo& cameraInfo);
  bool calculatePixelInsideBoundary(const float x,
                                    const float y,
                                    const FieldBoundary& FieldBoundary,
                                    const CameraInfo& cameraInfo);
  float findBoundaryY(const float x, const FieldBoundary& FieldBoundary, const CameraInfo& cameraInfo);
} // namespace Geometry