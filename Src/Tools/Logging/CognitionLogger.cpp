...
#include "Core/Debugging/Asserts.h"
...
CognitionLogger::CognitionLogger(const ModuleManager& moduleManager)
    : params(*(new Parameters("logger.cfg"))), state(PRE_INITIAL), initialized(false), readIndex(0), writeIndex(0),
      frameCounter(0), framesToWrite(0), shouldPlayLogSound(false), moduleManager(moduleManager) {
  initStateFunctors();
...
void CognitionLogger::initRepresentations() {
  ...
  REGISTER_REPRESENTATION(RobotPoseSamples);
  REGISTER_REPRESENTATION(BallPercept);
  REGISTER_REPRESENTATION(LineAnalysis);
  REGISTER_REPRESENTATION(FieldBoundary);
  REGISTER_REPRESENTATION(ImageCoordinateSystem);
  REGISTER_REPRESENTATION(PenaltyMarkPercept);
  REGISTER_REPRESENTATION(GoalPercept);
  REGISTER_REPRESENTATION(PlayerPercept);
  REGISTER_REPRESENTATION(PlayerModel);
...
void CognitionLogger::preInitial() { // wait till logger is enabled and blackboard is available, go to INITIAL if it is
...
void CognitionLogger::idle() {
  ...
  if (gameState == STATE_READY || gameState == STATE_SET || gameState == STATE_PLAYING) {
    ...
    state = RUNNING;
  } else if (shouldPlayLogSound && writerIdle &&
             Time::getTimeSince(writerIdleStart) > 5000) { // if the writer is idle and 5 seconds have passed we can
                                                           // safely assume that the log file has been completely
                                                           // written
...
void CognitionLogger::running() {
  ...
  if (freeSpace < params.minFreeSpace) { // end writer thread if disk space has fallen below the minimum allowed value and go
                                         // into error state
...
void CognitionLogger::logFrame() {
  if (writeIndex ==
      mod((readIndex - 1),
          params.maxBufferSize)) // use mathematically mod instead of % because (readIndex -1) might be negative
  ...
  if (frameCounter >= 60) {
    ...
    frameCounter = 0;
    writeIndex =
      (writeIndex + 1) % params.maxBufferSize; // we can use normal % instead of mod() because writeIndex is unsigned
...
string CognitionLogger::generateFilename() {
  ...
  stringstream ss;
  ss << (params.logFilePath) << Global::getSettings().playerNumber << "_" << Global::getSettings().robot.c_str() << "_"
     << buf << ".log";
  return ss.str();
#endif
  ...
  while (writerThread.isRunning()) // check if we are expecting more data
  {
    if (framesToWrite.wait(100)) // wait 200 ms for new data then check again if we should quit
    {
      ...
      if (numOfMsgs > 0) {
        ...
        VERIFY(snappy_compress(uncompresedBuffer.data(), (size_t)mem.getLength(), compressedBuffer.data(), &size) ==
               SNAPPY_OK);
        file << (unsigned)size;
...
unsigned CognitionLogger::getFreeSpace() const {
  ...
  if (0 == statvfs(logFilename.c_str(), &data)) {
    uint64_t size = (((uint64_t)data.f_bavail) / 1024) * ((uint64_t)data.f_bsize);
    if (size > INT_MAX) {
      return INT_MAX;
    } else {
      return size;
