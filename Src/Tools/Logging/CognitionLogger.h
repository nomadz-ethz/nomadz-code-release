...
private:
  ...
  ENUM(State,
       ...
       ERROR_STATE  // logger does nothing, can never recover from this state  (has to be named ERROR_STATE instead of ERROR
                    // because ERROR is a global variable in windows...)
  );
...
private:
  ...
  std::function<void()>
    states[numOfStates]; /**< This array holds one functor per state. Whenever we enter a state the functor is called */
  std::unordered_map<std::string, std::pair<MessageID, Streamable*>>
    representations; /**< Map from representation names to actual representations and their message ids */
  ...
  const ModuleManager&
    moduleManager; /**< Reference to the module manager. Used to gather information about execution order of modules */
