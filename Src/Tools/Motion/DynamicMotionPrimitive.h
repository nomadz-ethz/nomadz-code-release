...
#pragma once
#include "Core/Math/FifthOrderPolynomial.h"
#include "Core/Math/Eigen.h"
#include "CanonicalSystem.h"
#include "Core/Math/RadialBasisFunctionApproximator.h"
...
private:
  ...
  VectorXf
  calculateForces(const int dim, const Matrixf& positions, const Matrixf& velocities, const Matrixf& accelerations) const;
...
template <int DIM> void DynamicMotionPrimitive<DIM>::stepInternal(const float dt) {
  ...
  for (int i = 0; i < eta.size(); ++i) {
    if (eta(i) == 0.0 || std::isnan(eta(i)) ||
        std::isinf(eta(i))) { // this happens when start and goal velocity are identical.
      eta(i) = 1.0;
    }
  }
  f.array() *= eta;
...
template <int DIM>
VectorXf DynamicMotionPrimitive<DIM>::calculateCenters(const float executionTime, const unsigned noCenters) const {
...
template <int DIM>
Eigen::Matrix<float, DIM, Eigen::Dynamic>
DynamicMotionPrimitive<DIM>::imitate(const Matrixf& positions, const Matrixf& velocities, const Matrixf& accelerations) {
...
template <int DIM>
VectorXf DynamicMotionPrimitive<DIM>::calculateForces(const int dim,
                                                      const Matrixf& positions,
                                                      const Matrixf& velocities,
