...
void UKFSample::init(const Pose2D& pose, const Pose2D& defaultPoseDeviation, float defaultValidity) {
  mean.x = pose.translation.x;
  mean.y = pose.translation.y;
  mean.z = pose.rotation;
  cov = Matrix3x3f();
  cov[0][0] = sqr(defaultPoseDeviation.translation.x); // NOLINT: prevent clang-tidy from changing cov to cos
  cov[1][1] = sqr(defaultPoseDeviation.translation.y); // NOLINT: prevent clang-tidy from changing cov to cos
  cov[2][2] = sqr(defaultPoseDeviation.rotation);      // NOLINT: prevent clang-tidy from changing cov to cos
  mirrored = false;
  // Buffer is always initially filled with average value
  for (int i = 0; i < validityBuffer.getMaxEntries(); ++i) {
    validityBuffer.add(defaultValidity);
  }
}

void UKFSample::motionUpdate(const Pose2D& odometryOffset,
                             const Pose2D& filterProcessDeviation,
                             const Pose2D& odometryDeviation,
                             const Vector2<>& odometryRotationDeviation) {
  generateSigmaPoints();
  performOdometryUpdate(odometryOffset, filterProcessDeviation, odometryDeviation, odometryRotationDeviation);
}
...
float UKFSample::computeValidity() {
  return validity = static_cast<float>(validityBuffer.getSum()) / validityBuffer.getNumberOfEntries();
}
...
void UKFSample::lineLocatorUpdate(const Pose2D& reading, const float covX, const float covY, const float covR) {
  Matrix3x3f readingCov = Matrix3x3f(Vector3f(covX, 0.f, 0.f), // col 1
                                     Vector3f(0.f, covY, 0.f), // col 2
                                     Vector3f(0.f, 0.f, covR)  // col 3
  );
  poseSensorUpdate(Vector3f(reading.translation.x, reading.translation.y, reading.rotation), readingCov);
}

void UKFSample::performOdometryUpdate(const Pose2D& odometryOffset,
                                      const Pose2D& filterProcessDeviation,
                                      const Pose2D& odometryDeviation,
                                      const Vector2<>& odometryRotationDeviation) {
...
void UKFSample::updateByGoalPercept(const GoalPercept& goalPercept,
  ...
  for (vector<GoalPost>::const_iterator goalPost = goalPercept.goalPosts.begin(); goalPost != goalPercept.goalPosts.end();
       goalPost++) {
    // Data association:
    Vector2<> associatedPost;
    bool postFound(false);
    if (goalPost->position == GoalPost::IS_UNKNOWN) {
      postFound = fieldModel.getAssociatedUnknownGoalPost(
        robotPose, goalPost->positionOnField, associatedPost, goalAssociationMaxAngle, goalAssociationMaxAngularDistance);
    } else { // goalPost->IS_LEFT || goalPost->IS_RIGHT
      postFound = fieldModel.getAssociatedKnownGoalPost(robotPose,
                                                        goalPost->positionOnField,
                                                        goalPost->position == GoalPost::IS_LEFT,
                                                        associatedPost,
                                                        goalAssociationMaxAngle,
                                                        goalAssociationMaxAngularDistance);
    }

    // Filter update:
    if (postFound) {
      Matrix2x2f cov = getCovOfPointInWorld(goalPost->positionOnField, 0.f, cameraMatrix, robotRotationDeviation) *
                       1.7f; // multiply by factor to account for bad/uncertain white goal detection
...
void UKFSample::updateByLineAnalysis(const LineAnalysis& lineAnalysis,
                                     const FieldModel& fieldModel,
  ...
  if (lineAnalysis.circle.found) {
    const Pose2D robotPose = getPose();
    const Vector2<> circleInWorld = robotPose * lineAnalysis.circle.pos;
    centerCircleCanBeUsed = circleInWorld.abs() <= centerCircleAssociationDistance;
    validityBuffer.add(centerCircleCanBeUsed ? 1.f : 0.f);
  }
  // Iterate over all observed lines:
  for (std::vector<LineAnalysis::ObservedFieldLine>::const_iterator it = lineAnalysis.lines.begin();
       it != lineAnalysis.lines.end();
       ++it) {
    const Pose2D robotPose = getPose();
    int index = fieldModel.getIndexOfAssociatedLine(robotPose, it->first, it->last, lineAssociationCorridor);
    if (index != -1) {
      ...
      dir.normalize();
      Matrix2x2f cov = getCovOfPointInWorld(center, 0.f, cameraMatrix, robotRotationDeviation);
      ...
      if (it->midLine && centerCircleCanBeUsed) {
        Matrix2x2f circleCov =
          getCovOfCircle(lineAnalysis.circle.pos, fieldDimensions.centerCircleRadius, cameraMatrix, robotRotationDeviation);
        ...
        const float measueredY = -(Pose2D(measuredAngle, measuredX, 0.f) * lineAnalysis.circle.pos).y;

        poseSensorUpdate(
          Vector3f(measuredX, measueredY, measuredAngle),
          Matrix3x3f(Vector3f(xVariance, 0.f, 0.f), Vector3f(0.f, covY[1][1], 0.f), Vector3f(0.f, 0.f, angleVariance)));
      ...
      // Integrate "normal" lines:
        ...
        if (fieldLine.vertical) {
          ...
          lineSensorUpdate(
            true, Vector2f(measuredY, measuredAngle), Matrix2x2f(Vector2f(yVariance, 0.f), Vector2f(0.f, angleVariance)));
        } else {
          const float measuredX = fieldLine.start.x - orthogonalProjection.x;
          const float xVariance = cov[0][0];
          const float angleVariance = sqr(atan(sqrt(4.f * xVariance / (it->first - it->last).squareAbs())));
          lineSensorUpdate(
            false, Vector2f(measuredX, measuredAngle), Matrix2x2f(Vector2f(xVariance, 0.f), Vector2f(0.f, angleVariance)));
        }
      }
    }
  }
  ...
  if (centerCircleCanBeUsed) {
    Matrix2x2f cov =
      getCovOfCircle(lineAnalysis.circle.pos, fieldDimensions.centerCircleRadius, cameraMatrix, robotRotationDeviation);
    landmarkSensorUpdate(Vector2<>(), Vector2f(lineAnalysis.circle.pos.x, lineAnalysis.circle.pos.y), cov);
  }
  ...
  for (std::vector<LineAnalysis::Intersection>::const_iterator intersection = lineAnalysis.intersections.begin();
       intersection != lineAnalysis.intersections.end();
       ++intersection) {
    const Pose2D robotPose = getPose();
    Vector2<> associatedCorner;
    if (fieldModel.getAssociatedCorner(robotPose, *intersection, associatedCorner, cornerAssociationDistance)) {
      Matrix2x2f cov = getCovOfPointInWorld(intersection->pos, 0.f, cameraMatrix, robotRotationDeviation);
      landmarkSensorUpdate(associatedCorner, Vector2f(intersection->pos.x, intersection->pos.y), cov);
    }
  }
}

Matrix2x2f UKFSample::getCovOfPointInWorld(const Vector2<>& pointInWorld2,
                                           float pointZInWorld,
                                           const CameraMatrix& cameraMatrix,
  ...
  Matrix2x2f cov(
    Vector2f(sqr(h / tan((distance == 0.f ? pi_2 : atan(h / distance)) - robotRotationDeviation.x) - distance), 0.f),
    Vector2f(0.f, sqr(tan(robotRotationDeviation.y) * distance)));
  return rot * cov * rot.transpose();
}

Matrix2x2f UKFSample::getCovOfCircle(const Vector2<>& circlePos,
  ...
  return getCovOfPointInWorld(increasedCirclePos, 0.f, cameraMatrix, robotRotationDeviation);
}

void UKFSample::landmarkSensorUpdate(const Vector2<>& landmarkPosition,
                                     const Vector2f& reading,
                                     const Matrix2x2f& readingCov) {
  generateSigmaPoints();
...
// TODO Put line processing logic here instead
void UKFSample::updateByTracker(const Vector3f& reading, const Matrix3x3f& readingCov, float ratio, bool goodCorrection) {
  if (ratio > 1.f) {
    ratio = 1.f;
  }
  if (ratio < 0.f) {
  ...
  if (goodCorrection && ratio > 0.f) {
    poseSensorUpdate(reading, readingCov);
  }
  validityBuffer.add(ratio);
}
...
void UKFSample::computeWeightingBasedOnValidity(const FieldDimensions& fieldDimensions, float baseValidityWeighting) {
  ...
  weighting = baseValidityWeighting + (1.f - baseValidityWeighting) * factor;
}

void UKFSample::computeWeightingBasedOnBallObservation(const Vector2<>& ballObservation,
  ...
  float originalWeighting = computeAngleWeighting(angleObserved, teamBallPosition, theRobotPose, standardDeviationBallAngle);
  originalWeighting *=
    computeDistanceWeighting(distanceAsAngleObserved, teamBallPosition, theRobotPose, camZ, standardDeviationBallDistance);

  // Weighting for mirrored pose
  const Pose2D mirroredPose = Pose2D(pi) + (Pose2D)(theRobotPose);
  float mirroredWeighting = computeAngleWeighting(angleObserved, teamBallPosition, mirroredPose, standardDeviationBallAngle);
  mirroredWeighting *=
    computeDistanceWeighting(distanceAsAngleObserved, teamBallPosition, mirroredPose, camZ, standardDeviationBallDistance);
...
float UKFSample::computeAngleWeighting(float measuredAngle,
...
float UKFSample::computeDistanceWeighting(float measuredDistanceAsAngle,
                                          const Vector2<>& modelPosition,
                                          const Pose2D& robotPose,
                                          float cameraZ,
                                          float standardDeviation) const {
...
void UKFSample::updateMirrorFlag(bool fallen,
                                 bool armContact,
                                 float maxDistanceToFieldCenterForMirrorActions,
                                 const FieldDimensions& fieldDimensions) {
  const float distToFieldCenter = getPose().translation.abs();
  if (fallen && distToFieldCenter < maxDistanceToFieldCenterForMirrorActions) {
    float probability(0.5f);
    const float rCircle = fieldDimensions.centerCircleRadius;
    if (distToFieldCenter > rCircle) {
      probability += ((distToFieldCenter - rCircle) / (maxDistanceToFieldCenterForMirrorActions - rCircle));
    }
  ...
  if (armContact && distToFieldCenter < maxDistanceToFieldCenterForMirrorActions) {
...
void UKFSample::draw(bool simple) {
  if (simple) {
    ...
    RECTANGLE2("module:SelfLocator:simples",
               Vector2<>(pose.translation - Vector2<>(55.f, 90.f)),
               110,
               180,
               pose.rotation,
               0,
    ...
    ELLIPSE("module:SelfLocator:samples",
            mean,
            sqrt(3.0f) * axis1,
            sqrt(3.0f) * axis2,
            angle,
