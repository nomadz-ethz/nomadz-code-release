...
Pose2D TemplateGenerator::getTemplateAtReenterPosition(int i, int num_samples) const {
  const float interval = 300.f; // mm
  std::vector<Pose2D> reenterPositions;
  for (int j = 0; j < num_samples / 2; j++) {
    reenterPositions.push_back({-pi_2,
                                theFieldDimensions.xPosOwnPenaltyMark - (num_samples / 4 * interval) + j * interval,
