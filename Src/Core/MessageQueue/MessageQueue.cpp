...
void MessageQueue::append(In& stream) {
  unsigned usedSize, numberOfMessages;
  ...
  if (dest) {
    ...
    for (unsigned i = 0; numberOfMessages == (unsigned)-1 ? !stream.eof() : i < numberOfMessages; ++i) {
      ...
      if ((id >= numOfDataMessageIDs || size == 0) && numberOfMessages == (unsigned)-1) {
        OUTPUT(idText,
               text,
               "MessageQueue: Logfile appears to be broken. Skipping rest of file. Read messages: "
                 << queue.numberOfMessages << " read size:" << queue.usedSize);
        break;
      }
