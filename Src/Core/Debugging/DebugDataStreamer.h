...
private:
  ...
  const char*
    name;    /**< The name of the next entry streamed. 0 if it does not have a name, because it is an array element. */
  ...
  static PROCESS_WIDE_STORAGE(
    const std::vector<const char*>) enumNames; /**< Helper to provide element names for the enum currently streamed. */
   ...
   */
  static const char* getName(unsigned char value);
   ...
   */
  template <typename T> void streamIt(In* in, Out* out, const char* (*enumToString)(unsigned char) = 0) {
    T t;
    if (in) {
