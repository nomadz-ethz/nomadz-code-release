...
public:
   ...
   */
  enum ShapeType { dot, line, cube, polygon, quad, coordinates, scale, rotate, translate, sphere, cylinder, image };
};
...
#define LINE3D(id, fromX, fromY, fromZ, toX, toY, toZ, size, color)                                                         \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::line << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(fromX)                 \
                                  << (float)(fromY) << (float)(fromZ) << (float)(toX) << (float)(toY) << (float)(toZ)       \
 ...
 * A macro that adds a line to a drawing.
 * @param id The drawing to which the line should be added.
 * @param from The vector position of the first point.
 * @param to The vector position of the second point.
 * @param size The thickness of the line in pixels.
 * @param color The color of the line
 */
#define LINE3D_VEC(id, from, to, size, color)                                                                               \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::line << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(from.x)                \
                                  << (float)(from.y) << (float)(from.z) << (float)(to.x) << (float)(to.y) << (float)(to.z)  \
                                  << (float)(size) << ColorRGBA(color));                                                    \
  })
...
#define QUAD3D(id, corner1, corner2, corner3, corner4, color)                                                               \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::quad << (char)Global::getDrawingManager3D().getDrawingId(id) << Vector3<>(corner1)             \
                                  << Vector3<>(corner2) << Vector3<>(corner3) << Vector3<>(corner4) << ColorRGBA(color));   \
  })
...
#define CUBE3D(id, a, b, c, d, e, f, g, h, size, color)                                                                     \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::cube << (char)Global::getDrawingManager3D().getDrawingId(id) << Vector3<>(a) << Vector3<>(b)   \
                                  << Vector3<>(c) << Vector3<>(d) << Vector3<>(e) << Vector3<>(f) << Vector3<>(g)           \
                                  << Vector3<>(h) << (float)(size) << ColorRGBA(color));                                    \
  })
...
#define COORDINATES3D(id, length, width)                                                                                    \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::coordinates << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(length)         \
                                         << (float)(width));                                                                \
  })
...
#define SCALE3D(id, x, y, z)                                                                                                \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::scale << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)      \
                                   << (float)(z));                                                                          \
  })
...
#define ROTATE3D(id, x, y, z)                                                                                               \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::rotate << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)     \
                                    << (float)(z));                                                                         \
  })
...
#define TRANSLATE3D(id, x, y, z)                                                                                            \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::translate << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)  \
                                       << (float)(z));                                                                      \
  })
...
#define POINT3D(id, x, y, z, size, color)                                                                                   \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::dot << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)        \
                                 << (float)(z) << (float)size << ColorRGBA(color) << false);                                \
  })
...
#define SPHERE3D(id, x, y, z, radius, color)                                                                                \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::sphere << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)     \
                                    << (float)(z) << (float)(radius) << ColorRGBA(color));                                  \
  })

/** A macro that adds a sphere to a drawing
 * @param id The drawing to which the sphere will be added
 * @param point The vector position of the sphere.
 * @param radius The radius of the sphere.
 * @param color The color of the line
 */
#define SPHERE3D_VEC(id, point, radius, color)                                                                              \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::sphere << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(point.x)             \
                                    << (float)(point.y) << (float)(point.z) << (float)(radius) << ColorRGBA(color));        \
  })
...
#define CYLINDER3D(id, x, y, z, a, b, c, radius, height, color)                                                             \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::cylinder << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)   \
                                      << (float)(z) << (float)(a) << (float)(b) << (float)(c) << (float)(radius)            \
...
#define CYLINDER3D2(id, x, y, z, a, b, c, baseRadius, topRadius, height, color)                                             \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::cylinder << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)   \
                                      << (float)(z) << (float)(a) << (float)(b) << (float)(c) << (float)(baseRadius)        \
                                      << (float)(topRadius) << (float)(height) << ColorRGBA(color));                        \
  })
...
#define IMAGE3D(id, x, y, z, a, b, c, width, height, i)                                                                     \
  NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, {                                                                     \
    OUTPUT(idDebugDrawing3D,                                                                                                \
           bin,                                                                                                             \
           (char)Drawings3D::image << (char)Global::getDrawingManager3D().getDrawingId(id) << (float)(x) << (float)(y)      \
                                   << (float)(z) << (float)(a) << (float)(b) << (float)(c) << (float)(width)                \
                                   << (float)(height) << i);                                                                \
  })
 ...
 */
#define COMPLEX_DRAWING3D(id, ...) NOT_POLLABLE_DEBUG_RESPONSE("debug drawing 3d:" id, __VA_ARGS__;)
