...
#include "Core/System/TcpComm.h"

#define MAX_PACKAGE_SIZE                                                                                                    \
  67108864 // max package size that can be received. prevent from allocating to much buffer (max ~64 MB)
...
class TcpConnection {
public:
  enum Handshake { noHandshake, sender, receiver };
...
public:
   ...
   */
  TcpConnection(
    const char* ip, int port, Handshake handshake = noHandshake, int maxPackageSendSize = 0, int maxPackageReceiveSize = 0) {
