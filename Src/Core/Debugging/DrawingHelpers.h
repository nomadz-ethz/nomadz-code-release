...
public:
   ...
   */
  static ColorRGBA
  heat(float value, float maxValue, float minValue = 0.0, float overlappingScale = 5.0f, unsigned char alpha = 255) {
    ...
    return ColorRGBA(
      static_cast<unsigned char>(red), static_cast<unsigned char>(green), static_cast<unsigned char>(blue), alpha);
  }
};
