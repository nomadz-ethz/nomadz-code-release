...
struct TimingManager::Pimpl {
  ...
  unordered_map<const char*, unsigned short>
    idTable; /**< Key: name of the stopwatch. Value: the id that is used when sending timing data over the network */
  ...
  int watchNameIndex; /**< Every frame a few watch names are transmitted. This is the index of the watchname that is to be
                         transmitted next */
};

void TimingManager::startTiming(const char* identifier) {
  unsigned long long startTime = Time::getCurrentThreadTime();
  if (prvt->timing.find(identifier) == prvt->timing.end()) { // create new entry
    prvt->watchNames.push_back(identifier);
    prvt->idTable[identifier] =
      (unsigned short)prvt->idTable
        .size(); // NOTE: this assumes that an unsigned short will always be big big enough to count the timers...
  }
...
void TimingManager::prepareData() {
   ...
   * for each stopwatch:
   *  short : id of the stopwatch  (depending on where you start in the stream you might not know the name of this watch,
   * yet) unsigned : time in microseconds
   *
