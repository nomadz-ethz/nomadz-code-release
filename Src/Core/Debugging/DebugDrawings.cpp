 ...
 *
 * @note The original author is Michael Spranger
 */
...
Out& operator<<(Out& stream, const DrawingManager& drawingManager) {
  ...
  for (std::unordered_map<const char*, char>::const_iterator iter = drawingManager.types.begin();
       iter != drawingManager.types.end();
       ++iter) {
  ...
  for (std::unordered_map<const char*, DrawingManager::Drawing>::const_iterator iter = drawingManager.drawings.begin();
       iter != drawingManager.drawings.end();
       ++iter) {
