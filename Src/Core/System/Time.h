...
public:
  ...
  static unsigned secAndNanoToMilli(unsigned sec, unsigned nanoSec) { return sec * 1000 + nanoSec / 1000000; }
...
inline unsigned int toMillisec(int sec, unsigned int nsec) {
  return static_cast<unsigned>(sec * 1000) + nsec / 1000000 - Time::getSystemTimeBase();
}
