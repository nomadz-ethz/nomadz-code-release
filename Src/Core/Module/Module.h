 ...
 *   PROVIDES(GoalsPercept)                               // Classprovides a method to update GoalsPercept.
...
#include "Core/Module/Blackboard.h"
...
public:
  ...
  public:
    ...
    void (*in)(In*,
               rclcpp::Node*); /**< The handler that is called to read the instance of the representation from a stream. */
     ...
     */
    Entry(const char* name, void (*create)(), void (*free)(), void (*in)(In*, rclcpp::Node*))
        : name(name), create(create), free(free), in(in) {}
...
protected:
   ...
   * @param in The handler that is called to read the instance of the representation from a stream.
   */
  void add(const char* name, void (*create)(), void (*free)(), void (*in)(In*, rclcpp::Node*));
};
 ...
 * @param in The handler that is called to read the instance of the representation from a stream.
 */
template <const char* (*getName)(), void (*create)(), void (*free)(), void (*in)(In*, rclcpp::Node*)>
class Requirement : private Requirements {
public:
  /**
...
public:
  ...
  public:
    ...
    void (*free)();              /**< The handler that is called to delete the instance of the representation. */
    void (*out)(Out*,
                rclcpp::Node*); /**< The handler that is called to write the instance of the representation to a stream. */
     ...
     */
    Entry(const char* name, void (*update)(Blackboard&), void (*create)(), void (*free)(), void (*out)(Out*, rclcpp::Node*))
        : name(name), update(update), create(create), free(free), out(out) {}
...
protected:
   ...
   */
  void
  add(const char* name, void (*update)(Blackboard&), void (*create)(), void (*free)(), void (*out)(Out*, rclcpp::Node*));
};
...
protected:
  ...
  Representations::List
    representations; /**< The list of all representations provided by the module created by this instance. */
...
public:
   ...
   */
  ModuleBase(const char* name, const char* category) : next(first), name(name), category(category) { first = this; }
...
private:
   ...
   */
  Blackboard* createNew() { return (Blackboard*)new M; }
...
public:
   ...
   * @param category The name of the category of this module.
   */
  Module(const char* name, const char* category) : ModuleBase(name, category) {
    Representations::entries = &representations;
    Requirements::entries = &requirements;
    // HACK: executes assignment operators -> recording information!
    B base{internal::ref_init_tag()};
    base = (const B&)base;
...
#define MODULE(module)                                                                                                      \
  ...
  private:                                                                                                                  \
    typedef module##Base _Me;                                                                                               \
                                                                                                                            \
  public:                                                                                                                   \
    module##Base(internal::ref_init_tag) {}                                                                                 \
    module##Base(const char* fileName = 0) : Blackboard(*Blackboard::theInstance), _initFirstAttribute(this) {              \
      if (_parameterType == 2)                                                                                              \
  ...
  private:                                                                                                                  \
    static PROCESS_WIDE_STORAGE(_Me) _this;                                                                                 \
    int _parameterType; /* 0: no params, 1: define them, 2: load them. */                                                   \
    class _InitFirstAttribute {                                                                                             \
    public:                                                                                                                 \
      _InitFirstAttribute() { _this = nullptr; }                                                                            \
      _InitFirstAttribute(_Me* value) {                                                                                     \
        _this = value;                                                                                                      \
        _this->_parameterType = 0;                                                                                          \
      }                                                                                                                     \
    } _initFirstAttribute;                                                                                                  \
    typedef void (_Me::*PSTREAMPROC)(In * in, Out* out);                                                                    \
                                                                                                                            \
  private:                                                                                                                  \
    std::list<PSTREAMPROC> _parameters;                                                                                     \
...
#define DEFINES_PARAMETER(type, name, ...)                                                                                  \
  _STREAM_EXPAND(_STREAM_EXPAND(                                                                                            \
    _STREAM_THIRD(__VA_ARGS__, _DEFINES_PARAMETER_WITH_CLASS, _DEFINES_PARAMETER_WITHOUT_CLASS))(type, name, __VA_ARGS__))

#define _DEFINES_PARAMETER_WITHOUT_CLASS(type, name, defaultValue)                                                          \
  _PARAMETER(type, name, moduleBase->name = defaultValue;, STREAM(name), 1)
#define _DEFINES_PARAMETER_WITH_CLASS(class, type, name, defaultValue)                                                      \
  _PARAMETER(class ::type, name, moduleBase->name = class ::defaultValue;, STREAM(name, class), 1)

#define LOADS_PARAMETER(type, ...)                                                                                          \
  _STREAM_EXPAND(_STREAM_EXPAND(_STREAM_THIRD(__VA_ARGS__, _LOADS_PARAMETER_WITH_CLASS, _LOADS_PARAMETER_WITHOUT_CLASS))(   \
    type, __VA_ARGS__))
...
public:                                                                                                                     \
  decltype(Streaming::TypeWrapper<theType>::type) name; /* The actual parameter */                                          \
private:                                                                                                                    \
  void _##name##Stream(In* in, Out* out) { streamCommand; }                                                                 \
  class _##name##Init {                                                                                                     \
  public:                                                                                                                   \
    _##name##Init() {                                                                                                       \
      if (_this == nullptr) {                                                                                               \
      ...
      ASSERT(!moduleBase->_parameterType ||                                                                                 \
             moduleBase->_parameterType ==                                                                                  \
               parameterType); /* Either using LOADS_PARAMETER or DEFINES_PARAMETER is allowed. */                          \
...
protected:                                                                                                                  \
  using Blackboard::the##representation;                                                                                    \
                                                                                                                            \
  /**                                                                                                                       \
   * The method creates the representation in the blackboard.                                                               \
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void create2##representation() {                                         \
   ...
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void free2##representation() {                                           \
    if (&((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation) {                                               \
   ...
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void replace2##representation(representation** r,                        \
                                                                                 representation* rNew) {                    \
   ...
   */                                                                                                                       \
  static void in##representation(In* stream, rclcpp::Node* ros_node) {                                                      \
    representation& repr = const_cast<representation&>(((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation);  \
    if (stream) {                                                                                                           \
      *stream >> repr;                                                                                                      \
    }                                                                                                                       \
    internal::ros_handle_in_repr<representation>(#representation, stream != nullptr, ros_node, repr);                       \
  }                                                                                                                         \
                                                                                                                            \
private:                                                                                                                    \
  /**                                                                                                                       \
  ...
  static const char* getName2##representation() { return #representation; }                                                 \
                                                                                                                            \
  Requirement<&_Me::getName2##representation,                                                                               \
              &_Me::create2##representation,                                                                                \
              &_Me::free2##representation,                                                                                  \
              &_Me::in##representation>                                                                                     \
    z##representation;
 ...
 * @param representation The representation that can be updated by this module.
 */
#define PROVIDES(representation) _PROVIDES(representation, )
...
#define _PROVIDES(representation, mod)                                                                                      \
   ...
   */                                                                                                                       \
protected:                                                                                                                  \
  virtual void update(representation& the##representation){};                                                               \
                                                                                                                            \
  /**                                                                                                                       \
   ...
   */                                                                                                                       \
private:                                                                                                                    \
  __attribute__((no_sanitize("undefined"))) static void update##representation(Blackboard& b) {                             \
   ...
   * The method creates the representation in the blackboard.                                                               \
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void create##representation() {                                          \
    if (!&((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation)                                                \
   ...
   * The method deletes the representation from the blackboard.                                                             \
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void free##representation() {                                            \
    if (&((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation) {                                               \
      delete &((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation;                                            \
   ...
   * @param rNew The new entry.                                                                                             \
   */                                                                                                                       \
  __attribute__((no_sanitize("undefined"))) static void replace##representation(representation** r, representation* rNew) { \
    for (unsigned i = 0; i < sizeof(Blackboard) / sizeof(representation*); ++i) {                                           \
      representation* temp = r[i];                                                                                          \
      r[i] = rNew;                                                                                                          \
      distract(); /* This call forces the compiler to consider possible changes to global data. */                          \
   ...
   */                                                                                                                       \
  static void out##representation(Out* stream, rclcpp::Node* ros_node) {                                                    \
    const representation& repr = ((_Me*)(Blackboard*)Blackboard::theInstance)->the##representation;                         \
    if (stream) {                                                                                                           \
      *stream << repr;                                                                                                      \
    }                                                                                                                       \
    internal::ros_handle_out_repr(#representation, stream != nullptr, ros_node, repr);                                      \
  }                                                                                                                         \
                                                                                                                            \
  /**                                                                                                                       \
   * The method returns the name of the representation.                                                                     \
  ...
  static const char* getName##representation() { return #representation; }                                                  \
                                                                                                                            \
  Representation<&_Me::getName##representation,                                                                             \
                 &_Me::update##representation,                                                                              \
                 &_Me::create##representation,                                                                              \
                 &_Me::free##representation,                                                                                \
                 &_Me::out##representation>                                                                                 \
    zz##representation;
