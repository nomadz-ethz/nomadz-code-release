...
DefaultModule::DefaultModule() : ModuleBase("default", "Infrastructure") {
  // base class will add this module to the list of all modules, but it should not
  // so remove it again
  first = first->next;
}

DefaultModule::DefaultModule(const char* name, const char* category) : ModuleBase(name, category) {
  // base class will add this module to the list of all modules, but it should not
  // so remove it again
  first = first->next;
}
...
void DefaultModule::setRepresentations(const std::list<ModuleManager::ModuleState>& modules) {
  for (std::list<ModuleManager::ModuleState>::const_iterator i = modules.begin(); i != modules.end(); ++i) {
    for (Representations::List::const_iterator j = i->module->representations.begin(); j != i->module->representations.end();
         ++j) {
...
ExternalModule::ExternalModule() : DefaultModule("external", "External") {}

ModuleManager::ModuleManager(const char** categories, size_t numOfCategories)
    : timeStamp(0), defaultModule(new DefaultModule), otherDefaultModule(new DefaultModule),
  ...
  externalModule->setRepresentations(modules);
  externalModule->setRepresentations(otherModules);
  otherModules.push_back(ModuleState(externalModule));
}
...
bool ModuleManager::calcShared(const Configuration& config) {
  for (const auto& representationProvider : config.representationProviders) {
    ...
    if (module == modules.end() && otherModule == otherModules.end()) {
      ...
      if (module != modules.end()) {
        ...
        provided |= std::find(representations.begin(), representations.end(), representationProvider.representation) !=
                    representations.end();
  ...
  std::set<std::string>& extReprs = externalModule->getRequirements();
  for (const std::string& repr : extReprs) {
    if (std::find(shared.begin(), shared.end(), repr) == shared.end()) {
      shared.emplace_back(repr, true);
    }
  }
  return true;
}
...
bool ModuleManager::calcShared(const Configuration& config,
                               ...
                               const std::list<ModuleState>& modules) {
  ...
  for (const auto& requirement : module.module->requirements) {
    ...
    if (!provided) {
      ...
      shared.emplace_back(std::string(requirement.name), providedExternally);
    }
  }
...
void ModuleManager::update(In& stream, unsigned timeStamp) {
  std::list<Provider> providersToDelete(providers), providersToCreate, providersBackup(providers);
  std::list<Shared> sharedToDelete(shared), sharedToCreate, sharedBackup(shared);

  std::string representation, module;
          ...
          if (rp.representation == i->name) {
            ...
            if (m == providers.end()) {
              ...
              for (k = shared.begin(); k != shared.end(); ++k) {
                ...
                {
                  Requirements::List::const_iterator m =
                    std::find(j->module->requirements.begin(), j->module->requirements.end(), k->representation);
                  if (m != j->module->requirements.end()) // yes, one of the requirements is shared
                  {
                    ...
                    std::list<Shared>::iterator n =
                      std::find(sharedToDelete.begin(), sharedToDelete.end(), k->representation);
...
void ModuleManager::load(std::shared_ptr<rclcpp::Node> ros_node_in) {
...
void ModuleManager::execute() {
  ...
  for (std::list<Provider>::iterator i = providers.begin(); i != providers.end(); ++i) {
    if (i->moduleState->required) {
      ...
      if (duration > 100 && ((!Global::getDebugRequestTable().isActive("representation:JPEGImage") &&
                              !Global::getDebugRequestTable().isActive("representation:Image")) ||
                             duration > 500))
        TRACE(
          "TIMING: providing %s took %d ms at %d s after start", i->representation.c_str(), duration, timeStamp / 1000 - 10);
#endif
    }
  }
  BH_TRACE;
...
#ifdef ENABLE_ROS
  // With ROS do not distinguish module timestamps
  bool sameTime = true;
#else
  // Communication is only possible if both sides are based on the same module request.
  unsigned timeStamp;
  stream >> timeStamp;
  bool sameTime = (timeStamp == this->timeStamp);
#endif
  if (sameTime) {
    for (std::list<Shared>::const_iterator i = shared.begin(); i != shared.end(); ++i) {
      if (i->in) {
        if (i->ros_external) {
          i->in(nullptr, ros_node.get());
        } else {
          i->in(&stream, ros_node.get());
        }
      }
    }
  } else {
...
void ModuleManager::writePackage(Out& stream) const {
  ...
  for (std::list<Shared>::const_iterator i = shared.begin(); i != shared.end(); ++i) {
    if (i->out) {
      if (i->ros_external) {
        i->out(nullptr, ros_node.get());
      } else {
        i->out(&stream, ros_node.get());
      }
    }
  }
}
