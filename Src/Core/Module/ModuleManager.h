...
private:
  ...
  public:
     ...
     */
    Provider(const std::string& representation,
             ModuleState* moduleState,
             void (*update)(Blackboard&),
             void (*create)(),
             void (*free)())
        : representation(representation), moduleState(moduleState), update(update), create(create), free(free) {}
  ...
  std::map<const char*, const char*>
    selected; /**< The providers selected. This is always the last configuration set. It may not work. */
  ...
  public:
    ...
    void (*create)();                 /**< The method to create a new instance of the representation. */
    void (*free)();                   /**< The method to delete an instance of the representation. */
    void (*out)(Out*, rclcpp::Node*); /**< The function that is able to write the representation to a stream. */
    void (*in)(In*, rclcpp::Node*);   /**< The function that is able to read the representation from a stream. */
    bool ros_external;                /**< If the shared representation is shared via external ROS (only). */

    /**
     * Constructor.
     * @param representation The name of the shared representation.
     */
    Shared(const std::string& representation, bool ros_ext = false)
        : representation(representation), create(0), free(0), out(0), in(0), ros_external(ros_ext) {}
...
public:
  ...
  STREAMABLE(Configuration, {
    public : STREAMABLE(RepresentationProvider,
                        {
                        public:
                          ...
                          ,
                            ...
                            (std::string)representation, (std::string)provider,
                        }),
...
private:
  ...
  unsigned timeStamp; /**< The timeStamp of the last module request. Communication is only possible if both sides use the
                         same timestamp. */
  ...
  ExternalModule* externalModule;    /**< The external module to select input via ROS. */
...
public:
   ...
   */
  void load(std::shared_ptr<rclcpp::Node> ros_node);
...
protected:
   ...
   */
  DefaultModule(const char* name, const char* category);
};

/**
 * The class defines an external module that provides everything externally.
 * The representations will be read using ROS subscribers.
 */
class ExternalModule : public DefaultModule {
public:
  /**
   * Default constructor
   */
  ...
  /** Add requirement to fullfill. */
  static void addRequirement(std::string repr) { getRequirements().insert(std::move(repr)); }

  /** Store static list of requirements. */
  static std::set<std::string>& getRequirements() {
    static std::set<std::string> requirements;
    return requirements;
  }
};
