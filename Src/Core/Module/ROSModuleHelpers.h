...
#ifdef ENABLE_ROS
  template <typename T, typename M = typename T::ROSType> void ros_publish(rcl_publisher_t* pub, T msg);
#endif

  template <typename T> bool ros_take(void*, const T&);
#ifdef ENABLE_ROS
  template <typename T, typename M = typename T::ROSType> bool ros_take(rcl_subscription_t* sub, T& msg);
#endif
