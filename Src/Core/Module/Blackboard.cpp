...
__attribute__((no_sanitize("undefined"))) Blackboard::Blackboard()
    : // Initialize all representations by themselves:
      ...
      // BehaviorControl
      theActivationGraph(theActivationGraph), theBehaviorStatus(theBehaviorStatus), theFieldPosition(theFieldPosition),
      theLocalSkill(theLocalSkill), theBehaviorControlOutput(theBehaviorControlOutput),
      ...
      theArmContact(theArmContact), theFootSupport(theFootSupport),

      // MotionControl
      theArmMotionEngineOutput(theArmMotionEngineOutput), theArmMotionRequest(theArmMotionRequest),
      theOdometryData(theOdometryData), theGroundTruthOdometryData(theGroundTruthOdometryData),
      theMotionRequest(theMotionRequest), theSpeedRequest(theSpeedRequest), theHeadAngleRequest(theHeadAngleRequest),
      theHeadMotionRequest(theHeadMotionRequest), theHeadJointRequest(theHeadJointRequest),
      theMotionSelection(theMotionSelection), theSpecialActionsOutput(theSpecialActionsOutput),
      theWalkingEngineOutput(theWalkingEngineOutput), theWalkingEngineStandOutput(theWalkingEngineStandOutput),
      theBikeEngineOutput(theBikeEngineOutput), theKickEngineOutput(theKickEngineOutput), theMotionInfo(theMotionInfo),
      theKinematicRequest(theKinematicRequest), theJoystickControl(theJoystickControl),
      theJoystickImageControl(theJoystickImageControl), theWalkGeneratorData(theWalkGeneratorData),
