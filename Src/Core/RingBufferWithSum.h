 ...
 *
 * @note The original authors are Matthias Jüngel and Tobias Oberlies
 */
...
public:
   ...
   */
  inline C getEntry(int i) const { return buffer[(n + current - i) % n]; }

  /**
   * returns the sum of all entries
   */
  inline C getSum() const { return sum; }
   ...
   * \return a reference to the buffer entry
   */
  inline C operator[](int i) const { return buffer[(n + current - i) % n]; }
