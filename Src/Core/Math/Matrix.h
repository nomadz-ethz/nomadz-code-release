...
public:
   ...
   */
  inline Vector<m, V>& operator[](int i) { return c[i]; }
   ...
   * @return reference to column
   */
  inline const Vector<m, V>& operator[](int i) const { return c[i]; }
   ...
   */
  Matrix<n, n, V> operator*=(const Matrix<n, n, V>& other) { return *this = *this * other; }
   ...
   */
  Matrix<m, n, V> operator*(const V& factor) const { return Matrix<m, n, V>(*this) *= factor; }
   ...
   * @return The sum
   */
  Matrix<m, n, V> operator+(const Matrix<m, n, V>& other) const { return Matrix<m, n, V>(*this) += other; }
...
typedef Matrix<6, 3, float> Matrix6x3f;
typedef Matrix<3, 6, float> Matrix3x6f;
typedef Matrix<6, 1, float> Vector6f;

// For FlipM
typedef Matrix<2, 1, Vector6f> ObsError;
