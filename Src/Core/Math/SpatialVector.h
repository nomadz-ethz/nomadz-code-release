...
public:
  ...
  SpatialVector<V>(const Vector3<V>& top, const Vector3<V>& bottom, bool motion = true)
      : top(top), bottom(bottom), motion(motion) {}
  ...
  SpatialVector<V> operator*(const V factor) const { return SpatialVector<V>(*this) *= factor; }

  SpatialVector<V> operator/(const V factor) { return SpatialVector<V>(*this) /= factor; }
  ...
  SpatialVector<V> operator-(const SpatialVector<V>& other) { return SpatialVector<V>(*this) -= other; }

  SpatialVector<V> operator+(const SpatialVector<V>& other) const { return SpatialVector<V>(*this) += other; }
  ...
  SpatialVector<V> operator-() const { return SpatialVector<V>(-top, -bottom, motion); }
  ...
  static Matrix3x3<> crossProductMatrix(const Vector3<>& v) {
    return Matrix3x3<>(0.0f, -v.z, v.y, v.z, 0.0f, -v.x, -v.y, v.x, 0.0f);
  }
};
