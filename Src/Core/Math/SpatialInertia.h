...
public:
  ...
  SpatialInertia(const SpatialInertia& other)
      : momentOfInertia(other.momentOfInertia), centerOfMass(other.centerOfMass), mass(other.mass) {}
  ...
  SpatialInertia operator+(const SpatialInertia& other) const { return SpatialInertia(*this) += other; }
  ...
  SpatialVector<> operator*(const SpatialVector<>& vector) const {
    ...
    return SpatialVector<>((momentOfInertia * vector.top) + (massMoment ^ vector.bottom),
                           (vector.bottom * mass) - (massMoment ^ vector.top),
                           false);
  }

  bool operator==(const SpatialInertia& other) const {
    return mass == other.mass && momentOfInertia == other.momentOfInertia && centerOfMass == other.centerOfMass;
  }

  bool operator!=(const SpatialInertia& other) const { return !(*this == other); }
  ...
  SpatialVector<> inverseApply(const SpatialVector<>& vector) const {
    ...
    return SpatialVector<>(moiComInv * ncxTf, vector.bottom / mass + cx * (moiComInv * ncxTf));
  }
