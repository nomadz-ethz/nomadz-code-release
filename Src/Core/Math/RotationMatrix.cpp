...
// For the get X,Y,Z Angle, a convention is needed, Since the same Rotation matrix can be reached by different sequences of
// rotations. Here the Tait-Bryan angles z-y-x ( or yaw pitch and roll ) is used.
float RotationMatrix::getXAngle() const {
  RotationMatrix rotMatrixWithoutYawAndPitch =
    fromRotationY(-getYAngle()) * fromRotationZ(-getZAngle()) * RotationMatrix(c0, c1, c2);
  return atan2(rotMatrixWithoutYawAndPitch.c1.z, rotMatrixWithoutYawAndPitch.c1.y);
}

float RotationMatrix::getYAngle() const {
  RotationMatrix rotMatrixWithoutYaw = fromRotationZ(-getZAngle()) * RotationMatrix(c0, c1, c2);
  return -atan2(rotMatrixWithoutYaw.c0.z, rotMatrixWithoutYaw.c0.x);
}

float RotationMatrix::getZAngle() const {
  return atan2(c0.y, c0.x);
}
...
Vector3<> RotationMatrix::getAngleAxis() const {
  ...
  Vector3<> result(c1.z - c2.y, c2.x - c0.z, c0.y - c1.x);
