...
template <class V> class YaVector : public std::vector<V> {
  int N;

public:
  YaVector(size_t entries, const V& defaultValue) : std::vector<V>(entries, defaultValue), N(entries) {}
  ...
  operator V() const { return (*this)[0]; }
};
 ...
 */
template <class V = float> class YaMatrix {
public:
  int M, N;
  /**
   ...
   * (c) 2023 BHuman and NomadZ team
   *
   * @note The original author is <a href="mailto:stefanuhrig@gmx.net">Stefan Uhrig</a>
  ...
  YaMatrix<V>& operator-() { return *this *= V(-1); }
  ...
  std::vector<V>& operator[](size_t row) { return v[row]; }

  const std::vector<V>& operator[](size_t row) const { return v[row]; }
};

template <class V> bool operator==(const YaMatrix<V>& a, const YaMatrix<V>& b) {
  for (int m = 0; m < a.M; m++)
    for (int n = 0; n < a.N; n++)
      if (a.v[m][n] != b.v[m][n])
        return false;
 ...
 */
template <class V> YaMatrix<V> operator*(const YaMatrix<V>& a, const YaMatrix<V>& b) {
...
template <class V> YaMatrix<V> operator+=(YaMatrix<V>& a, const YaMatrix<V>& b) {
  return a = a + b;
}
 ...
 * \return product
 */
template <class V> YaMatrix<V> operator*(const V f, const YaMatrix<V>& a) {
 ...
 * \return product
 */
template <class V> YaMatrix<V> operator*(const YaMatrix<V>& a, const V f) {
  return f * a;
}

template <class V> YaMatrix<V>& operator*=(YaMatrix<V>& a, const V f) {
  return a = f * a;
}
 ...
 * \return division
 */
template <class V> YaMatrix<V> operator/(const YaMatrix<V>& a, const V d) {
  YaMatrix<V> b(a.M, a.N);
  for (int m = 0; m < a.M; m++)
    for (int n = 0; n < a.N; n++)
      b.v[m][n] = a.v[m][n] / d;
...
template <class T> bool isNaN(const YaMatrix<T>& value) {
...
template <class T> bool isInf(const YaMatrix<T>& value) {
  for (int m = 0; m < value.M; m++)
    for (int n = 0; n < value.N; n++)
      if (isInf(value[m][n]))
        return true;
...
template <class T> bool equals(const YaMatrix<T>& a, const YaMatrix<T>& b, T delta) {
  if (a.M != b.M || a.N != b.N)
    return false;
...
template <class T> YaMatrix<T> yaMatrix(int M, int N, T first, ...) {
...
template <class T> YaMatrix<T> yaVector(int N, T first, ...) {
