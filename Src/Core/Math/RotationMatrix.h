 ...
 * <a href="mailto:thomas.kindler@gmx.de">Thomas Kindler</a>,
 * and Max Risler
 */
...
public:
  ...
  template <typename U> inline RotationMatrix(U& ref, internal::ref_init_tag t) : Matrix3x3<>(ref, t) {}
  template <typename U> inline RotationMatrix(U& ref, internal::ref_alias_tag t) : Matrix3x3<>(ref, t) {}
   ...
   */
  RotationMatrix(const Vector3<>& c0, const Vector3<>& c1, const Vector3<>& c2) : Matrix3x3<>(c0, c1, c2) {}

  RotationMatrix(const float& yaw, const float& pitch, const float& roll) : Matrix3x3<>() {
  ...
  inline RotationMatrix invert() const {
    return RotationMatrix(Vector3<>(c0.x, c1.x, c2.x), Vector3<>(c0.y, c1.y, c2.y), Vector3<>(c0.z, c1.z, c2.z));
  }
   ...
   * This function is reimpletened here since the RotationMatrix * Matrix3x3<> reimplementation makes Matrix3x3<> * Vector3<>
   * inaccessible
  ...
  Vector3<> operator*(const Vector3<>& vector) const {
    return Vector3<>(c0.x * vector.x + c1.x * vector.y + c2.x * vector.z,
