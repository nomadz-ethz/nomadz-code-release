...
void GaussianDistribution2D::generateDistributionFromMeasurements(float* x, int numOfX, float* y, int numOfY) {
...
void GaussianDistribution2D::merge(const GaussianDistribution2D& other) {
  ...
  meanMatrix.c0.y = meanMatrix.c1.x = diffMean.x * diffMean.y;
...
bool GaussianDistribution2D::choleskyDecomposition(const Matrix2x2<float>& A, Matrix2x2<float>& L, float eps) const {
  float sum(A.c0.x);
  if (sum < -eps) {
    return false;
  }
...
void GaussianDistribution2D::getEigenVectorsAndEigenValues(Vector2<>& eVec1,
                                                           Vector2<>& eVec2,
                                                           float& eValue1,
                                                           float& eValue2) const {
  // Compute eigenvalues
  ...
  // | cm.c0.x - eValue  cm.c1.x          |  *  |x|   = |0|
  // | cm.c0.y           cm.c1.y - eValue |     |y|     |0|
  // First eigenvector (x set to 1):
  eVec1.x = 1.0f;
  eVec1.y = -covariance.c0.y / (covariance.c1.y - eValue1);
  eVec1.normalize();
  // Second eigenvector (y set to 1):
  eVec2.x = -covariance.c1.x / (covariance.c0.x - eValue2);
  eVec2.y = 1.0f;
  eVec2.normalize();
}
