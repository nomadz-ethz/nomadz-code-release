...
public:
  ...
  /** Constructor*/
  GaussianTable(int maxX, int xSteps, int minStdDev, int maxStdDev, int stdDevSteps) : table(0) {
  ...
  bool computeTable(int maxX, int xSteps, int minStdDev, int maxStdDev, int stdDevSteps) {
    if (!table || this->maxX != maxX || this->xSteps != xSteps + 1 || this->minStdDev != minStdDev ||
        this->maxStdDev != maxStdDev || this->stdDevSteps != stdDevSteps + 1) {
      this->maxX = maxX;
  ...
  void init() {
    ...
    for (int s = 1; s < stdDevSteps; s++)
      for (int x = 0; x < xSteps; x++) {
        float value = float((maxStdDev + minStdDev) / 2) *
                      gaussianProbability((float)(x * xSegmentWidth), (float)(s * stdDevSegmentWidth));
