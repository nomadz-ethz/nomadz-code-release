...
public:
  ...
  SpatialInertiaDerivative(const SpatialInertiaDerivative& other) : topLeft(other.topLeft), topRight(other.topRight) {}

  SpatialInertiaDerivative(const Matrix3x3<>& topLeft, const Matrix3x3<>& topRight) : topLeft(topLeft), topRight(topRight) {}
  ...
  SpatialVector<> operator*(const SpatialVector<>& vector) const {
    ...
    return SpatialVector<>(topLeft * vector.top + topRight * vector.bottom, bottomLeft * vector.top, false);
  }
