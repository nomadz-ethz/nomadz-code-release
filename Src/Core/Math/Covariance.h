...
namespace Covariance {
  ...
  inline const Matrix2x2<> create(const Vector2<>& dev, const float angle) {
    ...
    const Matrix2x2<> r = Matrix2x2<>(Vector2<>(cosRotation, sinRotation), Vector2<>(-sinRotation, cosRotation));
   ...
   */
  inline void
  errorEllipse(const Matrix2x2<>& covariance, float& axis1, float& axis2, float& angle, const float factor = 1.0f) {
    const float cov012 = covariance[0][1] * covariance[0][1];
    ...
    angle = 0.5f * std::atan2(2.0f * covariance[0][1], varianceDiff);
