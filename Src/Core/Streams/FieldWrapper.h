...
public:
  ...
  typename wrapped_type::iterator insert(typename wrapped_type::iterator pos, const T& value) {
    return ptr_->insert(pos, value);
  }

  template <typename InputIt>
  typename wrapped_type::iterator insert(typename wrapped_type::iterator pos, InputIt first, InputIt last) {
    return ptr_->insert(pos, first, last);
  }

  typename wrapped_type::iterator erase(typename wrapped_type::iterator pos) { return ptr_->erase(pos); }
...
public:
  ...
  typename std::vector<T>::iterator insert(typename std::vector<T>::iterator pos, T value) {
    ASSERT(vec_.size() == ptr_->size());
    ASSERT(ptr_->size() <= ptr_->capacity());
    // get the iterator
    auto ptr_it = ptr_->begin() + std::distance(vec_.begin(), pos);
    // insert into the underlying vector
    auto inserted_ptr_it = ptr_->insert(ptr_it, static_cast<typename T::ROSType>(value));
    // update the wrapper vector
    init_vec();
    // return the iterator to corresponding element in vec_
    return vec_.begin() + std::distance(ptr_->begin(), inserted_ptr_it);
  }

  template <typename InputIt>
  typename std::vector<T>::iterator insert(typename std::vector<T>::iterator pos, InputIt first, InputIt last) {
    ASSERT(vec_.size() == ptr_->size());
    ASSERT(ptr_->size() <= ptr_->capacity());
    // get the iterator
    auto ptr_it = ptr_->begin() + std::distance(vec_.begin(), pos);
    // insert into the underlying vector
    auto inserted_ptr_it = ptr_->insert(ptr_it, first, last);
    // update the wrapper vector
    init_vec();
    // return the iterator to corresponding element in vec_
    return vec_.begin() + std::distance(ptr_->begin(), inserted_ptr_it);
  ...
  typename std::vector<T>::iterator erase(typename std::vector<T>::iterator pos) {
    ...
    // get the iterator
    auto ptr_it = ptr_->begin() + std::distance(vec_.begin(), pos);
    // erase from the underlying vector
    auto erased_ptr_it = ptr_->erase(ptr_it);
    // update the wrapper vector
    init_vec();
    // return the iterator to corresponding element in vec_
    return vec_.begin() + std::distance(ptr_->begin(), erased_ptr_it);
  }
