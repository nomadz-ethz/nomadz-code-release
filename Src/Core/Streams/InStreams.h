...
public:
   ...
   * @param size The number of bytes to be read.
   */
  virtual void read(void* p, int size) { R::readData(p, size, *this); }

  /**
   * The function skips a number of bytes in the stream.
   * @param size The number of bytes to be skipped.
   */
  virtual void skip(int size) { R::skipData(size, *this); }
...
protected:
   ...
   * @param stream A stream to read from.
   */
  virtual void readData(void* p, int size, PhysicalInStream& stream) { stream.readFromStream(p, size); }
   ...
   * @param stream The stream to read from.
   */
  virtual void skipData(int size, PhysicalInStream& stream) { stream.skipInStream(size); }
};
...
public:
   ...
   * @return End of file reached?
   */
  virtual bool getEof() const { return (memory != 0 ? memory >= end : false); }
...
protected:
  ...
  void open(const void* mem, unsigned size = 0) {
    if (memory == 0) {
      memory = (const char*)mem;
      end = (const char*)mem + size;
    }
  }
...
public:
   ...
   */
  InTextMemory(const void* mem, unsigned size) {
    open(mem, size);
    initEof(*this);
  }
};
...
private:
  ...
  public:
    ...
    Entry(const char* key, const SimpleMap::Value* value, int type, const char* (*enumToString)(unsigned char))
