...
public:
   ...
   * @param size The number of bytes to be written.
   */
  virtual void write(const void* p, int size) { W::writeData(p, size, *this); }
...
protected:
   ...
   * @param stream the stream to write on.
   */
  virtual void writeData(const void* p, int size, PhysicalOutStream& stream) { stream.writeToStream(p, size); }
};
...
class OutMap : public Out {
  ...
  public:
    ...
    Entry(int type, const char* (*enumToString)(unsigned char))
