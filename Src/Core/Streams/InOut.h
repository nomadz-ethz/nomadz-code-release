...
namespace internal {
  template <typename T, typename = void> struct is_enum : public std::false_type {};
  template <typename T> struct is_enum<T, std::enable_if_t<std::is_enum<T>::value>> : public std::true_type {};

  template <typename T, typename = void> struct has_wrapped_type : public std::false_type {};
  template <typename T> struct has_wrapped_type<T, std::void_t<typename T::wrapped_type>> : public std::true_type {};
} // namespace internal
...
public:
  ...
  virtual void select(const char* name, int type, const char* (*enumToString)(unsigned char) = 0) {}
  ...
  template <class T> Out& operator<<(const T& t) {
    return EnumHelpers::EnumOrClass<T, ::internal::is_enum<T>::value>::write(*this, t);
  }
...
public:
  ...
  template <class T> In& operator>>(T& t) {
    return EnumHelpers::EnumOrClass<T, ::internal::is_enum<T>::value>::read(*this, t);
  }
...
namespace EnumHelpers {
   ...
   */
  template <class T, bool isEnum> struct EnumOrClass {
    __attribute__((no_sanitize("undefined"))) static Out& write(Out& out, const T& t) { return (Out2&)out << t; }
    __attribute__((no_sanitize("undefined"))) static In& read(In& in, T& t) { return (In2&)in >> t; }
  };
  ...
  template <class T> struct EnumOrClass<T, true> {
    static Out& write(Out& out, const T& t) { return sizeof(t) == 1 ? out << (unsigned char)t : out << (int)t; }
