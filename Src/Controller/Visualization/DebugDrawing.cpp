...
void DebugDrawing::arrow(Vector2<> start, Vector2<> end, Drawings::PenStyle penStyle, int width, ColorRGBA color) {
  ...
  line((int)(end.x),
       (int)(end.y),
       (int)(end.x - startToEnd.x + perpendicular.x),
       (int)(end.y - startToEnd.y + perpendicular.y),
       penStyle,
       width,
       color);
  line((int)(end.x),
...
void DebugDrawing::line(
  int xStart, int yStart, int xEnd, int yEnd, Drawings::PenStyle penStyle, int width, ColorRGBA penColor) {
  Line element;
...
void DebugDrawing::gridMono(
  int x, int y, int cellSize, int cellsX, int cellsY, const ColorRGBA& baseColor, unsigned char* cells) {
  GridMono element;
  element.x = x;
  element.y = y;
  element.cellSize = cellSize;
...
void DebugDrawing::dot(int x, int y, ColorRGBA penColor, ColorRGBA fillColor) {
  ...
  points[3].x = x - 1;
  points[3].y = y + 1;
  polygon(points, 4, 0, Drawings::ps_solid, penColor, Drawings::bs_solid, fillColor);
}
...
void DebugDrawing::largeDot(int x, int y, ColorRGBA penColor, ColorRGBA fillColor) {
  ...
  points[3].x = x - 25;
  points[3].y = y + 25;
  polygon(points, 4, 3, Drawings::ps_solid, penColor, Drawings::bs_solid, fillColor);
}
...
void DebugDrawing::midDot(int x, int y, ColorRGBA penColor, ColorRGBA fillColor) {
  ...
  points[3].x = x - 2;
  points[3].y = y + 2;
  polygon(points, 4, 1, Drawings::ps_solid, penColor, Drawings::bs_solid, fillColor);
}
...
bool DebugDrawing::addShapeFromQueue(InMessage& message, Drawings::ShapeType shapeType) {
  ...
  case Drawings::polygon: {
    ...
    this->polygon(
      points, numberOfPoints, penWidth, (Drawings::PenStyle)penStyle, penColor, (Drawings::FillStyle)fillStyle, fillColor);
    delete[] points;
  } break;
  ...
  case Drawings::arrow: {
    ...
    message.bin >> penColor;
    this->arrow(
      Vector2<>((float)x1, (float)y1), Vector2<>((float)x2, (float)y2), (Drawings::PenStyle)penStyle, penWidth, penColor);
  } break;
  case Drawings::dot: {
    int x, y;
    ColorRGBA penColor, fillColor;
...
const DebugDrawing::Element* DebugDrawing::getNext(const Element* element) const {
  ...
  case Element::GRID_RGBA:
    element = (const Element*)((const char*)element + sizeof(GridRGBA) +
                               sizeof(ColorRGBA) * ((const GridRGBA*)element)->cellsX * ((const GridRGBA*)element)->cellsY);
    break;
  case Element::GRID_MONO:
    element = (const Element*)((const char*)element + sizeof(GridMono) +
                               ((const GridMono*)element)->cellsX * ((const GridMono*)element)->cellsY);
    break;
  case Element::TEXT:
