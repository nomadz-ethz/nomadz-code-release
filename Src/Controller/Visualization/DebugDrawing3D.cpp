 ...
 *
 * @note The original authors are Philippe Schober
...
void DebugDrawing3D::draw2() {
  ...
  // Custom rotation.
  if (rotateX != 0) {
    glRotated(toDegrees(rotateX), 1, 0, 0);
  }
  if (rotateY != 0) {
    glRotated(toDegrees(rotateY), 0, 1, 0);
  }
  if (rotateZ != 0) {
    glRotated(toDegrees(rotateZ), 0, 0, 1);
  }
...
void DebugDrawing3D::line(
  float xStart, float yStart, float zStart, float xEnd, float yEnd, float zEnd, float width, ColorRGBA color) {
  Line element;
...
void DebugDrawing3D::polygon(const Vector3<>* points, float width, ColorRGBA color) {
  Polygon element;
  for (int i = 0; i < 3; i++) {
...
void DebugDrawing3D::dot(Vector3<> v, float w, ColorRGBA color) {
  Dot element;
  element.point = v;
  element.color = color;
  element.width = w;
...
void DebugDrawing3D::sphere(Vector3<> v, float r, ColorRGBA color) {
  Sphere element;
  element.point = v;
  element.color = color;
  element.radius = r;
...
void DebugDrawing3D::cylinder(Vector3<> v, Vector3<> rot, float baseRadius, float topRadius, float h, ColorRGBA color) {
  Cylinder element;
  element.point = v;
...
void DebugDrawing3D::image(Vector3<> v, Vector3<> rot, float w, float h, Image* i) {
  Image3D element;
  element.point = v;
  element.rotation = rot;
  element.image = i;
