 ...
 *
 * @note The original author is <a href="mailto:Thomas.Roefer@dfki.de">Thomas Röfer</a> and Colin Graf
 */
...
ImageWidget::ImageWidget(ImageView& imageView)
    : imageView(imageView), imageData(0), imageWidth(maxResolutionWidth), imageHeight(maxResolutionHeight),
      lastImageTimeStamp(0), lastColorReferenceTimeStamp(0), lastDrawingsTimeStamp(0), dragStart(-1, -1), zoom(1.f),
      offset(0, 0), headControlMode(false) {
...
void ImageWidget::paint(QPainter& painter) {
  {
    ...
    const Image* image = 0;
    RobotConsole::Images& currentImages =
      imageView.upperCam ? imageView.console.upperCamImages : imageView.console.lowerCamImages;
...
void ImageWidget::paintDrawings(QPainter& painter) {
  ...
  for (std::list<std::string>::const_iterator i = drawings.begin(), end = drawings.end(); i != end; ++i) {
    const DebugDrawing& debugDrawing(imageView.upperCam ? imageView.console.upperCamImageDrawings[*i]
                                                        : imageView.console.lowerCamImageDrawings[*i]);
...
void ImageWidget::copyImage(const Image& srcImage) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (; cur < end; ++cur) {
      ...
      *p++ = (r < 0 ? 0 : r > 255 ? 255 : r) << 16 | (g < 0 ? 0 : g > 255 ? 255 : g) << 8 | (b < 0 ? 0 : b > 255 ? 255 : b) |
             0xff000000;
    }
  }
...
void ImageWidget::copyImageSegmented(const Image& srcImage) {
  ...
  unsigned currentTime = Time::getCurrentSystemTime();
  ...
  static const unsigned baseColors[] = {0xffff7f00, 0xffffff00, 0xff0000ff, 0xffffffff, 0xff00ff00, 0xff000000, 0xffff0000};
...
bool ImageWidget::needsRepaint() const {
  ...
  return (image && (image->timeStamp != lastImageTimeStamp ||
                    (imageView.segmented && imageView.console.colorReferenceTimeStamp != lastColorReferenceTimeStamp))) ||
         (imageView.segmented && imageView.console.colorReference.changed) || (!image && lastImageTimeStamp);
}
...
void ImageWidget::handleHeadControl(QMouseEvent& event) {
  if (headControlMode) {
    ...
    command << "set parameters:ManualHeadMotionProvider xImg = " << x << "; yImg = " << y
...
QMenu* ImageWidget::createUserMenu() const {
  ...
  QAction* colorButtons[] = {greenAct, yellowAct, orangeAct, redAct, blueAct, whiteAct, blackAct, allColorsAct};
...
ImageView::ImageView(const QString& fullName,
                     ...
                     bool upperCam,
                     float gain)
    : upperCam(upperCam), fullName(fullName), icon(":/Icons/tag_green.png"), console(console), background(background),
      name(name), segmented(segmented), gain(gain), isActImage(strcmp(name.c_str(), "act") == 0) {}
