...
private:
  ...
  virtual void paint(QPainter& painter) {
    ...
    painter.setTransform(
      QTransform(scale, 0, 0, -scale, size.width() / 2. - offset.x() * scale, size.height() / 2. - offset.y() * scale));
  ...
  void mouseMoveEvent(QMouseEvent* event) {
...

    {
      ...
      if (text) {
        ...
        setToolTip(
          QString("x=%1, y=%2").arg(pos.rx()).arg(-pos.ry())); // Field y-coords are mirrored from mouse-event coords
      }
    }
  }
  ...
  QSize sizeHint() const {
    return QSize(int(fieldDimensions.xPosOpponentFieldBorder * 0.2f), int(fieldDimensions.yPosLeftFieldBorder * 0.2f));
  }
