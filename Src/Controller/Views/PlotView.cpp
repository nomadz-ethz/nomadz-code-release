...
PlotWidget::PlotWidget(PlotView& plotView, PlotWidget*& plotWidget)
    : plotView(plotView), plotWidget(plotWidget), lastTimeStamp(0), blackPen(QColor(0x00, 0x00, 0x00), 0.2),
      grayPen(QColor(0xbb, 0xbb, 0xbb), 0.2), drawUnits(true), drawLegend(true), antialiasing(false) {
...
bool PlotWidget::needsRepaint() const {
  ...
  for (std::list<RobotConsole::Layer>::const_iterator i = plotView.console.plotViews[plotView.name].begin(),
                                                      end = plotView.console.plotViews[plotView.name].end();
       i != end;
       ++i) {
...
void PlotWidget::paint(QPainter& painter) {
  ...
  // draw axis lables
  {
    ...
    // y
    {
      ...
      rect.moveTop((plotView.valueLength - (plotView.maxValue - plotView.minValue)) * plotRect.height() /
                     plotView.valueLength +
                   topMargin - textHeight / 2.);
      painter.drawText(rect, Qt::AlignRight, tr(buf));
      sprintf(buf, "%g", plotView.minValue);
      rect.moveTop((plotView.valueLength - (plotView.minValue - plotView.minValue)) * plotRect.height() /
                     plotView.valueLength +
                   topMargin - textHeight / 2.);
      painter.drawText(rect, Qt::AlignRight, tr(buf));
      rect.moveTop((plotView.valueLength - (0. - plotView.minValue)) * plotRect.height() / plotView.valueLength + topMargin -
                   textHeight / 2.);
      ...
      for (float pos = stepY; pos <= plotView.maxValue - minOffset; pos += stepY) {
        sprintf(buf, "%g", pos);
        rect.moveTop((plotView.valueLength - (pos - plotView.minValue)) * plotRect.height() / plotView.valueLength +
                     topMargin - textHeight / 2.);
        painter.drawText(rect, Qt::AlignRight, tr(buf));
      }
      for (float pos = -stepY; pos >= plotView.minValue + minOffset; pos -= stepY) {
        sprintf(buf, "%g", pos);
        rect.moveTop(((plotView.maxValue - plotView.minValue) - (pos - plotView.minValue)) * plotRect.height() /
                       plotView.valueLength +
                     topMargin - textHeight / 2.);
        painter.drawText(rect, Qt::AlignRight, tr(buf));
      }
  ...
  // setup plot-paint-transformation
  {
    ...
    transform.translate(plotRect.right(),
                        plotRect.top() + float(plotRect.height()) * plotView.maxValue / plotView.valueLength);
  ...
  grayPen.setWidthF(std::min(1.0f, (plotView.maxValue - plotView.minValue) / 100));
  ...
  grayPen.setWidthF(plotSizeF / 100);
  for (float pos = stepX; pos < plotSizeF; pos += stepX) {
    painter.drawLine(QPointF(pos, plotView.minValue), QPointF(pos, plotView.maxValue));
  }
  blackPen.setWidthF(std::min(1.0f, (plotView.maxValue - plotView.minValue) / 100));
  painter.setPen(blackPen);
...

  {
    ...
    for (std::list<RobotConsole::Layer>::const_iterator i = plotList.begin(), end = plotList.end(); i != end; ++i) {
      plotView.console.plots[i->layer].maxPlotSize = plotView.plotSize;
      ...
      if (numOfPoints > 1) {
        ...
        for (int j = plotView.plotSize - numOfPoints; j < int(plotView.plotSize); ++j) {
          plotView.points[j].setY(*(k++));
        }
        const ColorRGBA& color(i->color);
        QPen colorPen(QColor(color.r, color.g, color.b));
        colorPen.setWidthF(std::min(2.0f, (plotView.maxValue - plotView.minValue) / 50));
    ...
    if (drawLegend && plotList.size() > 0) {
      QRect legendRect((int)(plotRect.left() + space),
                       (int)(plotRect.top() + space),
                       (int)(legendWidth + space * 3 + 10),
                       (int)(space + plotList.size() * (textHeight + space)));
      QRect rect((int)(legendRect.left() + space + 10 + space),
                 (int)(legendRect.top() + space),
                 (int)(legendRect.width() - (space + 10 + space)),
                 (int)textHeight);
      QLine line((int)(legendRect.left() + space),
                 (int)(legendRect.top() + space + textHeight / 2 + 1),
                 (int)(legendRect.left() + space + 10),
                 (int)(legendRect.top() + space + textHeight / 2 + 1));
      ...
      for (std::list<RobotConsole::Layer>::const_iterator i = plotList.begin(), end = plotList.end(); i != end; ++i) {
        ...
        const ColorRGBA& color(i->color);
        QPen colorPen(QColor(color.r, color.g, color.b), 10);
        painter.setPen(colorPen);
...
void PlotWidget::exportAsGnuplot() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
    this, tr("Export as Gnuplot"), settings.value("ExportDirectory", "").toString(), tr("Gnuplot (*.plt)"));
  if (fileName.isEmpty()) {
    return;
  }
...

  {
    const std::list<RobotConsole::Layer>& plotList(plotView.console.plotViews[plotView.name]);
    int currentPlot = 0;
    for (std::list<RobotConsole::Layer>::const_iterator i = plotList.begin(), end = plotList.end(); i != end;
         ++i, ++currentPlot) {
      const std::list<float>& list = plotView.console.plots[i->layer].points;
...
PlotView::PlotView(const QString& fullName,
    ...
    : fullName(fullName), icon(":/Icons/tag_green.png"), console(console), plotWidget(0), name(name), plotSize(plotSize),
      minValue(minValue), maxValue(maxValue), valueLength(maxValue - minValue), yUnit(yUnit), xUnit(xUnit), xScale(xScale) {
...
void PlotView::setParameters(
  unsigned int plotSize, float minValue, float maxValue, const std::string& yUnit, const std::string& xUnit, float xScale) {
  delete[] points;
  points = new QPointF[plotSize];
  for (unsigned int i = 0; i < plotSize; ++i) {
    points[i].rx() = plotSize - 1 - i;
