...
bool ViewBikeMath::intersectRayAndBox(const Vector3<>& rayPosition,
                                      const Vector3<>& ray,
...
bool ViewBikeMath::intersectRayAndPlane(
...
void ViewBikeMath::transformRayToObjectAtOrigin(const Vector3<>& rayPosition,
                                                const Vector3<>& ray,
...
bool ViewBikeMath::intersectRayWithAxisAlignedPlane(const Vector3<>& p,
                                                    const Vector3<>& v,
                                                    float distance,
                                                    int thirdAxis,
                                                    float maxAbsAxis1,
  ...
  if (vv[thirdAxis] != 0.0) {
    ...
    i = (thirdAxis + 2) % 3;
    intersectionPosv[i] = s * vv[i] + pv[i];
    if (std::abs(intersectionPosv[i]) > maxAbsAxis2) {
      return false;
    }
    return true;
  } else {
...
Pose3D ViewBikeMath::calculateFootPos(const SensorData& sensorData,
...
Pose3D ViewBikeMath::calculateHandPos(const JointData& jointData,
                                      const JointData::Joint& joint,
                                      const RobotDimensions& robotDimensions) {
  Pose3D handPos;
...
Vector3<> ViewBikeMath::calcFootPos(const float& leg0,
                                    const float& leg1,
                                    const float& leg2,
                                    const float& leg3,
                                    const float& leg4,
