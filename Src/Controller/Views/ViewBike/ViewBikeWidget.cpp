...
ViewBikeWidget::ViewBikeWidget(ViewBike& viewBike, KickEngineParameters& parameters, ViewBikeHeaderedWidget* parent)
    : QWidget(parent), parent(parent), glWidget(new ViewBikeGLWidget(viewBike, parameters, this)), viewBike(viewBike),
      parameters(parameters), phaseDrawings(true), singleDraw(false), reachedDraw(false), tra2dWindows(false),
      tra1dWindows(false), velocityWindows(false), accelWindows(false), followMode(false), dragPlane(0, 1, 0), getString(4),
      ghost(0), mirror(false) {
  // Vertical Layouts
...
void ViewBikeWidget::removePhase() {
  ...
  if (phaseNumber > -1) {
    ...
    if (it != parameters.phaseParameters.end()) {
      disconnect(*(phaseTab.begin() + phaseNumber),
                 SIGNAL(itemChanged(QStandardItem*)),
                 this,
                 SLOT(updatePhaseParameters(QStandardItem*)));
...
void ViewBikeWidget::makeNewPhaseWithModelAndTree(const int& phaseNumber) {
  ...
  if (phaseNumber > 0) {
    ...
    for (int point = 0; point < NUM_OF_POINTS; ++point) {
      newPhase.controlPoints[Phase::rightFootTra][point] =
        Vector3<>(parameters.footOrigin.x, -parameters.footOrigin.y, parameters.footOrigin.z);
      newPhase.controlPoints[Phase::leftFootTra][point] = parameters.footOrigin;
      newPhase.controlPoints[Phase::rightFootRot][point] =
        Vector3<>(-parameters.footRotOrigin.x, parameters.footRotOrigin.y, -parameters.footRotOrigin.z);
      newPhase.controlPoints[Phase::leftFootRot][point] = parameters.footRotOrigin;
      newPhase.controlPoints[Phase::rightArmTra][point] =
        Vector3<>(parameters.armOrigin.x, -parameters.armOrigin.y, parameters.armOrigin.z);
      newPhase.controlPoints[Phase::leftArmTra][point] = parameters.armOrigin;
      newPhase.controlPoints[Phase::rightHandRot][point] =
        Vector3<>(-parameters.handRotOrigin.x, parameters.handRotOrigin.y, -parameters.handRotOrigin.z);
  ...
  if (phaseNumber > 0) // if there is a phase before copy last parameters
  {
    for (int limb = 0; limb < Phase::numOfLimbs; ++limb) {
      ...
      for (int point = 0; point < (int)NUM_OF_POINTS; ++point) {
        parameters.phaseParameters[phaseNumber].controlPoints[limb][point] =
          parameters.phaseParameters[phaseNumber - 1].controlPoints[limb][NUM_OF_POINTS - 1];
        parameters.phaseParameters[phaseNumber].comTra[point] =
          parameters.phaseParameters[phaseNumber - 1].comTra[NUM_OF_POINTS - 1];
      }
      ...
      if (phaseNumber < parameters.numberOfPhases - 1) {
        ...
        parameters.phaseParameters[phaseNumber + 1].comTra[0] =
          parameters.phaseParameters[phaseNumber].comTra[2] - parameters.phaseParameters[phaseNumber].comTra[1];
        ...
        parameters.phaseParameters[phaseNumber + 1].headTra[0] =
          parameters.phaseParameters[phaseNumber].headTra[2] - parameters.phaseParameters[phaseNumber].headTra[1];
      ...
      parameters.phaseParameters[phaseNumber].comTra[0] =
        parameters.phaseParameters[phaseNumber - 1].comTra[2] - parameters.phaseParameters[phaseNumber - 1].comTra[1];
      ...
      parameters.phaseParameters[phaseNumber].headTra[0] =
        parameters.phaseParameters[phaseNumber - 1].headTra[2] - parameters.phaseParameters[phaseNumber - 1].headTra[1];
...
void ViewBikeWidget::playWholeMotion() {
  if (parameters.numberOfPhases > 0) {
    ...
    int idxKickMotion = viewBike.motionRequestCommand.find("kickMotionType", idxSpecialAction);
    int idxAutoProceed = viewBike.motionRequestCommand.find("autoProceed", idxKickMotion);
    todo << viewBike.motionRequestCommand.substr(0, idxMotion + 9) << "kick; ";
    todo << viewBike.motionRequestCommand.substr(idxSpecialAction, idxKickMotion + 17 - idxSpecialAction)
         << "newKick; mirror = ";
    todo << (mirror ? "true" : "false") << "; " << viewBike.motionRequestCommand.substr(idxAutoProceed);
...
void ViewBikeWidget::playMotion(int phase) {
  std::string setNewKickMotion =
    "set module:KickEngine:newKickMotion footOrigin = { x = " + floatToStr(parameters.footOrigin.x) +
    "; y = " + floatToStr(parameters.footOrigin.y) + "; z = " + floatToStr(parameters.footOrigin.z) +
    "; }; footRotOrigin = { x = " + floatToStr(parameters.footRotOrigin.x) +
    "; y = " + floatToStr(parameters.footRotOrigin.y) + "; z = " + floatToStr(parameters.footRotOrigin.z) +
    "; }; armOrigin = { x = " + floatToStr(parameters.armOrigin.x) + "; y = " + floatToStr(parameters.armOrigin.y) +
    "; z = " + floatToStr(parameters.armOrigin.z) + "; }; handRotOrigin = { x = " + floatToStr(parameters.handRotOrigin.x) +
    "; y = " + floatToStr(parameters.handRotOrigin.y) + "; z = " + floatToStr(parameters.handRotOrigin.z) +
    "; }; comOrigin = { x = " + floatToStr(parameters.comOrigin.x) + "; y = " + floatToStr(parameters.comOrigin.y) +
    "; }; headOrigin = { x = " + floatToStr(parameters.headOrigin.x) + "; y = " + floatToStr(parameters.headOrigin.y) +
    "; }; kpx = " + floatToStr(parameters.kpx) + "; kix = " + floatToStr(parameters.kix) +
    "; kdx = " + floatToStr(parameters.kdx) + "; kpy = " + floatToStr(parameters.kpy) +
    "; kiy = " + floatToStr(parameters.kiy) + "; kdy = " + floatToStr(parameters.kdy) +
    "; loop = " + boolToStr(parameters.loop) + "; standLeft = " + boolToStr(parameters.standLeft) +
    "; adjustKickFootPosition = " + intToStr(parameters.adjustKickFootPosition) +
    "; ignoreHead = " + boolToStr(parameters.ignoreHead) + "; phaseParameters = [";
  ...
  for (int i = 0; i < phase; i++) {
    ...
    setNewKickMotion +=
      " { duration = " + intToStr(parameters.phaseParameters[i].duration) + "; " +
      ...
      "; y = " + floatToStr(parameters.phaseParameters[i].controlPoints[Phase::rightHandRot][2].y) +
      "; z = " + floatToStr(parameters.phaseParameters[i].controlPoints[Phase::rightHandRot][2].z) + "; }; " +
      "comTra1 = { x = " + floatToStr(parameters.phaseParameters[i].comTra[1].x) +
      "; y = " + floatToStr(parameters.phaseParameters[i].comTra[1].y) + "; }; " +
      "comTra2 = { x = " + floatToStr(parameters.phaseParameters[i].comTra[2].x) +
      "; y = " + floatToStr(parameters.phaseParameters[i].comTra[2].y) + "; }; " +
      "headTra1 = { x = " + floatToStr(parameters.phaseParameters[i].headTra[1].x) +
      "; y = " + floatToStr(parameters.phaseParameters[i].headTra[1].y) + "; }; " +
      "headTra2 = { x = " + floatToStr(parameters.phaseParameters[i].headTra[2].x) +
      "; y = " + floatToStr(parameters.phaseParameters[i].headTra[2].y) + "; }; " +
      "odometryOffset = { x = " + floatToStr(parameters.phaseParameters[i].odometryOffset.x) +
      "; y = " + floatToStr(parameters.phaseParameters[i].odometryOffset.y) +
      "; z = " + floatToStr(parameters.phaseParameters[i].odometryOffset.z) + "; }; }";
  }
...
void ViewBikeWidget::updateCommon() {
  ...
  rootItem->child(10, 1)->setData(QVariant(parameters.standLeft), Qt::DisplayRole);
  rootItem->child(11, 1)->setData(QVariant(parameters.loop), Qt::DisplayRole);
  rootItem->child(12, 1)->setData(QVariant(parameters.adjustKickFootPosition), Qt::DisplayRole);
...
void ViewBikeWidget::updateEditorView() {
  ...
  if (!phaseTab.empty()) {
    for (itModel = phaseTab.begin(); itModel < phaseTab.end(); itModel++) {
      ...
      disconnect(((QStandardItemModel*)(*itModel)),
                 SIGNAL(itemChanged(QStandardItem*)),
                 this,
                 SLOT(updatePhaseParameters(QStandardItem*)));
      delete *itModel;
...
void ViewBikeWidget::fillModelWithPhaseData(int i) {
  if (i < parameters.numberOfPhases) {
    ...
    for (unsigned int k = 0; k < NUM_OF_POINTS; k++) {
      rootItem->child(5, 0)->child(0, 0)->child(k, 2)->setData(QVariant(parameters.phaseParameters[i].comTra[k].x),
                                                               Qt::DisplayRole);
      rootItem->child(5, 0)->child(0, 0)->child(k, 3)->setData(QVariant(parameters.phaseParameters[i].comTra[k].y),
                                                               Qt::DisplayRole);
      rootItem->child(6, 0)->child(0, 0)->child(k, 2)->setData(QVariant(parameters.phaseParameters[i].headTra[k].x),
                                                               Qt::DisplayRole);
      rootItem->child(6, 0)->child(0, 0)->child(k, 3)->setData(QVariant(parameters.phaseParameters[i].headTra[k].y),
                                                               Qt::DisplayRole);
    }
...
void ViewBikeWidget::updateCommonParameters(QStandardItem* item) {
  ...
  if (!mamaItem) {
    if (row == 10) {
      parameters.standLeft = item->data(Qt::DisplayRole).toInt();
    }
    if (row == 11) {
      parameters.loop = item->data(Qt::DisplayRole).toInt();
    }
    if (row == 12) {
      parameters.adjustKickFootPosition = item->data(Qt::DisplayRole).toInt();
    }
    if (row == 13) {
...
void ViewBikeWidget::recordPose() {
  ...
  if (phaseNumber > -1) {
    ...
    if (lfrot->isChecked()) {
      ...
      RotationMatrix footRot = RotationMatrix()
                                 .rotateX((jointData.angles[joint + 1] + pi_4) * -sign)
                                 .rotateY(jointData.angles[joint + 2] + jointData.angles[joint + 3]);
    ...
    if (lftra->isChecked()) {
      p = ViewBikeMath::calculateFootPos(
