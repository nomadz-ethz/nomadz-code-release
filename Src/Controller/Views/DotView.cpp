...
void DotViewWidget::exportAsPdf() {
  QSettings& settings = RoboCupCtrl::application->getSettings();
  QString fileName = QFileDialog::getSaveFileName(
    this, tr("Export as PDF"), settings.value("ExportDirectory", "").toString(), tr("Portable Document Format (*.pdf)"));
  if (fileName.isEmpty()) {
    return;
  }
  settings.setValue("ExportDirectory", QFileInfo(fileName).dir().path());
...
void DotViewWidget::exportAsDot() {
  QSettings& settings = RoboCupCtrl::application->getSettings();
  QString fileName = QFileDialog::getSaveFileName(
    this, tr("Export as DOT File"), settings.value("ExportDirectory", "").toString(), tr("Graphviz Graph File (*.dot)"));
  if (fileName.isEmpty()) {
    return;
  }
  settings.setValue("ExportDirectory", QFileInfo(fileName).dir().path());
