...
public:
  FootWidget(FootView& footView, const RobotBalance& robotBalance)
      : footView(footView), robotBalance(robotBalance), ankleMark(-5.0f, -5.0f, 10.0f, 10.0f), maxFootLength(175.0f),
        maxFootWidth(110.0f) {
...
private:
  ...
  virtual void paint(QPainter& painter) {
    ...
    rightFootShape = QRectF(1.5f * footSpacing + maxFootWidth, 0.5f * footSpacing, maxFootWidth, maxFootLength);
...
FootView::FootView(const QString& fullName, RobotConsole& console, const RobotBalance& robotBalance, const std::string& name)
    : fullName(fullName), icon(":/Icons/tag_green.png"), console(console), robotBalance(robotBalance), name(name) {}
