...
public:
  ThresholdSelector(const QString& name, ColorCalibrationWidget* parent, const int min, const int max);
...
public:
  MinRSelector(const QString& name, ColorCalibrationWidget* parent, const int min, const int max)
      : ThresholdSelector(name, parent, min, max) {}

  void updateColorReference(const int value, ColorReference::ColorThreshold<>& color) { color.first = slider->value(); }

  void updateWidgetsPrivate(ColorReference::ColorThreshold<>& color) { slider->setValue(color.first); }
};

class MinBSelector : public ThresholdSelector {
public:
  MinBSelector(const QString& name, ColorCalibrationWidget* parent, const int min, const int max)
      : ThresholdSelector(name, parent, min, max) {}

  void updateColorReference(const int value, ColorReference::ColorThreshold<>& color) { color.second = slider->value(); }

  void updateWidgetsPrivate(ColorReference::ColorThreshold<>& color) { slider->setValue(color.second); }
};

class MinRBSelector : public ThresholdSelector {
public:
  MinRBSelector(const QString& name, ColorCalibrationWidget* parent, const int min, const int max)
      : ThresholdSelector(name, parent, min, max) {}

  void updateColorReference(const int value, ColorReference::ColorThreshold<>& color) { color.third = slider->value(); }

  void updateWidgetsPrivate(ColorReference::ColorThreshold<>& color) { slider->setValue(color.third); }
};
