...
public:
  RangeSelector(
    const QString& name, ColorCalibrationWidget* parent, const float min, const float max, const unsigned int decimals);
...
private:
  ...
  virtual void updateColorReference(const float min, const float max, ColorReference::HSVColorDefinition& color) = 0;
  ...
  public:
    Validator(float bottom, float top, int decimals, QObject* parent) : QDoubleValidator(bottom, top, decimals, parent) {
  ...
  public:
    NumberInput(int init, QWidget* parent, float min, float max, int decimals) : QLineEdit(QString::number(init), parent) {
      setFixedWidth(40);
...
public:
  ...
  void updateColorReference(const float min, const float max, ColorReference::HSVColorDefinition& color) {
    color.hue.min = min;
    color.hue.max = max;
  }
