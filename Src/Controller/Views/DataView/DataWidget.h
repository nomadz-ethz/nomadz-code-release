...
public:
  ...
  DataWidget(DataView& view, QtVariantPropertyManager& manager)
      : theView(view), pTheBrowser(new TreePropertyBrowser(this)), pTheCurrentProperty(NULL), theEditorFactory(&view),
        theManager(manager), theRootPropertyHasChanged(false) {
  ...
  QSize sizeHint() const { return QSize(350, 800); }
  ...
  void setUnchangedButtonEnabled(bool value) { pUnchangedAction->setEnabled(value); }

  void setSetButtonEnabled(bool value) { pSetAction->setEnabled(value); }

private slots:
  /**
   * Is called when the set button has been pressed
   */
  void setPressed() { theView.set(); }

  void autoSetToggled(bool checked) { theView.setAutoSet(checked); }
