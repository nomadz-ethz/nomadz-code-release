...
DataView::DataView(const QString& fullName, const std::string& repName, RobotConsole& console, StreamHandler& streamHandler)
    : theFullName(fullName), theIcon(":/Icons/tag_green.png"), theConsole(console), theName(repName), pTheWidget(NULL),
      theStreamHandler(streamHandler), lastUpdated(0), theAutoSetModeIsEnabled(true),
      theUpdateTime(50) // 20 times per second
{}                      // FIXME destructor
...
QtVariantProperty*
DataView::getProperty(const std::string& fqn, int propertyType, const QString& name, QtProperty* pParent) {
