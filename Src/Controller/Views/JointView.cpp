...
public:
  ...
  void paintEvent(QPaintEvent* event) {
    ...
    paintRectField3 = QRect(
      headerView->sectionViewportPosition(3) + textOffset, 0, headerView->sectionSize(3) - textOffset * 2, lineSpacing);
    paintRectField4 = QRect(
      headerView->sectionViewportPosition(4) + textOffset, 0, headerView->sectionSize(4) - textOffset * 2, lineSpacing);
    {
      ...
      for (int i = 0; i < JointData::numOfJoints; ++i) {
        if (i == JointData::LShoulderPitch || i == JointData::RShoulderPitch || i == JointData::LHipYawPitch ||
            i == JointData::RHipYawPitch) {
          newSection();
        }
        jointRequest.angles[i] == JointData::off ? (void)strcpy(request, "off")
                                                 : (void)sprintf(request, "%.1f°", toDegrees(jointRequest.angles[i]));
        jointData.angles[i] == JointData::off ? (void)strcpy(sensor, "?")
                                              : (void)sprintf(sensor, "%.1f°", toDegrees(jointData.angles[i]));
...
private:
  ...
  QSize sizeHint() const { return QSize(400, 800); }
};
...
JointView::JointView(const QString& fullName,
                     RobotConsole& robotConsole,
                     const JointData& jointData,
                     const SensorData& sensorData,
                     const JointData& jointRequest)
    : fullName(fullName), icon(":/Icons/tag_green.png"), console(robotConsole), jointData(jointData), sensorData(sensorData),
