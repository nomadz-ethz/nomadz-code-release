 ...
 *
 * @note The original authors are <a href="mailto:Thomas.Roefer@dfki.de">Thomas Röfer</a>, Jeff and Colin Graf
 */
...
class SensorViewWidget : public QWidget {
public:
  SensorViewWidget(SensorView& sensorView, QHeaderView* headerView, QWidget* parent)
  ...
  bool formatSensor(Type type, float inValue, bool filtered, char* value, SensorData::Sensor sensorType) const {
    ...
    switch (type) {
    case SensorViewWidget::distance:
      sprintf(value, "%.1f mm", inValue);
      break;
    case SensorViewWidget::pressure:
      sprintf(value, "%.1f g", inValue * 1000);
      break;
    case SensorViewWidget::acceleration:
      sprintf(value, "%.2f m/s²", inValue);
      break;
    case SensorViewWidget::batteryLevel:
      sprintf(value, "%.1f V", inValue);
      break;
    case SensorViewWidget::angularSpeed:
      sprintf(value, "%.1f °/s", toDegrees(inValue));
      break;
    case SensorViewWidget::ratio:
      sprintf(value, "%.1f %%", inValue * 100);
      break;
    case SensorViewWidget::angle2:
      sprintf(value, "%.1f °", toDegrees(inValue));
      break;
    case SensorViewWidget::usFromLeftDistance:
    case SensorViewWidget::usFromRightDistance:
  ...
  void paintEvent(QPaintEvent* event) {
    static const ViewInfo viewInfoNao[] = {
      {SensorData::gyroX, angularSpeed, false},     {SensorData::gyroY, angularSpeed, false},
      {SensorData::gyroZ, angularSpeed, true},

      {SensorData::accX, acceleration, false},      {SensorData::accY, acceleration, false},
      {SensorData::accZ, acceleration, true},

      {SensorData::batteryLevel, ratio, true},

      {SensorData::fsrLFL, pressure, false},        {SensorData::fsrLFR, pressure, false},
      {SensorData::fsrLBL, pressure, false},        {SensorData::fsrLBR, pressure, false},
      {SensorData::fsrRFL, pressure, false},        {SensorData::fsrRFR, pressure, false},
      {SensorData::fsrRBL, pressure, false},        {SensorData::fsrRBR, pressure, true},

      {SensorData::usL, usFromLeftDistance, false}, {SensorData::usL, usFromRightDistance, false},
      {SensorData::usR, usFromLeftDistance, false}, {SensorData::usR, usFromRightDistance, true},

      {SensorData::angleX, angle2, false},          {SensorData::angleY, angle2, false},
    ...
    paintRectField0 = QRect(
      headerView->sectionViewportPosition(0) + textOffset, 0, headerView->sectionSize(0) - textOffset * 2, lineSpacing);
    paintRectField1 = QRect(
      headerView->sectionViewportPosition(1) + textOffset, 0, headerView->sectionSize(1) - textOffset * 2, lineSpacing);
    paintRectField2 = QRect(
      headerView->sectionViewportPosition(2) + textOffset, 0, headerView->sectionSize(2) - textOffset * 2, lineSpacing);
    {
      char value[200];
      char filtered[200];
      SYNC_WITH(sensorView.console);
...
public:
  SensorHeaderedWidget(SensorView& sensorView, RobotConsole& console) {
    ...
    sensorWidget = new SensorViewWidget(sensorView, headerView, this);
    setWidget(sensorWidget);
  }

private:
  SensorViewWidget* sensorWidget;
  virtual QWidget* getWidget() { return this; }
  virtual void update() { sensorWidget->update(); }
};

SensorView::SensorView(const QString& fullName,
                       RobotConsole& console,
                       const SensorData& sensorData,
                       const SensorData& filteredSensorData)
    : fullName(fullName), icon(":/Icons/tag_green.png"), console(console), sensorData(sensorData),
