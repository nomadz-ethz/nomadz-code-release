...
public:
  TeamComm3DWidget(TeamComm3DView& view, QHeaderView* headerView, QWidget* parent)
      : QWidget(parent), view(view), headerView(headerView), baseBrush(QPalette().base()), noPen(Qt::NoPen) {
  ...
  void paintEvent(QPaintEvent* event) {
    ...
    paintRectField1 = QRect(
      headerView->sectionViewportPosition(1) + textOffset, 0, headerView->sectionSize(1) - textOffset * 2, lineSpacing);
    ...
    for (std::map<unsigned int, TeamComm3DCtrl::RobotData>::iterator iter = teamListener.robotData.begin(),
                                                                     end = teamListener.robotData.end();
         iter != end;
         ++iter) {
      ...
      std::string robotName = robotData.robotHealth.robotName;
      if (!robotName.empty()) {
        sprintf(formatedValue,
                "%u.%u.%u.%u (%s)",
                (ipAddress >> 24) & 0xff,
                (ipAddress >> 16) & 0xff,
                (ipAddress >> 8) & 0xff,
      ...
      print("Ping, Packet Last Received",
            formatedValue,
            std::min(computeRegularity(robotData.ping, 100.f, 1000.f),
                     computeRegularity(now - robotData.timeStamp, expectedPacketDelay + 50.f, expectedPacketDelay + 100.f)));
      // sprintf(formatedValue, "%d ms", int(robotData.lastPacketLatency));
      // print("Packet Latency", formatedValue);
      unsigned int oldestPacketTimeStamp =
        robotData.packetTimeStamps.getEntry(robotData.packetTimeStamps.getNumberOfEntries() - 1);
      float traffic =
        now != oldestPacketTimeStamp ? float(robotData.packetSizes.getSum() * 1000) / float(now - oldestPacketTimeStamp) : 0;
      totalTraffic += traffic;
      float averagePackageSize =
        float(robotData.packetSizes.getSum()) / float(robotData.packetSizes.getNumberOfEntries()) - 28.f;
      sprintf(formatedValue, "%.0f bytes/s, 28 + %.1f bytes", traffic, averagePackageSize);
      print("Traffic, Average Packet Size",
            formatedValue,
            std::min(computeRegularity(traffic, 5000.f, 10000.f), computeRegularity(averagePackageSize, 500.f, 1000.f)));
      ...
      // float expectedPacketDelay = robotData.robotNumber != 0 ? 100.f : 2000.f;
      // print("Packet Last Received", formatedValue, computeRegularity(now - robotData.timeStamp, expectedPacketDelay +
      // 50.f, expectedPacketDelay + 100.f));
      newBlock();
      ...
      if (robotData.robotNumber != 0) {
        ...
        if (!robotName.empty()) {
          ...
          sprintf(formatedValue,
                  "%.1f %.1f %.1f, %hhu%%",
                  loadAverage1,
                  float(robotData.robotHealth.load[1]) / 10.f,
                  float(robotData.robotHealth.load[2]) / 10.f,
                  (unsigned char)robotData.robotHealth.memoryUsage);
          print("Load Average, Memory Usage",
                formatedValue,
                std::min(computeRegularity(loadAverage1, 1.f, 1.1f),
                         computeRegularity(robotData.robotHealth.memoryUsage, 50.f, 90.f)));
          newBlock();

          sprintf(formatedValue,
                  "%.1f fps, %.1f fps",
                  float(robotData.robotHealth.cognitionFrameRate),
                  float(robotData.robotHealth.motionFrameRate));
          print("Cognition, Motion Rate",
                formatedValue,
                std::min(computeRegularity(robotData.robotHealth.cognitionFrameRate, 29.f, 28.f),
                         computeRegularity(robotData.robotHealth.motionFrameRate, 99.f, 98.f)));
          newBlock();

          sprintf(formatedValue,
                  "%hhu%%, %hhu°C",
                  (unsigned char)robotData.robotHealth.batteryLevel,
                  (unsigned char)robotData.robotHealth.maxJointTemperature);
          print("Battery Level, Max Temperature",
                formatedValue,
                std::min(computeRegularity(robotData.robotHealth.batteryLevel, 30.f, 10.f),
                         computeRegularity(robotData.robotHealth.maxJointTemperature, 60.f, 80.f)));
          newBlock();

          if (robotData.robotHealthTimeStamps.getNumberOfEntries() > 0) {
            unsigned int oldestRobotHealthTimeStamp =
              robotData.robotHealthTimeStamps.getEntry(robotData.robotHealthTimeStamps.getNumberOfEntries() - 1);
            unsigned int newestRobotHealthTimeStamp = robotData.robotHealthTimeStamps.getEntry(0);
            unsigned int oldestBallPerceptCount =
              robotData.ballPercepts.getEntry(robotData.ballPercepts.getNumberOfEntries() - 1);
            unsigned int oldestLineAnalysisCount =
              robotData.lineAnalyses.getEntry(robotData.lineAnalyses.getNumberOfEntries() - 1);
            unsigned int oldestGoalPerceptCount =
              robotData.goalPercepts.getEntry(robotData.goalPercepts.getNumberOfEntries() - 1);
            sprintf(formatedValue,
                    "%.1f, %.1f, %.1f",
                    float(robotData.robotHealth.ballPercepts - oldestBallPerceptCount) /
                      (float(newestRobotHealthTimeStamp - oldestRobotHealthTimeStamp) * 0.001f / 60.f),
                    float(robotData.robotHealth.goalPercepts - oldestGoalPerceptCount) /
                      (float(newestRobotHealthTimeStamp - oldestRobotHealthTimeStamp) * 0.001f / 60.f),
                    float(robotData.robotHealth.lineAnalyses - oldestLineAnalysisCount) /
                      (float(newestRobotHealthTimeStamp - oldestRobotHealthTimeStamp) * 0.001f / 60.f));
            print("Ball-, Goal-, LineAnalyses / min", formatedValue, 1.f);
            newBlock();
          }
        }
        ...
        // sprintf(formatedValue, robotData.goalPercept.timeWhenOwnGoalLastSeen ||
        // robotData.goalPercept.timeWhenOppGoalLastSeen ? "%d ms" : "never", std::min(int(now -
        // robotData.goalPercept.timeWhenOwnGoalLastSeen), int(now - robotData.goalPercept.timeWhenOppGoalLastSeen)));
        ...
        if (robotData.robotPose.deviation == RobotPose::unknownDeviation) {
          ...
          sprintf(formatedValue, "%.1f mm", static_cast<double>(robotData.robotPose.deviation));
        }
        print("Pose deviation",
              formatedValue,
              computeRegularity(robotData.robotPose.deviation, 100.f, RobotPose::unknownDeviation));
        newBlock();

        sprintf(formatedValue,
                "%s%s",
                BehaviorStatus::getName(robotData.behaviorStatus.role),
                robotData.isPenalized ? " (Penalized)" : "");
...
private:
  ...
  void indent(int count) { paintRectField0.setLeft(defaultLeft + 10 * count); }
  ...
  void print(const char* name, const char* value, float regularity, bool bold = false, bool rightAlign = false) {
    ...
    bgColorGradient.setColorAt(1.f,
                               QColor(std::max(std::min(int(255.f * (1.f - regularity)), 255), 0),
                                      std::max(std::min(int(255.f * regularity), 255), 0),
                                      0));
    ...
    if (name) {
      ...
      painter.drawText(paintRectField0,
                       ((!value /*|| rightAlign*/) ? Qt::TextDontClip : 0) | Qt::TextSingleLine | Qt::AlignVCenter,
                       tr(name));
      if (bold) {
        painter.setFont(font);
      }
    ...
    if (value) {
      painter.drawText(
        paintRectField1, (rightAlign ? Qt::AlignRight : Qt::AlignLeft) | Qt::TextSingleLine | Qt::AlignVCenter, tr(value));
    }
  ...
  QSize sizeHint() const { return QSize(300, 800); }
};
