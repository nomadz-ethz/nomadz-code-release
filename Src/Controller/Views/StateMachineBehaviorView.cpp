/**
 * @file StateMachineBehaviorView.cpp
 *
...
public:
  StateMachineBehaviorWidget(StateMachineBehaviorView& stateMachineBehaviorView, QWidget* parent)
      : QWidget(parent), stateMachineBehaviorView(stateMachineBehaviorView), noPen(Qt::NoPen) {
  ...
  void paintEvent(QPaintEvent* event) {
    ...
    paintRectField0 = QRect(defaultLeft, 0, paintRect.right() - textOffset * 2, lineSpacing);
    {
      ...
      for (std::vector<ActivationGraph::ActivationGraphNode>::const_iterator i = info.graph.begin(), end = info.graph.end();
           i != end;
           ++i) {
        const ActivationGraph::ActivationGraphNode& activeOption = *i;
        paintRectField0.setLeft(defaultLeft + 10 * activeOption.depth);

        sprintf(formatedTime, "%.02f", float(activeOption.optionTime) / 1000.f);
        print(static_cast<std::string>(activeOption.option).c_str(), formatedTime, true, true);

        paintRectField0.setLeft(paintRectField0.left() + 5);
        sprintf(formatedTime, "%.02f", float(activeOption.stateTime) / 1000.f);
        print(static_cast<std::string>(activeOption.state).c_str(), formatedTime, false, true);
        newBlock();
      }
    }
    painter.end();
...
private:
  ...
  void print(const char* name, const char* value, bool bold = false, bool rightAlign = false) {
    ...
    if (value) {
      painter.drawText(
        paintRectField0, (rightAlign ? Qt::AlignRight : Qt::AlignLeft) | Qt::TextSingleLine | Qt::AlignVCenter, tr(value));
    }
...
StateMachineBehaviorView::StateMachineBehaviorView(const QString& fullName,
                                                   RobotConsole& console,
                                                   const ActivationGraph& activationGraph,
                                                   const unsigned& timeStamp)
    : fullName(fullName), icon(":/Icons/tag_green.png"), console(console), activationGraph(activationGraph),
      timeStamp(timeStamp) {}
