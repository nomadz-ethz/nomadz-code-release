...
bool TeamComm3DCtrl::compile() {
  ...
  for (unsigned i = 0, count = application->getObjectChildCount(*group); i < count; ++i) {
    ...
    puppetData.initialPosition =
      Vector3<>(initialPosition[0] * 1000.f, initialPosition[1] * 1000.f, initialPosition[2] * 1000.f);
  ...
  debugRequestTable.addRequest(DebugRequest("debug drawing 3d:"
                                            "representation:SideConfidence"));
  debugRequestTable.addRequest(DebugRequest("debug drawing 3d:"
                                            "representation:BallModel"));
  debugRequestTable.addRequest(DebugRequest("debug drawing 3d:"
      ...
      if (puppetData.robot) {
        ...
        if (!robotData || (int)(now - robotData->timeStamp) > (int)teamMateData.networkTimeout || robotData->isPenalized) {
          ...
          if (puppetData.online) {
            ...
            for (std::unordered_map<const char*, DebugDrawing3D>::iterator i = puppetData.drawings3D.begin(),
                                                                           end = puppetData.drawings3D.end();
                 i != end;
                 ++i) {
              i->second.reset();
            }
          ...
          if (puppetData.updateTimeStamp != robotData->timeStamp || !robotData->hasGroundContact) {
            ...
            puppetData.oracle.moveRobot(
              Vector3<>(robotPose.translation.x,
                        robotPose.translation.y,
                        robotData->hasGroundContact ? puppetData.initialPosition.z : puppetData.initialPosition.z + 600.f),
              Vector3<>(0, 0, robotPose.rotation),
              true);
          }
        ...
        for (int i = 0; i < JointData::numOfJoints; ++i) {
          needPhysics |= puppetData.jointData.angles[i] != JointData::off &&
                         std::abs(puppetData.jointData.angles[i] - jointData.angles[i]) > 0.001f;
        }
...
bool TeamComm3DCtrl::handleMessage(InMessage& message) {
  ...
  case idNTPHeader:
    ...
    message.resetReadPosition();
    {
      ...
      currentRobotData->lastPacketLatency =
        currentListener->ntp.receiveTimeStamp -
        currentListener->ntp.getRemoteTimeInLocalTime(currentListener->ntp.sendTimeStamp);
  ...
  case idTeamMateHasGroundContact:
    ...
    currentRobotData->hasGroundContact = hasGroundContact;
    return true;
  ...
  case idTeamMatePlayerModel: {
    ...
    currentRobotData->playerModel = playerModelCompressed;
    for (size_t i = 0; i < currentRobotData->playerModel.players.size(); i++) {
      currentRobotData->playerModel.players[i].timeStamp =
        currentListener->ntp.getRemoteTimeInLocalTime(currentRobotData->playerModel.players[i].timeStamp);
    }
    return true;
  }
  ...
  case idTeamMateBallModel: {
    ...
    if (currentRobotData->ballModel.timeWhenLastSeen) {
      currentRobotData->ballModel.timeWhenLastSeen =
        currentListener->ntp.getRemoteTimeInLocalTime(currentRobotData->ballModel.timeWhenLastSeen);
    }
    if (currentRobotData->ballModel.timeWhenDisappeared) {
      currentRobotData->ballModel.timeWhenDisappeared =
        currentListener->ntp.getRemoteTimeInLocalTime(currentRobotData->ballModel.timeWhenDisappeared);
    }
    return true;
  }
  case idTeamMateBehaviorStatus: {
    ...
    int teamColor = behaviorStatus.teamColor == BehaviorStatus::red
                      ? red
                      : behaviorStatus.teamColor == BehaviorStatus::blue ? blue : numOfTeamColors;
  ...
  case idRobotHealth:
    ...
    currentRobotData->lineAnalyses.add(currentRobotData->robotHealth.lineAnalyses);
  ...
  case idTeamMateGoalPercept: {
    ...
    if (goalPercept.timeWhenGoalPostLastSeen) {
      goalPercept.timeWhenGoalPostLastSeen =
        currentListener->ntp.getRemoteTimeInLocalTime(goalPercept.timeWhenGoalPostLastSeen);
    }
    if (goalPercept.timeWhenCompleteGoalLastSeen) {
      goalPercept.timeWhenCompleteGoalLastSeen =
        currentListener->ntp.getRemoteTimeInLocalTime(goalPercept.timeWhenCompleteGoalLastSeen);
    }
    return true;
  }
  //  case idTeamMateBallAfterKickPose: //Imported from BH2012
  //  {
  //    BallAfterKickPose& ballAfterKickPose = currentRobotData->ballAfterKickPose;
  //    message.bin >> ballAfterKickPose;
  //    ballAfterKickPose.timeWhenLastKickWasPerformed =
  //    currentListener->ntp.getRemoteTimeInLocalTime(ballAfterKickPose.timeWhenLastKickWasPerformed);
  //    return true;
  //  }
  //  case idTeamMatePassTarget: //Imported from BH2012
  //    message.bin >> currentRobotData->passTarget;
  //    return true;
  //  case idTeamMateDefenderData: //NEW
  //    message.bin >> currentRobotData->defenderData;
  //    return true;
  case idLineAnalysis:
    message.bin >> currentRobotData->lineAnalysis;
    return true;
  ...
  case idDebugDrawing3D: {
    ...
    currentRobotData->puppetData->drawings3D[drawingManager3D.getDrawingName(id)].addShapeFromQueue(
      message, (::Drawings3D::ShapeType)shapeType, 0);
  }
    return true;
  }
  return false;
