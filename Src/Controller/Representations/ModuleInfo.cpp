 ...
 *
 * @note The original author is is <a href="mailto:Thomas.Roefer@dfki.de">Thomas Röfer</a>
 */
...
std::string ModuleInfo::sendRequest(Out& stream, bool sort) const {
  ...
  for (std::list<Provider>::const_iterator i = providers.begin(); i != providers.end(); ++i) {
    if (i->selected != "") {
      ...
      for (std::vector<std::string>::const_iterator k = j->requirements.begin(); k != j->requirements.end(); ++k) {
        ...
        for (m = std::find(providers.begin(), providers.end(), provider); m != providers.end() && m->selected == "";
             m = std::find(++m, providers.end(), provider)) {
          ;
        }
        if (m == providers.end()) {
          // no provider in same process
          for (m = std::find(providers.begin(), providers.end(), *k); m != providers.end() && m->selected == "";
               m = std::find(++m, providers.end(), *k)) {
            ;
          }
          if (m == providers.end()) {
            return "Error: no provider for required representation " + *k + " which is required by " + i->selected +
                   " for providing " + i->representation;
          }
        }
      }
    }
  ...
  for (std::list<Provider>::const_iterator i = providers.begin(); i != providers.end(); ++i) {
    if (i->selected != "") {
      config.representationProviders.push_back(
        ModuleManager::Configuration::RepresentationProvider(i->representation, i->selected));
    }
  }
