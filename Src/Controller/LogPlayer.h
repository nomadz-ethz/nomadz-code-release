...
public:
   ...
   * @param fileName the name of one file to write, all files will be enumerated by appending a 3 digit number to the
   * filename.
   * @return if the writing of all files was successful
   */
  bool saveImages(const bool raw, const char* fileName, const bool upper, const bool lower);
...
private:
  ...
  int numberOfMessagesWithinCompleteFrames{}; /**< The number of messages within complete frames. Messages behind that number
                                             will be skipped. */
  bool loop{};
