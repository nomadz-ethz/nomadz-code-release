...
public:
  ...
  RoboticAnnotator* roboticAnnotator;

  MessageQueue& getDebugOut() { return debugOut; }
...
protected:
  ...
  StreamHandler streamHandler; /**< Local stream handler. Note: Process::streamHandler may be accessed unsynchronized in
                                  different thread, so don't use it here. */
  ...
  PlayerModel playerModel;                                                 /**< Player model from team communication. */
  ...
  LineAnalysis lineAnalysis;             /**< Line percept from team communication. */
  ...
  int lastBehaviorTime;                  /**< The optionTime value of the root node of the last ActivationGraph received. */
  unsigned playerModelReceived,          /**< When was the player model received from team communication. */
    ...
    lineAnalysisReceived,                /**< When was the line percept received from team communication. */
...
public:
  ...
  Drawings3D lowerCamDrawings3D; /**< Buffers for 3d drawings from the debug queue. */
  ...
  Views fieldViews,      /**< The map of all field views. */
    ...
    videoViews;          /**< The map of all video views. The second part (std::list<std::string>) is unused. */
  ...
  /**
   * A MessageHandler that parses debug responses containing representation data and forwards them to the correct
   * representation view
   */
  ...
  RepViewWriter repViewWriter; /**< The repViewWriter which is used to translate representations into a format that can be
                                  understood by the representation views */
...
private:
  ...
  std::string getOrSetWaitsFor;    /**< The name of the representation get or set are waiting for. If empty, they are not
                                      waiting for any. */
...
public:
  ...
  /**
   * The function removes all views under this robot's name from the scene graph.
   */
  void removeViews();
...
private:
  ...
  bool viewData(
    In& stream); /**< Creates a new representation view. Stream should contain the name of the debug data to display. */
  bool viewBike();
