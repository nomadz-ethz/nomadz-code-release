...
RemoteRobot::RemoteRobot(const char* name, const char* ip)
    : RobotConsole(theDebugReceiver, theDebugSender), theDebugReceiver(this, "Receiver.MessageQueue.O"),
      theDebugSender(this, "Sender.MessageQueue.S"), bytesTransfered(0), transferSpeed(0), timeStamp(0) {
...
void RemoteRobot::update() {
  ...
  if (puppet) {
    JointData dummy;
    oracle.getAndSetJointData(jointData, dummy);
    if (moveOp != noMove) {
      if (moveOp == moveBoth) {
  ...
  std::string statusText =
    std::string(robotName.mid(robotName.lastIndexOf(".") + 1).toUtf8().constData()) + ": connected to " + ip + ", " + buf;
