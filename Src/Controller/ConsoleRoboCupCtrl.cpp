...
#include "Views/CABSLGraphView.h"
#include "Views/TeachView/ClassifierView.h"
#include "Views/TeachView/DatasetView.h"
#include "Views/TeachView/GroupView.h"
#include "Views/TeachView/SampleView.h"
#include "Views/TeachView/TeachView.h"
#include "Views/TeachView/TrainView.h"
...
ConsoleRoboCupCtrl::ConsoleRoboCupCtrl(SimRobot::Application& application)
    : RoboCupCtrl(application), mode(SystemCall::simulatedRobot), calculateImage(true), calculateImageFps(FRAMES_PER_SECOND),
      globalNextImageTimeStamp(0), INIT_TEAM_COMM, timeStamp(0), robotNumber(-1), toolBar(*this) {
  ...
  representationToFile["parameters:ShapePerceptor"] = "shapePerceptor.cfg";
  representationToFile["parameters:DribblingParameters"] = "DribblingParameters.cfg";
  representationToFile["parameters:SelfLocator"] = "selfLocator.cfg";
  representationToFile["parameters:CameraSettingsRandomizer"] = "cameraSettingsRandomizer.cfg";
}
...
bool ConsoleRoboCupCtrl::compile() {
  ...
  std::string fileName = application->getFilePath()
                         ...
                         ()
                           .constData();
  int p = fileName.find_last_of("\\/"), p2 = fileName.find_last_of(".");
  if (p2 > p) {
...
void ConsoleRoboCupCtrl::update() {
  ...
  Global::theStreamHandler = &streamHandler;
  ntp.doSynchronization(Time::getRealSystemTime(), theTeamSender.out);
  theTeamHandler.send();
...
void ConsoleRoboCupCtrl::executeFile(std::string name, bool printError, RobotConsole* console) {
  if (nesting == 10) {
    ...
    ++nesting;
    auto targetFileName = QFileInfo(QString::fromStdString(name)).fileName();

    const auto directory = std::string(File::getBHDir()) + "/Config/Scenes";
    QDirIterator it(QString::fromStdString(directory), QStringList() << "*.con", QDir::Files, QDirIterator::Subdirectories);

    while (it.hasNext()) {
      QString filename = it.next();
      QFileInfo fileInfo(filename);

      if (fileInfo.baseName() == targetFileName) {
...
void ConsoleRoboCupCtrl::executeConsoleCommand(std::string command, RobotConsole* console) {
  ...
  } else if (buffer == "teach") {
    if (!addTeachView(stream)) {
      printLn("Failed to add Teach view.");
    }
...
void ConsoleRoboCupCtrl::help(In& stream) {
  ...
  list("  su <name> <number> : Starts a UDP team connection to a remote robot with a certain player number. Requires "
       "command tc.",
       pattern,
       true);
  ...
  list("  gc initial | ready | set | playing | finished | ... : Set "
       "GameController state.",
       pattern,
       true);
  list("  help | ? [<pattern>] : Display this text.", pattern, true);
  list("  robot ? | all | <name> {<name>} : Connect console to a set of active robots. Alternatively, double click on "
       "robot.",
       pattern,
       true);
  ...
  list("  ac ? | [<imageView>] (lower | upper | both) : Change the camera that is used in the specified image view. / "
       "Change the process that provides drawings in the field and 3d views.",
       pattern,
       true);
  ...
  list("  jc hide | show | motion <num> <command> | ( press | release ) <button> <command> : Set joystick motion (use "
       "$1 .. "
       "$6) or button command.",
       pattern,
       true);
  list("  jm <axis> ( off | <button> <button> ) : Map two buttons on an axis.", pattern, true);
  list("  js <axis> <speed> <threshold> [<center>] : Set axis maximum speed and ignore threshold for \"jc motion <num>\" "
       "commands.",
       pattern,
       true);
  list("  log start | stop | clear | save <file> | full | jpeg : Record log file and (de)activate image compression.",
       pattern,
       true);
  list("  log saveImages (upper | lower | ) (raw) (upper|lower| ) <file> : Save images from log.", pattern, true);
  list("  log saveTiming <file> : Save timing data from log to csv.", pattern, true);
  list("  log ? | load <file> | ( keep | remove ) <message> {<message>} : Load, filter, and display information about log "
       "file.",
       pattern,
       true);
  list("  log start | pause | stop | forward [image] | backward [image] | repeat | goto <number> | cycle | once | "
       "fast_forward | fast_rewind : Replay log file.",
       pattern,
       true);
  list("  mof : Recompile motion net and send it to the robot. ", pattern, true);
  list("  msg off | on | log <file> | enable | disable : Switch output of text messages on or off. Log text messages to a "
       "file. Switch message handling on or off.",
       pattern,
       true);
  ...
  list("  pr none | ballHolding | playerPushing | inactivePlayer | illegalDefender | leavingTheField | playingWithHands | "
       "requestForPickup : Penalize robot.",
       pattern,
       true);
  list("  qfr queue | replace | reject | collect <seconds> | save [<seconds>] : Send queue fill request.", pattern, true);
  list("  set ? [<pattern>] | <key> ( ? | unchanged | diff | <data> ) : Change debug data or show its specification.",
       pattern,
       true);
  ...
  list("  teach [<lesson>] : Add the Teach view to annotate data and train classifiers. Optionally loads "
       "/Config/Lessons/<lesson>.lesson .",
       pattern,
       true);
  ...
  list("  vfd ? [<pattern>] | <name> ( ? [<pattern>] | <drawing> ( on | off ) ) : (De)activate debug drawing in field "
       "view.",
       pattern,
       true);
  list(
    "  vi ? [<pattern>] | <image> [jpeg] [segmented] [upperCam] [<name>] [gain <value>] : Add image view.", pattern, true);
  list("  vid ? [<pattern>] | <name> ( ? [<pattern>] | <drawing> ( on | off ) ) : (De)activate debug drawing in image "
       "view.",
       pattern,
       true);
  ...
  list("  vpd ? [<pattern>] | <name> ( ? [<pattern>] | <drawing> ( ? [<pattern>] | <color> [<description>] | off ) ) : "
       "Plot "
       "data in a certain color in plot view.",
       pattern,
       true);
  list("  vv <name> : Add a video player view under the given name.", pattern, true);
...
bool ConsoleRoboCupCtrl::startLogFile(In& stream) {
  ...
  logFile = fileName;
  this->robotName = robotName.c_str();
  robots.push_back(new ControllerRobot(name.c_str()));
  this->robotName = 0;
  logFile = "";
  selected.clear();
...
ControllerRobot* ConsoleRoboCupCtrl::spawnAnnotatorRobot(std::string name = "Annotator") {
  std::string robotName = "." + name;
  mode = SystemCall::logfileReplay;
  logFile = "non-existent log file :)"; // hacking the log file replay mechanism to create a subservient annotator robot

  this->robotName = robotName.c_str();
  ControllerRobot* robot = new ControllerRobot(name.c_str());
  this->robotName = 0;
  ...
  // TODO For some reason, command tab completion doesn't work on newly spawned robot until something like "get thing" is
  // run
  //      Figure out why; don't just run "get thing" on every new spawn!
}

void ConsoleRoboCupCtrl::removeAnnotatorRobot(ControllerRobot* robot) {
  // Undo what spawnAnnotatorRobot did, in reverse order
  robot->announceStop();
  robot->stop();
  selected.remove(robot->getRobotProcess());
  robots.remove(robot);
  delete robot;
}
...
bool ConsoleRoboCupCtrl::addTeachView(In& stream) {
  if (!teachView) {
    teachView = new TeachView("Teach", *this);
    addView(teachView, nullptr, 0);
    addView(new ClassifierView("Teach.classifiers", *this, *teachView), teachView, 0);
    addView(new DatasetView("Teach.datasets", *this, *teachView), teachView, 0);
    addView(new GroupView("Teach.groups", *this, *teachView), teachView, 0);
    addView(new SampleView("Teach.samples", *this, *teachView), teachView, 0);
    addView(new TrainView("Teach.train", *this, *teachView), teachView, 0);
  }

  std::string subcommand;
  stream >> subcommand;

  teachView->handleConsole(subcommand);
...
void ConsoleRoboCupCtrl::createCompletion() {
  const char* commands[] = {"ac both",
                            ...
                            "log save",
                            "log saveImages",
                            "log saveImages upper",
                            "log saveImages lower",
                            "log saveTiming",
                            "log saveImages raw",
                            "log saveImages raw upper",
                            "log saveImages raw lower",
                            "log clear",
                            "log full",
                            "log jpeg",
                            "log ?",
  ...
  addCompletionFiles("log load ", std::string(File::getBHDir()) + "/Config/Logs", std::string("*.log"));
  addCompletionFiles("log save ", std::string(File::getBHDir()) + "/Config/Logs", std::string("*.log"));

  addCompletionFiles("call ", std::string(File::getBHDir()) + "/Config/Scenes", std::string("*.con"));

  addCompletionFiles("teach ", std::string(File::getBHDir()) + "/Config/Lessons", std::string("*.lesson"));
  ...
  if (debugRequestTable) {
    for (int i = 0; i < debugRequestTable->currentNumberOfDebugRequests; ++i) {
      ...
      if (debugRequestTable->debugRequests[i].description.substr(0, 13) == "debug images:") {
        completion.insert(std::string("v3 ") + translate(debugRequestTable->debugRequests[i].description.substr(13)) +
                          " jpeg");
        completion.insert(std::string("vi ") + translate(debugRequestTable->debugRequests[i].description.substr(13)) +
                          " jpeg segmented");
        completion.insert(std::string("vi ") + translate(debugRequestTable->debugRequests[i].description.substr(13)) +
                          " segmented");
      } else if (debugRequestTable->debugRequests[i].description.substr(0, 11) == "debug data:") {
        completion.insert(std::string("vd ") + translate(debugRequestTable->debugRequests[i].description.substr(11)) +
                          " off");
        completion.insert(std::string("vd ") + translate(debugRequestTable->debugRequests[i].description.substr(11)) +
                          " on");
  ...
  if (drawingManager) {
    ...
    for (std::unordered_map<const char*, DrawingManager::Drawing>::const_iterator i = drawingManager->drawings.begin();
         i != drawingManager->drawings.end();
         ++i) {
  ...
  if (videoViews) {
    for (const auto& i : *videoViews) {
      completion.insert(std::string("vv ") + i.first);
    }
  }

  for (std::unordered_map<std::string, std::string>::const_iterator i = representationToFile.begin();
       i != representationToFile.end();
       ++i) {
...
void ConsoleRoboCupCtrl::addCompletionFiles(const std::string& command,
                                            const std::string& directory,
                                            const std::string& pattern) {
  QDirIterator it(
    QString::fromStdString(directory), QStringList() << pattern.c_str(), QDir::Files, QDirIterator::Subdirectories);

  while (it.hasNext()) {
    auto filename = it.next();
    QFileInfo fileInfo(filename);
    completion.insert(command + fileInfo.baseName().toStdString());
  }
}
