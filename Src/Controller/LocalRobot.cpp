...
LocalRobot::LocalRobot()
    : RobotConsole(theDebugReceiver, theDebugSender), theDebugReceiver(this, "Receiver.MessageQueue.O"),
      theDebugSender(this, "Sender.MessageQueue.S"), image(false), nextImageTimeStamp(0), imageLastTimeStampSent(0),
      jointLastTimeStampSent(0), updatedSignal(1), puppet(0) {
  ...
  if (mode == SystemCall::logfileReplay) {
    ...
    logPlayer.play();
    puppet =
      (SimRobotCore2::Body*)RoboCupCtrl::application->resolveObject("RoboCup.puppets." + robotName, SimRobotCore2::body);
    if (puppet) {
      oracle.init(puppet);
    }
  } else if (mode == SystemCall::simulatedRobot) {
    SimRobotCore2::Body* robot =
      (SimRobotCore2::Body*)RoboCupCtrl::application->resolveObject(RoboCupCtrl::getRobotFullName(), SimRobotCore2::body);
    ASSERT(robot);
    oracle.init(robot);
...
void LocalRobot::update() {
  ...
  // Only one thread can access *this now.
  {
    ...
    if (mode == SystemCall::logfileReplay) {
      ...
      if (puppet) {
        oracle.getAndSetJointData(RobotConsole::jointData, jointData);
      }
    }
    ...
    if (mode == SystemCall::simulatedRobot) {
      ...
      if (now >= nextImageTimeStamp) {
        ...
        oracle.getWorldState(worldState);
        oracle.toggleCamera();
      } else {
        oracle.getRobotPose(robotPose);
      }
    ...
    if (!statusText.empty()) {
      ((ConsoleRoboCupCtrl*)ConsoleRoboCupCtrl::controller)
        ->printStatusText((robotName + ": " + statusText.c_str()).toUtf8().constData());
    }
  }
