...
Cognition::Cognition()
    : INIT_DEBUGGING, INIT_RECEIVER(MotionToCognition), INIT_SENDER(CognitionToMotion), INIT_TEAM_COMM,
      moduleManager(categories, sizeof(categories) / sizeof(*categories)), logger(moduleManager) {
  ...
  theTeamReceiver.setSize(4 * 2 * 1450); // >= 4 packages
...
void Cognition::init() {
  ...
  START_TEAM_COMM;
#ifdef ENABLE_ROS
  std::string node_name = "Cognition_" + Global::getSettings().robot + "_" +
  ...
  ros_node = std::make_shared<rclcpp::Node>(node_name, node_namespace);
...
#ifdef TARGET_ROBOT
  static bool imageSubscribersSet = false;
  if (!imageSubscribersSet) {
    imageSubscribersSet = CameraProvider::setupImageSubscribers(ros_node);
  }
#endif
#endif
  ...
  if (CognitionLogDataProvider::isFrameDataComplete() && CameraProvider::isFrameDataComplete() &&
    ...
    DEBUG_RESPONSE("process:Cognition:jointDelay", {
      if (&Blackboard::theInstance->theFrameInfo && &Blackboard::theInstance->theFilteredJointData) {
        OUTPUT(idText,
               text,
               Blackboard::theInstance->theFrameInfo.getTimeSince(Blackboard::theInstance->theFilteredJointData.timeStamp));
      }
    });
    ...
    theCognitionToMotionSender.timeStamp = Time::getCurrentSystemTime();
    ...
    if (theDebugSender.getNumberOfMessages() > numberOfMessages + 1) {
      ...
      if (&Blackboard::theInstance->theCameraInfo &&
          Blackboard::theInstance->theCameraInfo.camera == CameraInfo::lower) { // lower camera -> process called 'd'
...
bool Cognition::handleMessage(InMessage& message) {
  ...
  default:
    return CognitionLogDataProvider::handleMessage(message) || Process::handleMessage(message);
  }
