...
v0.1.3 20120108 rsz Created work-around for unique id generation with IDGenerator that avoids retarded c++ translation unit
linker errors with single-header static variables. Forced inline on all methods to please retard compiler and avoid multiple
def errors.
...
#define DEBUGLINE() printf("%s:%d\n", __FILE__, __LINE__);
  ...
  For example, -d < --dimension < --dmn, and also lower come before upper. The default STL std::string compare doesn't do
  that.
  */
  ...
  */
  ...
  >, >=. Use two values when requiring a test such as <x<, <=x<, <x<=, <=x<=.
  ...
  {                                                                                                                         \
    ...
    if (type != T) {
      ...
      case S8: {
        ...
        if (isdigit(valueAsString) && (valueAsString->size() > 18) && valueAsString->compare("9223372036854775807") > 0) {
          if (!quiet)
      ...
      case U8: {
        ...
        if (isdigit(valueAsString) && (valueAsString->size() > 19) && valueAsString->compare("18446744073709551615") > 0) {
          if (!quiet)
...
#define VALIDATE(T, U, LIST)                                                                                                \
  ...
  public:
    ...
    inline void add(const char* defaults,
                    bool required,
                    int expectArgs,
                    char delim,
                    const char* help,
  ...
  /* ################################################################### */
  void ezOptionParser::add(const char* defaults,
                           bool required,
                           int expectArgs,
                           char delim,
                           const char* help,
  ...
  void ezOptionParser::getUsageDescriptions(std::string& usage, int width, Layout layout) {
    ...
    for (i = 0; i < (long int)groups.size(); ++i) {
      ...
      // All the following split-fu could be optimized by just using substring (offset, length) tuples, but just to get it
      // done, we'll do some not-too expensive string copying.
      SplitDelim(groups[k]->help, '\n', desc);
      // Split lines longer than allowable help width.
      for (insertionIter = desc.begin(), cIter = insertionIter++; cIter != desc.end(); cIter = insertionIter++) {
